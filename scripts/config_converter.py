...
def parse_old_config(old_config, new_config):
    try:
        config = configparser.ConfigParser(strict=False, delimiters=(' '),
                                           ...
                                           allow_no_value=True)
        # Do not parse options and values. Capittal letters are relevant.
    ...
    for section in config.sections():
        match = re.match("(Bdev|Nvme|Malloc|VirtioUser\d+|Split|Pmem|AIO|"
                         "iSCSI|PortalGroup\d+|InitiatorGroup\d+|TargetNode\d+|"
                         "Nvmf|Subsystem\d+|VhostScsi\d+|VhostBlk\d+|VhostNvme\d+)",
                         section)
        if match:
            match_section = ''.join(letter for letter in match.group(0)
                                    if not letter.isdigit())
            items = section_to_method[match_section](config, section)
            for item in items:
                if match_section == "VhostScsi":
                    ...
                    section_to_subsystem[match_section][item['method']].append(item)
        elif section == "Global":
            pass
        else:
    ...
    with open(new_config, "w") as new_file:
        json.dump(generate_new_json_config(), new_file, indent=2)
        new_file.write("\n")
...
def get_nvmf_json(config, section):
    params = [
    ...
    for option in config.options(name):
        if "Portal" == option:
            for value in config.get(name, option).split("\n"):
                items = re.findall("\S+", value)
                portal = {'host': items[1].rsplit(":", 1)[0].replace(
                    "[", "").replace("]", "")}
                if "@" in items[1]:
                    portal['port'] = items[1].rsplit(":")[1].split("@")[0]
                    portal['cpumask'] = items[1].rsplit(":")[1].split("@")[1]
                else:
...
def get_bdev_json(config, section):
    params = [
...
def get_aio_json(config, section):
    aio_json = []
    value = None
...
def get_nvme_json(config, section):
    params = [
...
def get_split_json(config, section):
    split_json = []
    value = []
...
def get_malloc_json(config, section):
    malloc_json = []
    params = [
        ['NumberOfLuns', '', int, -1],
        ['LunSizeInMB', '', int, 20],
...
section_to_method = {
    "Nvme": get_nvme_json,
    "Malloc": get_malloc_json,
    "VirtioUser": get_virtio_user_json,
    "Split": get_split_json,
    "iSCSI": get_iscsi_json,
    "Pmem": get_pmem_json,
    "AIO": get_aio_json,
    "Bdev": get_bdev_json,
    "PortalGroup": get_portal_group_json,
    "InitiatorGroup": get_initiator_group_json,
    "TargetNode": get_target_node_json,
    "Nvmf": get_nvmf_json,
    "Subsystem": get_subsystem_json,
    "VhostScsi": get_vhost_scsi_json,
    "VhostBlk": get_vhost_blk_json,
    "VhostNvme": get_vhost_nvme_json
}
