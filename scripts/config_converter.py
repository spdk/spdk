...
def add_missing_parts():
    set_bdev_nvme_hotplug = False

    if not set_bdev_nvme_hotplug:
        for subsystem in new_json_config['subsystems']:
            if subsystem['subsystem'] == "bdev":
                for item in subsystem['config']:
                    if item['method'] == set_bdev_nvme_hotplug:
                        set_bdev_nvme_hotplug = True
                if not set_bdev_nvme_hotplug:
                    index = get_bdev_insert_index('set_bdev_nvme_hotplug')
                    subsystem['config'].insert(index, {
                        "params": {
                            "enable": False,
                            "period_us": 10000000
                        },
...
def parse_old_config(old_config, new_config):
    ...
    if "[Nvme]" not in headers:
         headers.append("[Nvme]")
         header_setup["[Nvme]"] = []
         parsed_headers["[Nvme]"] = False
    if "[Nvmf]" not in headers:
         headers.append("[Nvmf]")
         header_setup["[Nvmf]"] = []
         parsed_headers["[Nvmf]"] = False
    if "[Bdev]" not in headers:
         headers.append("[Bdev]")
         header_setup["[Bdev]"] = []
         parsed_headers["[Bdev]"] = False
    if "[iSCSI]" not in headers:
         headers.append("[iSCSI]")
         header_setup["[iSCSI]"] = []
         parsed_headers["[iSCSI]"] = False
...
def get_iscsi_json(setup):
    ...
    iscsi_json['params']['max_session'] = max_session
    ...
    print "iscsi_json %s" % iscsi_tmp_json['params']
