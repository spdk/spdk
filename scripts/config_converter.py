...
def get_vhost_blk_json(setup):
    ...
    return { "method": "construct_vhost_blk_controller", "params": to_json_params(params)}
...
def get_nvmf_json(setup):
    ...
    for item in setup:
        item = re.search("(AcceptorPollRate|MaxQueuesPerSession|MaxQueueDepth|"\
                         "InCapsuleDataSize|MaxIOSize|IOUnitSize)[\s]+(\S+)", item)
        if not item:
            continue
        set_param(params, item.groups()[0], item.groups()[1])
    nvmf_json = []
...
def get_malloc_json(setup):
    ...
    for item in setup:
        item = re.search("(NumberOfLuns|LunSizeInMB|BlockSize)[\s]+(\S+)", item)
        if not item:
            continue
        set_param(params, item.groups()[0], item.groups()[1])
...
def get_iscsi_json(setup):
    ...
    for item in setup:
        item = re.search("(AllowDuplicateIsid|DefaultTime2Retain|DiscoveryAuthMethod|"\
                         "MaxConnectionsPerSession|Timeout|DiscoveryAuthMethod|NodeBase|"\
                         "AuthFile|DiscoveryAuthGroup|MaxSessions|ImmediateData|"\
                         "ErrorRecoveryLevel|NopInInterval|MinConnectionsPerCore|"\
                         "DefaultTime2Wait|QueueDepth)[\s]+(\S+)", item)
        if not item:
            continue
        set_param(params, item.groups()[0], item.groups()[1])
