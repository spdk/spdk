        ...
        if re.match("\[VirtioUser\d+\]", header):
            ...
            for subsystem in new_json_config['subsystems']:
                if subsystem['subsystem'] == "bdev":
                    ...
                    subsystem['config'].insert(index, virti_json)
...
def get_target_node_json(setup, name):
    ...
    print "jnjlkj %s" % target_tmp_json['params']
    ...
    for item in setup:
        ...
        if "Portal" == item[0]:
            ...
            if "@" in item[2]:
                ...
                portal['cpumask'] = cpumask #item[2].split(":")[1].split("@")[1]
            else:
...
def get_iscsi_json(setup):
    ...
    for item in setup:
        ...
        #    iscsi_json['params']['discovery_auth_method'] = item[1]
        elif "DiscoveryAuthGroup" == item[0]:
            iscsi_json['params']['discovery_auth_group'] = int(item[1].replace("AuthGroup", ""))
        elif "MaxSessions" == item[0]:
            iscsi_json['params']['max_sessions'] = int(item[1])
        elif "ImmediateData" == item[0]:
            iscsi_json['params']['immediate_data'] = False
            if 'yes' == item[1].lower():
                iscsi_json['params']['immediate_data'] = True
        elif "ErrorRecoveryLevel" == item[0]:
            iscsi_json['params']['error_recovery_level'] = int(item[1])
        #elif "MaxR2T" == item[0]:
        #    iscsi_json['params']['max_r2t'] = item[1]
        elif "NopInInterval" == item[0]:
            iscsi_json['params']['nop_in_interval'] = int(item[1])
        elif "AllowDuplicatedIsid" == item[0]:
            allow_duplicated_isid = item[1]
        elif "DefaultTime2Retain" == item[0]:
            default_time2retain = item[1]
        elif "DefaultTime2Wait" == item[0]:
            iscsi_json['params']['default_time2wait'] = int(item[1])
        elif "MaxConnectionsPerSession" == item[0]:
            max_connections_per_session = item[1]
        elif "QueueDepth" == item[0]:
            iscsi_json['params']['max_queue_depth'] = 64 #int(item[1])
        elif "MinConnectionsPerCore" == item[0]:
            iscsi_json['params']['min_connections_per_core'] = int(item[1])
    ...
    iscsi_tmp_json = {'params': {}, 'method': iscsi_json['method']}
    for k in sorted(iscsi_json['params'], key=iscsi_json['params'].get, reverse=True):
       iscsi_tmp_json['params'][k] = iscsi_json['params'][k]
    print "jnjlkj %s" % iscsi_tmp_json['params']

    return iscsi_tmp_json
