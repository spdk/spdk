    ...
    if last_stat:
        ...
        for bdev in stat['bdevs']:
            ...
            info_stat = [
                _stat.bdev_name,
                str(tps),
                str(round(_percent_cal(_stat.rd_sectors, upt, 2) / unit, 2)),
                str(round(_percent_cal(_stat.wr_sectors, upt, 2) / unit, 2)),
                str(round(_percent_cal(_stat.dc_sectors, upt, 2) / unit, 2)),
                str(round(_stat.rd_sectors / unit, 2)),
                str(round(_stat.wr_sectors / unit, 2)),
...
def io_stat_display_loop(args):
    ...
    if interval == 0 and time_in_second == 0:
