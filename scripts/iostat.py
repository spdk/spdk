...
def _stat_format(data, header, leave_first=False):
    ...
    format = '  '.join('%%-%ss' % item_sizes[i] for i in range(0, header_len))
...
def read_cpu_stat(last_cpu_info, cpu_info):
    ...
    if last_cpu_info:
        info_stat = [
            ...
            _cpu_percent_cal_diff(cpu_info[2] + cpu_info[5]
                                  + cpu_info[6], last_cpu_info[2]
                                  + last_cpu_info[5] + last_cpu_info[6], jiffies),
        ...
        info_stat = [
            ...
            _cpu_percent_cal(cpu_info[2] + cpu_info[5]
                             + cpu_info[6], jiffies),
...
def get_cpu_stat(flags=0):
    ...
    while line:
        ...
        if "cpu " in line and flags == 0:
            ...
            cpu_dump_info = [int(data) for data in line.split(' ')]
    ...
    if last_stat:
        for bdev in stat['bdevs']:
            ...
            info_stat = [
                _stat.bdev_name,
                tps,
                round(_percent_cal_diff(_stat.rd_sectors,
                                        _last_stat.rd_sectors, upt, 2) / unit, 2),
                round(_percent_cal_diff(_stat.wr_sectors,
                                        _last_stat.wr_sectors, upt, 2) / unit, 2),
                round(_percent_cal_diff(_stat.dc_sectors,
        ...
        for bdev in stat['bdevs']:
            ...
            tps = _percent_cal(
                _stat.rd_ios + _stat.dc_ios + _stat.wr_ios, upt, 2)
            info_stat = [
                _stat.bdev_name,
                tps,
                round(_percent_cal(_stat.rd_sectors, upt, 2) / unit, 2),
                round(_percent_cal(_stat.wr_sectors, upt, 2) / unit, 2),
                round(_percent_cal(_stat.dc_sectors, upt, 2) / unit, 2),
                round(_stat.rd_sectors / unit, 2),
                round(_stat.wr_sectors / unit, 2),
                round(_stat.dc_sectors / unit, 2)
            ]
            info_stats.append(info_stat)
...
def io_stat_display_loop(args):
    ...
    if time_in_second == 0:
        io_stat_display(args, last_cpu_stat, bdev_stats)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument('-u', '--use-uptime', dest='use_uptime',
                        action='store_true', help='Use uptime or spdk ticks(default) as the interval variable to calculate iostat changes.',
                        required=False, default=False)

    parser.add_argument('-i', '--interval', dest='interval',
                        type=check_positive, help='Time interval (in seconds) on which to poll I/O stats. Used in conjunction with -t',
                        required=False, default=0)

    parser.add_argument('-t', '--time', dest='time_in_second',
                        type=check_positive, help='The number of second to display stats before returning. Used in conjunction with -i',
                        required=False, default=0)
