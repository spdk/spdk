...
class memory:
    ...
    def print_summary(self):
        print("DPDK memory size {} in {} heap(s), {} mempool(s), and {} independent memzone(s)"
              .format(B_to_MiB(self.size), len(self.heaps), len(self.mempools), len(self.memzones)))
        print("heaps totaling size {}".format(B_to_MiB(self.get_total_heap_size())))
        for x in sorted(self.heaps, key=lambda x: x.size, reverse=True):
            x.print_summary('  ')
        print("end heaps----------")
        print("mempools totaling size {}".format(B_to_MiB(self.get_total_mempool_size())))
        for x in sorted(self.mempools, key=lambda x: x.get_memzone_size_sum(), reverse=True):
            x.print_summary('  ')
        print("end mempools-------")
        print("memzones totaling size {}".format(B_to_MiB(self.get_total_memzone_size())))
...
class heap_element:
    ...
    def print_summary(self, header):
        print("{}element at address: {} with size: {}".format(header, hex(self.addr), B_to_MiB(self.size)))
...
class heap:
    ...
    def print_element_stats(self, element_type, header):
        ...
        print("{}list of {} elements. size: {}".format(header, list_type, B_to_MiB(self.get_element_size(element_type))))
        for x in sorted(list_to_check, key=lambda x: x.size, reverse=True):
    ...
    def print_summary(self, header):
        print("{}size: {} heap id: {}".format(header, B_to_MiB(self.size), self.id))

    def print_detailed_stats(self):
        print("heap id: {} total size: {} number of busy elements: {} number of free elements: {}"
              .format(self.id, B_to_MiB(self.size), len(self.busy_elements), len(self.free_elements)))
...
class mempool:
    ...
    def print_detailed_stats(self):
        print("size: {} name: {} comprised of {} memzone(s):"
...
class memzone:
    ...
    def print_summary(self, header):
        print("{}size: {} name: {}".format(header,  B_to_MiB(self.size), self.name))
...
class segment:
    ...
    def print_summary(self, header):
        print("{}address: {} length: {}".format(header, hex(self.address), B_to_MiB(self.size)))
