...
function usage()
{
	...
	echo "$0"
	echo "  -h --help"
	echo "  -i --install-crypto Install ipsec"
	echo "  -l --install-isal Install isa-l"
	echo ""
	exit 0
}
...
while getopts 'hil-:' optchar; do
	case "$optchar" in
		-)
		case "$OPTARG" in
			help) usage;;
			install-crypto) INSTALL_CRYPTO=true;;
			install-reduce) INSTALL_ISAL=true;;
...
if $INSTALL_ISAL; then
	...
	if [ "$isal" == "" ]; then
		...
		if [ "$isal_submodule_cloned" != "" ]; then
			su - $SUDO_USER -c "cd $rootdir/isa-l; ./autogen.sh"
			su - $SUDO_USER -c "cd $rootdir/isa-l; ./configure"
