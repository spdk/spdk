...
# compression dependency that must run as root
isal="$(find /usr -name isa-l.h 2>/dev/null)"
if [ "$ipsec" == "" ]; then
	isal_submodule_cloned="$(find $rootdir/isa-l -name isa-l.h 2>/dev/null)"
...
# crypto dependency that must run as root
