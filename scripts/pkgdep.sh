...
if [ -s /etc/redhat-release ]; then
	...
	if [[ -n "$http_proxy" ]]; then
		gpg_options=" --keyserver hkp://keys.gnupg.net:80 --keyserver-options \"http-proxy=$http_proxy\""
	fi
	...
	# Additional dependencies for building pmem based backends
	# Use github instead of AUR because the latter is not frequently updated.
	# ldconfig needs to be updated manually, otherwise it will break fio installation
	# (from vm_setup.sh) which detects pmdk headers, but cannot find libraries on runtime
