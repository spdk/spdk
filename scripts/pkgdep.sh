...
function usage()
{
	...
	echo "  -p --install-compress Install isa-l dependencies"
	echo ""
	exit 0
}

INSTALL_CRYPTO=false
INSTALL_COMPRESS=false

while getopts 'hip-:' optchar; do
	case "$optchar" in
		-)
		case "$OPTARG" in
			help) usage;;
			install-crypto) INSTALL_CRYPTO=true;;
			install-compress) INSTALL_COMPRESS=true;;
...
if [ -s /etc/redhat-release ]; then
	...
	# Additional dependencies for ISA-L used in compression
	if $INSTALL_COMPRESS; then
		zypper install -y autoconf automake
	fi
...
if [[ $INSTALL_CRYPTO ]] || [[ $INSTALL_COMPRESS ]]; then
