...
function usage()
{
	...
	echo "$0"
	echo "  -h --help"
	echo "  -d   build with develoer dependencies"
	echo ""
	exit 0
}
...
while getopts 'dhi-:' optchar; do
	case "$optchar" in
		...
		;;
	h) usage;;
	d) INSTALL_DEV_DEPENDENCIES=true;;
	*) echo "Invalid argument '$OPTARG'"
	usage;;
	esac
done
...
if [ -s /etc/redhat-release ]; then
	...
	# Additional dependencies for DPDK
	if [[ $NAME == "Ubuntu" ]] && [[ $VERSION -lt 19 ]]; then
	...
	if [[ $NAME == "Ubuntu" ]] && [[ $VERSION -gt 18 ]]; then
	...
	if [[ $INSTALL_DEV_DEPENDENCIES == "true" ]]; then
		...
		# Additional dependencies for FUSE and CUSE
		if [[ $NAME == "Ubuntu" ]] && [[ $VERSION -gt 14 ]] && [[ $VERSION -lt 19 ]]; then
			...
			sudo add-apt-repository ppa:bkryza/fuse3
			apt-get update
		fi
