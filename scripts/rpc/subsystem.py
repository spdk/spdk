    ...
    for bdev in reversed(bdev_config):
        ...
        if 'method' in bdev:
            if bdev['method'] == 'construct_nvme_bdev':
                for rpc_bdev in bdev_config:
                    rpc_bdev_name = ""
                    if 'params' in rpc_bdev and 'name' in rpc_bdev['params']:
                        rpc_bdev_name = bdev['params']['name']
                    if bdev_name in rpc_bdev_name:
                        args.client.call("delete_bdev", {'name': "%s" % rpc_bdev_name})
