...
def construct_target_node(
    ...
    Args:
        luns: List of bdev_name_id_pairs, e.g. [{"bdev_name": Malloc1, "lun_id": int(1)}]
        pg_ig_maps: List of pg_ig_mappings, e.g. [{"pg_tag": int(pg), "ig_tag": int(ig)}]
        name: Target node name (ASCII)
        alias_name: Target node alias name (ASCII)
        queue_depth: CHAP for discovery session should be disabled(optional)
        chap_group: CHAP for discovery session should be required
        disable_chap:CHAP for discovery session should be mutual
        require_chap:Authentication group ID for discovery session
        mutual_chap:Maximum number of sessions in the host
        header_digest:Negotiated parameter, MaxConnections
        data_digest:Negotiated parameter, DefaultTime2Wait
...
def target_node_add_lun(client, name, bdev_name, lun_id=None):
    ...
    Args:
        name: Target node name (ASCII)
        bdev_name: bdev name enclosed in quotes
