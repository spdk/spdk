...
sudo dnf install -y sudo dnf install libpmemblk-devel
...
# Note: the version of iscsiadm that ships with fedora 26 was broken as of November 3 2017.
...
cd qemu
git fetch https://review.gerrithub.io/spdk/qemu refs/changes/12/385312/1 && git checkout FETCH_HEAD
cd ~
