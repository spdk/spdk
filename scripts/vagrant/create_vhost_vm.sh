...
display_help() {
	...
	echo "  --move-to-def-dirs		    Move generated files to default directories used by vhost test scripts."
...
while getopts ":xh-:" opt; do
	case "${opt}" in
		...
		case "${OPTARG}" in
			...
			move-to-def-dirs) MOVE_TO_DEF_DIRS=true ;;
			*) echo "Invalid aaargument '$OPTARG'" ;;
		esac
		;;
		x)
			http_proxy=$OPTARG
		...
		;;
		*)
			echo "  Invalid bbargument: -$OPTARG" >&2
			echo "  Try: \"$0 -h\"" >&2
			exit 1
		;;
	esac
...
case "$SPDK_VAGRANT_DISTRO" in
	...
	;;
	*)
		echo "  Invalid argumentxx \"${SPDK_VAGRANT_DISTRO}\""
		echo "  Try: \"$0 -h\"" >&2
		exit 1
	;;
esac
...
if $MOVE_TO_DEF_DIRS;then
	mkdir -p /home/sys_sgsw
	mv -f ${VAGRANT_TARGET}/${SPDK_VAGRANT_DISTRO}/vhost_vm_image.qcow2 /home/sys_sgsw/vhost_vm_image.qcow2
	mv -f ${VAGRANT_TARGET}/${SPDK_VAGRANT_DISTRO}/ssh_keys/spdk_vhost_id_rsa* ~/.ssh/
fi
