...
Usage: $0 [-reh] [--recreate|eradicate|help]
  ...
  -e|--eradicate : After running the case, eradicate the virtual machine.
                 : Without parameters means a VM which already exists. If there is not VM, it will create
                 : a new default VM with VM_OS=fedora28, MEMORY_SIZE=16384, NCPU=16.
                 : if you want to change them ,please export variables. Such as export NCPU=8, MEMORY_SIZE=8096
                 : VM_OS now only support fedora28, and the default VM provider is virtualbox.
  -h|--help      : Show help.
EOF
}
function vm_create() {
