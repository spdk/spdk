...
function vm_setup_env() {
	...
	vagrant_run_cmd "sudo spdk_repo/spdk/test/common/config/vm_setup.sh -i $conf"
}
...
SPDK_TEST_VHOST=${SPDK_TEST_VHOST:-0}

if [ $SPDK_TEST_VHOST -eq 1 ]; then
	conf=""
else
	conf="-t librxe,iscsi,rocksdb,fio,flamegraph,tsocks,vpp,libiscsi,nvmecli,qat"
fi
