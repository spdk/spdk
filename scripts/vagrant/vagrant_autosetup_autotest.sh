...
#This is left for debug.sometimes we need to check some kinds issue by setting without code commited.
...
Usage: $0 [-reh] [--recreate|eradicate|help]
  ...
  -e|--eradicate : After running the case, eradicate the virtual machine.
                 : Without parameters means there's a VM which already exist.
  -h|--help      : Show help.
EOF
}
function vm_create() {
...
# we can left this in the config file
