...
if ! echo "$SPDK_VAGRANT_DISTRO" | grep -q -E 'fedora|ubuntu|centos' && [ $DEPLOY_TEST_VM -eq 1 ]; then
	echo "Warning: Test machine deployment is available on fedora/ubuntu/centos distros. Disabling it for this build"
	DEPLOY_TEST_VM=0
fi
