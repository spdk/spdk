...
display_help() {
	...
	echo "  --vhost-emulator-dir=<path>     directory patch with emulator, default: ${SPDK_VAGRANT_EMULATOR}"
	echo "  -r dry-run"
...
SPDK_VAGRANT_EMULATOR="/usr/bin/kvm-spices"
OPTIND=1
...
while getopts ":b:n:s:x:p:vrldh-:" opt; do
	case "${opt}" in
		...
		case "${OPTARG}" in
			...
			vhost-emulator-dir=*) SPDK_VAGRANT_EMULATOR="${OPTARG#*=}" ;;
...
if [ ${VERBOSE} = 1 ]; then
	...
	echo SPDK_VAGRANT_EMULATOR=$SPDK_VAGRANT_EMULATOR
	echo
fi
...
if [ ${DRY_RUN} = 1 ]; then
	...
	printenv SPDK_VAGRANT_EMULATOR
	printenv SPDK_DIR
fi
