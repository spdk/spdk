...
function vagrant_provision()
{
	if [[ "$(vagrant_get_powerstatus)" = poweroff && $PROVIDER = virtualbox ]]; then
		#New version vbox6.0 no longer needs this,vagrant version can't support it.
		if [[ ${VERSION%%.*}<6 ]];then
			if vagrant plugin list | grep -q vagrant-vbguest; then
				echo "vagrant-vbguest already installed... skipping"
			else
				vagrant plugin install vagrant-vbguest
			fi
		fi
	elif [[ "$(vagrant_get_powerstatus)" = shutoff && $PROVIDER = libvirt ]]; then
		if vagrant plugin list | grep -q vagrant-libvirt; then
			echo "vagrant-libvirt already installed... skipping"
		else
...
function clear_provider_mutex()
{
	if [ $PROVIDER = virtualbox ]; then
		PID=`ps -ef | grep '/usr/bin/qemu-system-x86_64 -machine accel=kvm'| awk '{print $2}'| head -n 1`
	else
		PID=`ps -ef | grep '/usr/lib/virtualbox/VBoxHeadless --comment'| awk '{print $2}'| head -n 1`
	fi
	#This will got a PID of virtualbox or libvirt executing process,at least command itself.
	kill -9 $PID || true
}
...
Usage: $0 [-resvch] [--recreate|eradicate|ssh-machine|vhost|check|help]
	...
	-c|--check: help to list all the VMs. and HDDs , NVME that attached to VM.
				If need to display virtual bridge
...
{
	vboxmanage list hdds|grep -E 'UUID|Location'
	vagrant global-status --prune| grep default | grep -E 'virtualbox|libvirt'
	vboxmanage list vms | grep -E 'virtualbox|libvirt'
	if [[ `vboxmanage list vms | grep -E 'virtualbox|libvirt'| wc -l` != `vagrant global-status --prune| grep default | grep -E 'virtualbox|libvirt' |wc -l` ]]; then
...
function vm_check_nvme_device()
{
	local cmdset="cd spdk_repo/spdk;\
		...
		sudo ./configure --enable-debug && make clean && make -j;\
...
function VM_create_demo_check()
{
	...
	clear_provider_mutex
	if vagrant plugin list | grep -q vagrant-vbguest; then
		if [[ ${VERSION%%.*}>=6 ]];then
			echo "vagrant-vbguest already installed... uninstall it for new version not support!"
