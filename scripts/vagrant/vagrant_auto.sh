...
function vagrant_provision()
{
	if [ "$(vagrant_get_powerstatus)" != 'running' ]; then
		vagrant_up || true
	elif [ $NCPU != 16 ] || [ $MEMORY_SIZE != 16384 ]; then
		vagrant_reload || true
		return
	fi
...
Usage: $0 [-resvch] [--recreate|eradicate|ssh-machine|vhost|check|help]
	Command usage:
				If need to switch HW config:
				   export NCPU=2
				   export VM_OS=fedora31/fedora30/ubuntu18
...
function clear_resource_conflicts()
{
	...
	if [ $PROVIDER = virtualbox ]; then
		...
		NUM=$(VBoxManage list hostonlyifs | grep -c HostInterfaceNetworking-vboxnet)
