...
Usage: $0 [-reh] [--recreate|eradicate|help]
  ...
  -e|--eradicate : Make sure you need to destroy all the directory. After running the case, eradicate the virtual machine,box files are all cleared.
                 : Without parameter means to re-run this case based on a virtulmachine which already created.
  -h|--help      : Show help.
EOF
}
