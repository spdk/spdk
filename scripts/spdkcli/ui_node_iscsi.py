...
class UIISCSIGlobalParams(UINode):
    ...
    def ui_command_set_auth(self, chap_group=None, disable_chap=None,
        ...
        Optional arguments:
            disable_chap: CHAP for discovery session should be disabled
...
class UIISCSIDevices(UINode):
    ...
    def ui_command_create(self, name, alias_name, bdev_name_id_pairs,
        ...
        chap_group = g if g else 0
        disable_chap = True if d else False
        require_chap = True if r else False
        mutual_chap = True if m else False
        header_digest = True if h else False
        data_digest = True if t else False
        try:
...
class UIISCSIDevice(UINode):
    ...
    def refresh(self):
        ...
        UIISCSIAuthentications(disc_service, self)
...
class UIISCSIAuthentications(UINode):
    def __init__(self, authentications, parent):
        UINode.__init__(self, "authentications", parent)
        self.authentications = authentications
        self.refresh()

    def refresh(self):
        self._children = set([])
        UIISCSIAuthentication(
            "disable_chap: %s" % self.authentications['disable_chap'], self)
        UIISCSIAuthentication(
            "require_chap: %s" % self.authentications['require_chap'], self)
        UIISCSIAuthentication(
            "mutual_chap: %s" % self.authentications['mutual_chap'], self)
        UIISCSIAuthentication(
            "chap_group: %s" % self.authentications['chap_group'], self)
        UIISCSIAuthentication(
            "data_digest: %s" % self.authentications['data_digest'], self)


class UIISCSIAuthentication(UINode):
    def __init__(self, authentication_to_display, parent):
        UINode.__init__(self, authentication_to_display, parent)
        self.authentication_to_display = authentication_to_display
        self.refresh()
...
class UIPortalGroups(UINode):
    ...
    def refresh(self):
        ...
        for pg in self.get_root().get_portal_groups():
            UIPortalGroupTag(pg, self)


class UIPortalGroupTag(UINode):
    def __init__(self, pg, parent):
        UINode.__init__(self, "%s" % pg.tag, parent)
        self.pg = pg
        self.refresh()
    ...
    def refresh(self):
        ...
        for portal in self.pg.portals:
            UIPortalGroup(portal['host'], portal['port'], portal['cpumask'], self)

    def summary(self):
        return "Portals: %s" % len(self.pg.portals), None
...
class UIInitiatorGroups(UINode):
    ...
    def refresh(self):
        ...
        for ig in self.get_root().get_initiator_groups():
            UIInitiatorGroupTag(ig, self)


class UIInitiatorGroupTag(UINode):
    def __init__(self, ig, parent):
        UINode.__init__(self, "%s" % ig.tag, parent)
        self.ig = ig
        self.refresh()
    ...
    def refresh(self):
        ...
        for initiator, netmask in zip(self.ig.initiators, self.ig.netmasks):
            UIInitiatorGroup(initiator, netmask, self)

    def summary(self):
        return "Initiators: %s" % len(self.ig.initiators), None


class UIInitiatorGroup(UINode):
    def __init__(self, initiator, netmask, parent):
        UINode.__init__(self, "%s %s" % (initiator, netmask), parent)
        self.refresh()
...
class UIISCSIAuthGroups(UINode):
    ...
    def ui_command_create(self, tag, secrets=None):
        ...
        Optional args:
           secrets: Array of secrets objects separated by equal sign,
