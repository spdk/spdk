    ...
    def refresh(self):
        ...
        for device in self.get_root().get_scsi_devices():
            for node in self.get_root().get_target_nodes():
                if hasattr(device, "device_name") and node['name'] == device.device_name:
    ...
    def ui_command_delete(self, name=None):
        ...
        if name is None:
            for device in self.devices:
                try:
                     self.get_root().delete_target_node(target_node_name=device.device_name)
                except JSONRPCException as e:
                    self.shell.log.error(e.message)
        else:
            try:
...
class UIScsiDevice(UINode):
    ...
    def summary(self):
        return "Id: %s, QueueDepth: %s" % (self.device.id, self.target['queue_depth']), None
...
class UIScsiDiscoveryServices(UINode):
    ...
    def refresh(self):
        self._children = set([])
        UIScsiDiscoveryService("disable_chap: %s" % self.discovery_service['disable_chap'], self)
        UIScsiDiscoveryService("require_chap: %s" % self.discovery_service['require_chap'], self)
        UIScsiDiscoveryService("mutual_chap: %s" % self.discovery_service['mutual_chap'], self)
        UIScsiDiscoveryService("chap_group: %s" % self.discovery_service['chap_group'], self)
        UIScsiDiscoveryService("data_digest: %s" % self.discovery_service['data_digest'], self)
...
class UIScsiLun(UINode):
    def __init__(self, lun, parent):
        UINode.__init__(self, "%s ID:%s" % (lun['bdev_name'], lun['lun_id']), parent)
        self.lun = lun
        self.refresh()
...
class UIScsiPgIg(UINode):
    def __init__(self, pg_ig, parent):
        UINode.__init__(self, "PG:%s IG:%s" % (pg_ig['pg_tag'], pg_ig['ig_tag']), parent)
        self.pg_ig = pg_ig
        self.refresh()
...
class UIISCSiAuthGroups(UINode):
    ...
    def refresh(self):
        ...
        print "iscsgr: %s" % iscsi_auth_groups
    ...
    def ui_command_create(self, tag, secrets=None):
        tag = self.ui_eval_param(tag, "number", None)
        if secrets:
            secrets = [dict(u.split(":") for u in a.split("-")) for a in secrets.split("=")]
        try:
    ...
    def ui_command_add_secret(self, tag, user, secret, muser=None, msecret=None):
        tag = self.ui_eval_param(tag, "number", None)
        try:
...
class UISCSiAuthSecret(UINode):
    def __init__(self, secret, parent):
        info = ", ".join("%s=%s" % (key,val) for key, val in secret.items())
