...
class UISubsystems(UINode):
    ...
    def ui_command_create(self, nqn, serial_number, listen=None, hosts=None, namespaces=None,
        ...
        listen - Optional parameter. Equals sign-separated list of Listen
                 <trtype:transport_name-traddr:address-trsvcid:port_id>.
        hosts - Optional parameter. Equals sign-separated list of host nqn list.
        namespaces - Optional parameter. Equals sign-separated list of namespaces.
        max_namespaces - Optional parameter. Maximum number of namespaces allowed to added during
                         active connection
        allow_any_host - Optional paremeter. Allow any host to connect (don't enforce host NQN
                         whitelist)
        """
        if listen:
            listen = [dict(u.split(":", 1)
                           for u in a.split("-")) for a in listen.split("=")]
        if hosts:
            hosts = hosts.split("=")
        if namespaces:
            tmp_namespaces = namespaces.split("=")
            namespaces = []
            for namespace in tmp_namespaces:
                bdev_name = namespace
                nsid = 0
                if ':' in namespace:
                    (bdev_name, nsid) = namespace.split(":")
...
class UISubsystemListeners(UINode):
    ...
    def ui_command_create(self, trtype, traddr, trsvcid, adrfam):
        ...
        if trtype is None or traddr is None or trsvcid is None or adrfam is None:
            self.shell.log.error("Please define all parameters.")
            return
        try:
