...
for driver in ${allowed_drivers[@]}; do
	if [ $driver == $driver_to_bind ]; then
		found_valid_driver=1;
	fi
done

if [ $found_valid_driver -eq 0 ]; then
	echo "No driver specified, defaulting to uio_pci_generic."
	for driver in ${allowed_drivers[@]}; do
		echo $driver
	done
	exit 1
...
for qat_bdf in $(lspci -Dd:37c8 | awk '{print $1}'); do
...
vf_bdfs=$(lspci -Dd:37c9 | awk '{print $1}')

# Unbind old driver if necessary.
for vf in $vf_bdfs; do
...
for vf in $vf_bdfs; do
	if ! ls -l /sys/bus/pci/devices/$vf/driver | grep -q $driver_to_bind; then
		...
		if [ "$driver_to_bind" == "uio_pci_generic" ]; then
			echo "Your version of the uio_pci_generic driver does not support binding to virtual functions."
