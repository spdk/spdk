...
parser.add_argument('-r', '--runtime', default="10", type=str,
                    help="Run time param for FIO. Default: 10")
parser.add_argument('-R', '--ramptime', default="10", type=str,
                    help="Ramp time param for FIO. Default: 10")
parser.add_argument('-c', '--ctrl-type', default="spdk_vhost_scsi", type=str,
                    help="Type of vhost controller to use in test. Default: spdk_vhost_scsi")
...
parser.add_argument('-d', '--max-disks', default=0, type=int,
                    help="How many disks to use in test. Default: all disks")
parser.add_argument('-v', '--vm-count', default=1, type=int,
                    help="How many VMs to run in test. Default: 1")

subparsers = parser.add_subparsers()
cpu_cfg_create_subparser = subparsers.add_parser('create_cpu_cfg',
                                                 help="Generate a CPU config file for test")
cpu_cfg_create_subparser.add_argument('spdk_cpu_list', default=None)
cpu_cfg_create_subparser.add_argument('vm_cpu_num', default=None, type=int)

cpu_cfg_load_subparser = subparsers.add_parser('load_cpu_cfg',
                                               help="Load and use a CPU config file for test \
                                               Example configuration files can be found in: \
                                               test/vhost/common/autotest.config")
cpu_cfg_load_subparser.add_argument('custom_mask_file', default=None)
