...
class Initiator(Server):
    def __init__(self, name, username, password, mode, nic_ips, ip, luns=1, iscsiadm_bin="iscsiadm", workspace="/tmp/spdk"):
    ...
    def discover_subsystems(self, address_list, subsys_no):
        ...
        n = int(self.num_luns / len(self.nic_ips))
...
class SPDKTarget(Target):
    def __init__(self, name, username, password, mode, nic_ips, num_cores, num_shared_buffers=4096,
                 use_null_block=False, portals=None, initiators=None, netmasks=None, num_luns=1, **kwargs):
    ...
    def spdk_tgt_add_nullblock(self, req_num_disks=None):
        ...
        for lun_id in range(self.num_luns):
            self.luns.append({"lun_id": lun_id, "bdev_name": "Nvme0n%s" % lun_id})
            rpc.bdev.construct_null_bdev(self.client, 102400, 4096, "Nvme0n%s" % lun_id)
...
class SPDKInitiator(Initiator):
    def __init__(self, name, username, password, mode, nic_ips, ip, num_luns=1, num_cores=None, **kwargs):
        super(SPDKInitiator, self).__init__(name, username, password, mode, nic_ips, ip, num_luns)
        if num_cores:
            self.num_cores = num_cores
        self.num_luns = num_luns
    ...
    def gen_fio_filename_conf(self, remote_subsystem_list):
        ...
        if hasattr(self, 'num_cores'):
            ...
            threads = range(0, int(self.num_luns))

        n = int(self.num_luns / len(threads))
