...
class VPP:
    def __init__(self, vpp_dir="/usr/local/src/vpp-19.04/build-root/install-vpp-native/vpp/"):
        self.mtu = None
        self.main_core = 0
        self.workers = 7
        self.corelist_workers = "1-8"

        self.ips = ["127.0.0.1", "127.0.0.2"]
        self.ifaces = ["EthernetSwitch8f/0/1", "EthernetSwitch90/0/1"]
        self.devices = ["0000:8f:00.1", "0000:90:00.1"]

        self.num_rx_queues = self.workers
        self.num_tx_queues = self.workers
    ...
    def configure(self):
            ...
            for iface, ip in zip(self.ifaces, self.ips):
...
class Target(Server):
    def __init__(self, name, username, password, mode, nic_ips, use_null_block=False):
...
class Initiator(Server):
    def __init__(self, name, username, password, mode, nic_ips, ip, iscsiadm_dir=None, workspace="/tmp/spdk"):
...
class SPDKTarget(Target):
    def __init__(self, name, username, password, mode, nic_ips, num_cores, num_shared_buffers=4096,
        ...
        self.portals = [{'host': "127.0.0.1", 'port': "3260", 'cpumask': "0xff"}]
        self.initiators = ["ANY"]
        self.netmasks = ["127.0.0.1/24"]
        self.luns = [{'lun_id': 0, 'bdev_name': "Nvme0n1"}]
    ...
    def spdk_tgt_configure(self):
        ...
        # TODO: create one target per LUN
    ...
    def spdk_tgt_add_subsystem_conf(self, ips=None, req_num_disks=None):
        ...
        self.log_print("SPDK iSCSI subsystem configuration:")
        rpc.client.print_dict(rpc.iscsi.get_target_nodes(self.client))
...
class SPDKInitiator(Initiator):
    def __init__(self, name, username, password, mode, nic_ips, ip, num_cores=None, **kwargs):
        super(SPDKInitiator, self).__init__(name, username, password, mode, nic_ips, ip)
        if num_cores:
            self.num_cores = num_cores
    ...
    def gen_spdk_bdev_conf(self, remote_subsystem_list):
        ...
        row_template = """  URL iscsi://{ip}:{port}/{iqn}/0 iSCSI{i}"""
