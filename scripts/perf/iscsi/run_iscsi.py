...
class VPP:
    def __init__(self, name, username, password, enabled=False,
                 ...
                 num_rx_queues=1, num_tx_queues=1, mode="dpdk", devices=[], ifs=[],
                 vpp_ifs=[], nic_ips=[], num_targets=1, buffers_per_numa=128000):
        self.mtu = mtu
        ...
        self.buffers_per_numa = buffers_per_numa
    ...
    def configure(self):
            ...
            if self.mode == "rdma":
                self.log_print("Configuring RDMA interfaces")
                count = 0
                for iface, ip in zip(self.ifs, self.nic_ips):
                    self.ctl("create int rdma host-if %s name rdma-%s" % (iface, count))
                    self.ctl("set interface state rdma-%s up" % count)
                    self.ctl("set interface ip address rdma-%s %s" % (count, ip))
                    if self.mtu is not None:
                        self.ctl("set interface mtu %s rdma-%s" % (self.mtu, count))
                    count += 1
            else:
                self.log_print("Configuring DPDK interfaces")
                for iface, ip in zip(self.vpp_ifs, self.nic_ips):
                    self.ctl("set interface state " + iface + " up")
                    self.ctl("set interface ip address " + iface + " " + ip)
                    if self.mtu is not None:
                        self.ctl("set interface mtu " + self.mtu + " " + iface)

            self.ctl("sh int address")
    ...
    def run(self):
        ...
        command += " buffers { buffers-per-numa " + str(self.buffers_per_numa) + " }"
        command += " tcp { cc-algo cubic }"
        command += " session { evt_qs_memfd_seg }"
        command += " socksvr { socket-name /run/vpp-api.sock }"

        if self.mode == "rdma":
            command += " plugins { plugin dpdk_plugin.so { disable } }"
        else:
            # DPDK
            ...
            for dev in self.devices:
                command += " dev %s" % dev
            command += " }"
            command += " plugins { plugin dpdk_plugin.so { enable } }"
...
class Target(Server):
    ...
    def zip_spdk_sources(self, spdk_dir, dest_file):
        ...
        subprocess.call(['tar', '-cf', dest_file, '-C', spdk_dir, spdk_dir, '.'])
...
class Initiator(Server):
    def __init__(self, name, username, password, mode, nic_ips, ip, num_targets=1, iscsiadm_bin="iscsiadm", workspace="/tmp/spdk"):
        super(Initiator, self).__init__(name, username, password, mode, nic_ips)
        self.ip = ip
        self.spdk_dir = workspace
        self.num_targets = num_targets
        self.iscsiadm_bin = iscsiadm_bin
    ...
    def discover_subsystems(self, address_list, subsys_no):
        ...
        n = int(self.num_targets / len(self.nic_ips))

        disk = 0
        for ip in self.nic_ips:
            for l in range(n):
                subsystems.append({"ip": ip,
                                   "port": 3260,
                                   "iqn": "iqn.2016-06.io.spdk",
                                   "disk": "disk%s" % disk,
                                   "lun": 0,
                                   "filename": "iSCSI%s" % disk})
                disk += 1
        return subsystems
...
class SPDKTarget(Target):
    def __init__(self, name, username, password, mode, nic_ips, num_cores, num_shared_buffers=4096,
                 use_null_block=False, portals=None, initiators=None, netmasks=None, num_targets=1, **kwargs):
    ...
    def spdk_tgt_configure(self):
        ...
        for target in self.targets:
            rpc.iscsi.construct_target_node(self.client,
                                            name="disk%s" % target['target_id'],
                                            alias_name="Data Disk %s" % target['target_id'],
                                            luns=target['luns'],
        ...
        # rpc.save_config(self.client, sys.stdout)
    ...
    def spdk_tgt_add_nullblock(self, req_num_disks=None):
        ...
        for target_id in range(self.num_targets):
            lun_id = 0
            self.targets.append({"target_id": target_id, "luns": [{"lun_id": lun_id, "bdev_name": "Null%s" % target_id}]})
            rpc.bdev.construct_null_bdev(self.client, 102400, 4096, "Null%s" % target_id)
    ...
    def spdk_tgt_add_nvme_conf(self, req_num_disks=None):
        ...
        n = math.ceil(self.num_targets / len(bdfs))
        if n == 0:
            n = 1
        ...
        for i, bdf in enumerate(bdfs):
            ...
            try:
                rpc.bdev.destruct_split_vbdev(self.client, "Nvme%sn1" % i)
            except Exception:
                pass

            rpc.bdev.construct_split_vbdev(self.client, "Nvme%sn1" % i, n)
            for p in range(0, n):
                self.targets.append({"target_id": target_id, "luns": [{"lun_id": lun_id, "bdev_name": "Nvme%sn1p%s" % (i, p)}]})
                print("... Nvme%sn1p%s ..." % (i, p))
...
class SPDKInitiator(Initiator):
    def __init__(self, name, username, password, mode, nic_ips, ip, num_targets=1, num_cores=None, **kwargs):
        super(SPDKInitiator, self).__init__(name, username, password, mode, nic_ips, ip, num_targets)
        if num_cores:
            self.num_cores = num_cores
        self.num_targets = num_targets

    def install_spdk(self, local_spdk_zip):
        self.put_file(local_spdk_zip, "/tmp/spdk_drop.tar")
        self.log_print("Copied sources zip from target")
        print("tar xvf /tmp/spdk_drop.tar -C %s" % self.spdk_dir)
        self.remote_call("tar xvf /tmp/spdk_drop.tar -C %s" % self.spdk_dir)

        self.log_print("Sources unpacked")
        self.remote_call("cd %s; git submodule update --init; ./configure --with-fio=/usr/src/fio --with-iscsi-initiator;"
    ...
    def gen_spdk_bdev_conf(self, remote_subsystem_list):
        ...
        row_template = """  URL iscsi://{ip}:{port}/{iqn}:{disk}/{lun} {filename}"""

        bdev_rows = [row_template.format(**x) for x in remote_subsystem_list]
        bdev_rows = "\n".join(bdev_rows)
    ...
    def gen_fio_filename_conf(self, remote_subsystem_list):
        ...
        if hasattr(self, 'num_cores') and self.num_cores < self.num_targets:
            self.log_print("Limiting FIO workload execution to %s cores" % self.num_cores)
            threads = range(0, int(self.num_cores))
        else:
            threads = range(0, int(self.num_targets))

        n = (int)(self.num_targets / len(threads))
        m = self.num_targets % len(threads)
        ...
        for t in threads:
            ...
            if t < m:
                j += 1
            header = "[filename%s]" % t
            disks = "\n".join(["filename=iSCSI%s" % x for x in range(i, i + j)])
