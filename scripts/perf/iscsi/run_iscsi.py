...
class Target(Server):
    ...
    def zip_spdk_sources(self, spdk_dir, dest_file):
        ...
        subprocess.call(['tar', '-cjf', dest_file, '-C', spdk_dir, spdk_dir, '.'])
...
class SPDKInitiator(Initiator):
    ...
    def install_spdk(self, local_spdk_zip):
        self.put_file(local_spdk_zip, "/tmp/spdk_drop.tar.bz2")
        self.log_print("Copied sources zip from target")
        print("tar xvjf /tmp/spdk_drop.tar.bz2 -C %s" % self.spdk_dir)
        self.remote_call("tar xvjf /tmp/spdk_drop.tar.bz2 -C %s" % self.spdk_dir)
...
if __name__ == "__main__":
    ...
    spdk_zip_path = "/tmp/spdk.tar.bz2"
