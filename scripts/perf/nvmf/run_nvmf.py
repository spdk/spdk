...
class Target(Server):
    def __init__(self, name, username, password, mode, nic_ips, transport="rdma",
        ...
        if pcm_settings:
            self.enable_pcm, self.pcm_interval, self.pcm_file_name, self.pcm_dir = pcm_settings
    ...
    def measure_pcm(self):
        subprocess.run("$s/pcm-memory.x $s -csv=memory_$s" % (self.pcm_dir, self.pcm_interval, self.pcm_file_name), shell=True, check=True)
        subprocess.run("$s/pcm.x $s -csv=cpu_$s" % (self.pcm_dir, self.pcm_interval, self.pcm_file_name), shell=True, check=True)
...
if __name__ == "__main__":
    ...
    for block_size, io_depth, rw in fio_workloads:
        ...
        if target_obj.enable_pcm:
            pcm_file_name = "_".join([str(block_size), str(rw), str(io_depth), "pcm"])
            pcm_file_name = ".".join([sar_file_name, "csv"])
