...
class Target(Server):
    ...
    def parse_results(self, results_dir, initiator_count=None, run_num=None):
        ...
        json_files = filter(lambda x: ".json" in x, files)
        ...
        for fio_config in fio_files:
            ...
            for i in inits_names:
                ...
                separate_stats = []
                for r in i_results:
                    test = self.read_json_stats(os.path.join(results_dir, r))
                    print(test)
                    separate_stats.append(test)

                zipped = zip(*separate_stats)
                z = [sum(c) for c in zipped]
                z = [c/len(separate_stats) for c in z]
                inits_avg_results.append(z)
                print(z)
            print("TOTAL")
            for s in inits_avg_results:
                print(s)
