...
class Initiator(Server):
    def __init__(self, name, username, password, mode, nic_ips, ip, transport="rdma", nvmecli_dir=None, fiobin_dir=None, workspace="/tmp/spdk"):
        ...
        if fiobin_dir:
            self.fiobin_dir = fiobin_dir
        else:
            self.fiobin_dir = "/usr/src/fio"
...
"""
    ...
    def run_fio(self, fio_config_file, run_num=None):
        ...
        fio_bin = os.path.join(self.fiobin_dir, "fio")
...
class SPDKInitiator(Initiator):
    ...
    def install_spdk(self, local_spdk_zip):
        ...
        self.log_print("Using fio directory %s" % self.fiobin_dir)
        self.remote_call("cd %s; git submodule update --init; ./configure --with-rdma --with-fio=%s;"
                         "make clean; make -j$(($(nproc)*2))" % (self.spdk_dir, self.fiobin_dir))
