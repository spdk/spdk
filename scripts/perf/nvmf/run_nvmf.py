...
class Target(Server):
    ...
    def measure_pcm_memory(self, pcm_file_name):
        ...
        subprocess.run("%s/pcm-memory.x %s -i=%s -csv=%s" % (self.pcm_dir, self.pcm_interval, self.pcm_count, self.pcm_file_name), shell=True, check=True)

    def measure_pcm(self, pcm_file_name):
        time.sleep(self.pcm_delay)
        subprocess.run("%s/pcm.x %s -i=%s -csv=%s" % (self.pcm_dir, self.pcm_interval, self.pcm_count, self.pcm_file_name), shell=True, check=True)
