...
class Target(Server):
    ...
    def measure_pcm_memory(self, pcm_file_name):
        ...
        pcm_memory = subprocess.Popen("%s/pcm-memory.x %s -csv=%s" % (self.pcm_dir, self.pcm_interval,
                       pcm_file_name), shell=True, check=True)
...
if __name__ == "__main__":
    ...
    for block_size, io_depth, rw in fio_workloads:
        ...
        if target_obj.enable_pcm:
            ...
            t = threading.Thread(target=target_obj.measure_pcm, args=(pcm_file_name,))
            threads.append(t)

        if target_obj.enable_pcm_memory:
            pcm_file_name = "_".join(["pcm_memory", str(block_size), str(rw), str(io_depth)])
            pcm_file_name = ".".join([pcm_file_name, "csv"])
            t = threading.Thread(target=target_obj.measure_pcm_memory, args=(pcm_file_name,))
            threads.append(t)
