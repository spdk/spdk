...
class Target(Server):
    ...
    def measure_sar(self, delay, interval, count, results_dir):
        self.log_print("Waiting %d delay before measuring SAR stats" % delay)
        time.sleep(delay)
        out = subprocess.check_output("sar -P ALL %s %s" % (interval, count), shell=True).decode(encoding="utf-8")
        with open(os.path.join(results_dir, "sar_stats.txt"), "w") as fh:
...
class SPDKTarget(Target):
    ...
    def spdk_tgt_configure(self):
        ...
        rpc.nvmf.nvmf_create_transport(self.client, trtype="RDMA")
...
if __name__ == "__main__":
    ...
    for block_size, io_depth, rw in fio_workloads:
        ...
        for i, cfg in zip(initiators, configs):
            ...
            threads.append(t)
            # Making sure that this works correctly, will uncomment soon
            # t = threading.Thread(target=target_obj.measure_sar, args=(10, 5, 3, target_results_dir))
            # threads.append(t)
        for t in threads:
            t.start()
        for t in threads:
