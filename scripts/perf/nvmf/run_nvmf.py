...
class KernelInitiator(Initiator):
    def __init__(self, name, username, password, mode, rdma_ips, ip):
...
class SPDKInitiator(Initiator):
    def __init__(self, name, username, password, mode, rdma_ips, ip, num_cores=None):
    ...
    def install_spdk(self, local_spdk_zip):
        ...
        stdin, stdout, stderr = self.ssh_connection.exec_command(
            "cd %s; git submodule update --init; ./configure --with-rdma --with-fio=/usr/src/fio; make -j$(($(nproc)*2))" % self.spdk_dir)
        stdout.channel.recv_exit_status()
...
if __name__ == "__main__":
    ...
    target_obj.zip_spdk_sources("/home/klateck/spdk/", "/tmp/spdk.zip")
    threads = []
    test = "/tmp/spdk.zip"
    for i in initiators:
        if i.mode == "spdk":
    ...
    for block_size, io_depth, rw in fio_workloads:
        ...
        for i in initiators:
            ...
            cfg = i.gen_fio_config(rw, fio_rw_mix_read, block_size, io_depth, fio_ramp_time, fio_run_time)
        for i in initiators:
