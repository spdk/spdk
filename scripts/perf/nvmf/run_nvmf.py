...
if __name__ == "__main__":
    ...
    transport = data["general"]["transport"]
    general_args = {
    ...
    for k, v in data.items():
        if "target" in k:
            if data[k]["mode"] == "spdk":
                target_obj = SPDKTarget(name=k, **general_args, **v)
            elif data[k]["mode"] == "kernel":
                target_obj = KernelTarget(name=k, **general_args, **v)
        elif "initiator" in k:
            if data[k]["mode"] == "spdk":
                init_obj = SPDKInitiator(name=k, **general_args, **v)
            elif data[k]["mode"] == "kernel":
                init_obj = KernelInitiator(name=k, **general_args, **v)
