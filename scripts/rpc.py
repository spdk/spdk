...
def subcommand(f):
    def inner(argv):
        p = argparse.ArgumentParser(description='SPDK RPC command line interface: {0}'.format(f.__name__))
        add_global_args(p)
        subparser = p.add_subparsers(help='RPC methods', dest='called_rpc_name')
        f(subparser)
        a = p.parse_args(argv)

        if args.dry_run:
            a.client = dry_run_client()
            print_dict = null_print
            print_json = null_print
            print_array = null_print
        else:
            a.client = rpc.client.JSONRPCClient(a.server_addr,
                                                a.port,
                                                a.timeout,
                                                log_level=getattr(logging, a.verbose.upper()))

        a.func(a)

    subcommands[f.__name__] = inner
    return f
...
if __name__ == "__main__":
    @subcommand
    def framework_start_init(s):
        def do_framework_start_init(args):
            rpc.framework_start_init(args.client)
        p = s.add_parser('framework_start_init', aliases=['start_subsystem_init'],
                         help='Start initialization of subsystems')
        p.set_defaults(func=do_framework_start_init)

    @subcommand
    def framework_wait_init(s):
        def do_framework_wait_init(args):
            rpc.framework_wait_init(args.client)
        p = s.add_parser('framework_wait_init', aliases=['wait_subsystem_init'],
                         help='Block until subsystems have been initialized')
        p.set_defaults(func=do_framework_wait_init)

    @subcommand
    def rpc_get_methods(s):
        def do_rpc_get_methods(args):
            print_dict(rpc.rpc_get_methods(args.client,
                                           current=args.current,
                                           include_aliases=args.include_aliases))
        p = s.add_parser('rpc_get_methods', aliases=['get_rpc_methods'],
        ...
        p.set_defaults(func=do_rpc_get_methods)

    @subcommand
    def spdk_get_version(s):
        def do_spdk_get_version(args):
            print_json(rpc.spdk_get_version(args.client))
        p = s.add_parser('spdk_get_version', aliases=['get_spdk_version'],
                         help='Get SPDK version')
        p.set_defaults(func=do_spdk_get_version)
    ...
    def save_config(s):
        def do_save_config(args):
            rpc.save_config(args.client,
                            sys.stdout,
                            indent=args.indent)
        p = s.add_parser('save_config', help="""Write current (live) configuration of SPDK subsystems and targets to stdout.
            """)
        p.add_argument('-i', '--indent', help="""Indent level. Value less than 0 mean compact mode. Default indent level is 2.
        """, type=int, default=2)
        p.set_defaults(func=do_save_config)
    ...
    def load_config(s):
        def do_load_config(args):
            rpc.load_config(args.client, sys.stdin,
                            include_aliases=args.include_aliases)
        p = s.add_parser('load_config',
                         help="""Configure SPDK subsystems and targets using JSON RPC read from stdin.""")
        p.add_argument('-i', '--include-aliases', help='include RPC aliases', action='store_true')
        p.set_defaults(func=do_load_config)

    @subcommand
    def save_subsystem_config(s):
        def do_save_subsystem_config(args):
            rpc.save_subsystem_config(args.client,
                                      sys.stdout,
                                      indent=args.indent,
                                      name=args.name)
        p = s.add_parser('save_subsystem_config', help="""Write current (live) configuration of SPDK subsystem to stdout.""")
        p.add_argument('-i', '--indent', help="""Indent level. Value less than 0 mean compact mode. Default indent level is 2.
        """, type=int, default=2)
        p.add_argument('-n', '--name', help='Name of subsystem', required=True)
        p.set_defaults(func=do_save_subsystem_config)

    @subcommand
    def load_subsystem_config(s):
        def do_load_subsystem_config(args):
            rpc.load_subsystem_config(args.client,
                                      sys.stdin)
        p = s.add_parser('load_subsystem_config',
                         help="""Configure SPDK subsystem using JSON RPC read from stdin.""")
        p.set_defaults(func=do_load_subsystem_config)

    # app
    @subcommand
    def spdk_kill_instance(s):
        def do_spdk_kill_instance(args):
            rpc.app.spdk_kill_instance(args.client,
                                       sig_name=args.sig_name)
        p = s.add_parser('spdk_kill_instance', aliases=['kill_instance'],
                         help='Send signal to instance')
        p.add_argument('sig_name', help='signal will be sent to server.')
        p.set_defaults(func=do_spdk_kill_instance)

    @subcommand
    def framework_monitor_context_switch(s):
        def do_framework_monitor_context_switch(args):
            enabled = None
            if args.enable:
                enabled = True
            if args.disable:
            ...
            print_dict(rpc.app.framework_monitor_context_switch(args.client,
                                                                enabled=enabled))
        p = s.add_parser('framework_monitor_context_switch', aliases=['context_switch_monitor'],
        ...
        p.set_defaults(func=do_framework_monitor_context_switch)

    # bdev
    @subcommand
    def bdev_set_options(s):
        def do_bdev_set_options(args):
            rpc.bdev.bdev_set_options(args.client,
                                      bdev_io_pool_size=args.bdev_io_pool_size,
                                      bdev_io_cache_size=args.bdev_io_cache_size)
        p = s.add_parser('bdev_set_options', aliases=['set_bdev_options'],
        ...
        p.set_defaults(func=do_bdev_set_options)

    @subcommand
    def bdev_compress_create(s):
        def do_bdev_compress_create(args):
            print_json(rpc.bdev.bdev_compress_create(args.client,
                                                     base_bdev_name=args.base_bdev_name,
                                                     pm_path=args.pm_path))
        p = s.add_parser('bdev_compress_create', aliases=['construct_compress_bdev'],
        ...
        p.set_defaults(func=do_bdev_compress_create)

    @subcommand
    def bdev_compress_delete(s):
        def do_bdev_compress_delete(args):
            rpc.bdev.bdev_compress_delete(args.client,
                                          name=args.name)
        p = s.add_parser('bdev_compress_delete', aliases=['delete_compress_bdev'],
                         help='Delete a compress disk')
        p.add_argument('name', help='compress bdev name')
        p.set_defaults(func=do_bdev_compress_delete)

    @subcommand
    def set_compress_pmd(s):
        def do_set_compress_pmd(args):
            rpc.bdev.set_compress_pmd(args.client,
                                      pmd=args.pmd)
        p = s.add_parser('set_compress_pmd', help='Set pmd option for a compress disk')
        p.add_argument('-p', '--pmd', type=int, help='0 = auto-select, 1= QAT only, 2 = ISAL only')
        p.set_defaults(func=do_set_compress_pmd)

    @subcommand
    def bdev_compress_get_orphans(s):
        def do_bdev_compress_get_orphans(args):
        ...
        p.set_defaults(func=do_bdev_compress_get_orphans)

    @subcommand
    def bdev_crypto_create(s):
        def do_bdev_crypto_create(args):
        ...
        p = s.add_parser('bdev_crypto_create', aliases=['construct_crypto_bdev'],
        ...
        p.set_defaults(func=do_bdev_crypto_create)

    @subcommand
    def bdev_crypto_delete(s):
        def do_bdev_crypto_delete(args):
            rpc.bdev.bdev_crypto_delete(args.client,
                                        name=args.name)
        p = s.add_parser('bdev_crypto_delete', aliases=['delete_crypto_bdev'],
                         help='Delete a crypto disk')
        p.add_argument('name', help='crypto bdev name')
        p.set_defaults(func=do_bdev_crypto_delete)

    @subcommand
    def bdev_ocf_create(s):
        def do_bdev_ocf_create(args):
            print_json(rpc.bdev.bdev_ocf_create(args.client,
                                                name=args.name,
                                                mode=args.mode,
                                                cache_bdev_name=args.cache_bdev_name,
                                                core_bdev_name=args.core_bdev_name))
        p = s.add_parser('bdev_ocf_create', aliases=['construct_ocf_bdev'],
        ...
        p.set_defaults(func=do_bdev_ocf_create)

    @subcommand
    def bdev_ocf_delete(s):
        def do_bdev_ocf_delete(args):
            rpc.bdev.bdev_ocf_delete(args.client,
                                     name=args.name)
        p = s.add_parser('bdev_ocf_delete', aliases=['delete_ocf_bdev'],
                         help='Delete an OCF block device')
        p.add_argument('name', help='Name of OCF bdev')
        p.set_defaults(func=do_bdev_ocf_delete)

    @subcommand
    def bdev_ocf_get_stats(s):
        def do_bdev_ocf_get_stats(args):
            print_dict(rpc.bdev.bdev_ocf_get_stats(args.client,
                                                   name=args.name))
        p = s.add_parser('bdev_ocf_get_stats', aliases=['get_ocf_stats'],
                         help='Get statistics of chosen OCF block device')
        p.add_argument('name', help='Name of OCF bdev')
        p.set_defaults(func=do_bdev_ocf_get_stats)

    @subcommand
    def bdev_ocf_get_bdevs(s):
        def do_bdev_ocf_get_bdevs(args):
            print_dict(rpc.bdev.bdev_ocf_get_bdevs(args.client,
                                                   name=args.name))
        p = s.add_parser('bdev_ocf_get_bdevs', aliases=['get_ocf_bdevs'],
        ...
        p.set_defaults(func=do_bdev_ocf_get_bdevs)

    @subcommand
    def bdev_malloc_create(s):
        def do_bdev_malloc_create(args):
        ...
        p = s.add_parser('bdev_malloc_create', aliases=['construct_malloc_bdev'],
        ...
        p.set_defaults(func=do_bdev_malloc_create)

    @subcommand
    def bdev_malloc_delete(s):
        def do_bdev_malloc_delete(args):
            rpc.bdev.bdev_malloc_delete(args.client,
                                        name=args.name)
        p = s.add_parser('bdev_malloc_delete', aliases=['delete_malloc_bdev'],
                         help='Delete a malloc disk')
        p.add_argument('name', help='malloc bdev name')
        p.set_defaults(func=do_bdev_malloc_delete)

    @subcommand
    def bdev_null_create(s):
        def do_bdev_null_create(args):
            num_blocks = (args.total_size * 1024 * 1024) // args.block_size
        ...
        p = s.add_parser('bdev_null_create', aliases=['construct_null_bdev'],
        ...
        p.set_defaults(func=do_bdev_null_create)

    @subcommand
    def bdev_null_delete(s):
        def do_bdev_null_delete(args):
            rpc.bdev.bdev_null_delete(args.client,
                                      name=args.name)
        p = s.add_parser('bdev_null_delete', aliases=['delete_null_bdev'],
                         help='Delete a null bdev')
        p.add_argument('name', help='null bdev name')
        p.set_defaults(func=do_bdev_null_delete)

    @subcommand
    def bdev_aio_create(s):
        def do_bdev_aio_create(args):
            print_json(rpc.bdev.bdev_aio_create(args.client,
                                                filename=args.filename,
                                                name=args.name,
                                                block_size=args.block_size))
        p = s.add_parser('bdev_aio_create', aliases=['construct_aio_bdev'],
        ...
        p.set_defaults(func=do_bdev_aio_create)

    @subcommand
    def bdev_aio_delete(s):
        def do_bdev_aio_delete(args):
            rpc.bdev.bdev_aio_delete(args.client,
                                     name=args.name)
        p = s.add_parser('bdev_aio_delete', aliases=['delete_aio_bdev'],
                         help='Delete an aio disk')
        p.add_argument('name', help='aio bdev name')
        p.set_defaults(func=do_bdev_aio_delete)

    @subcommand
    def bdev_uring_create(s):
        def do_bdev_uring_create(args):
            print_json(rpc.bdev.bdev_uring_create(args.client,
                                                  filename=args.filename,
                                                  name=args.name,
                                                  block_size=args.block_size))

        p = s.add_parser('bdev_uring_create', help='Create a bdev with io_uring backend')
        p.add_argument('filename', help='Path to device or file (ex: /dev/nvme0n1)')
        p.add_argument('name', help='bdev name')
        p.add_argument('block_size', help='Block size for this bdev', type=int, nargs='?', default=0)
        p.set_defaults(func=do_bdev_uring_create)

    @subcommand
    def bdev_uring_delete(s):
        def do_bdev_uring_delete(args):
            rpc.bdev.bdev_uring_delete(args.client,
                                       name=args.name)
        p = s.add_parser('bdev_uring_delete', help='Delete a uring bdev')
        p.add_argument('name', help='uring bdev name')
        p.set_defaults(func=do_bdev_uring_delete)

    @subcommand
    def bdev_nvme_set_options(s):
        def do_bdev_nvme_set_options(args):
        ...
        p = s.add_parser('bdev_nvme_set_options', aliases=['set_bdev_nvme_options'],
        ...
        p.add_argument('--low-priority-weight',
                       help='the maximum number of commands that the controller may launch at one time from a low priority queue',
                       type=int)
        p.add_argument('--medium-priority-weight',
                       help='the maximum number of commands that the controller may launch at one time from a medium priority queue',
                       type=int)
        p.add_argument('--high-priority-weight',
                       help='the maximum number of commands that the controller may launch at one time from a high priority queue',
                       type=int)
        ...
        p.set_defaults(func=do_bdev_nvme_set_options)

    @subcommand
    def bdev_nvme_set_hotplug(s):
        def do_bdev_nvme_set_hotplug(args):
            rpc.bdev.bdev_nvme_set_hotplug(args.client, enable=args.enable, period_us=args.period_us)
        p = s.add_parser('bdev_nvme_set_hotplug', aliases=['set_bdev_nvme_hotplug'],
        ...
        p.set_defaults(func=do_bdev_nvme_set_hotplug)

    @subcommand
    def bdev_nvme_attach_controller(s):
        def do_bdev_nvme_attach_controller(args):
        ...
        p = s.add_parser('bdev_nvme_attach_controller', aliases=['construct_nvme_bdev'],
        ...
        p.set_defaults(func=do_bdev_nvme_attach_controller)

    @subcommand
    def bdev_nvme_get_controllers(s):
        def do_bdev_nvme_get_controllers(args):
            print_dict(rpc.nvme.bdev_nvme_get_controllers(args.client,
                                                          name=args.name))
        p = s.add_parser('bdev_nvme_get_controllers', aliases=['get_nvme_controllers'],
        ...
        p.set_defaults(func=do_bdev_nvme_get_controllers)

    @subcommand
    def bdev_nvme_detach_controller(s):
        def do_bdev_nvme_detach_controller(args):
            rpc.bdev.bdev_nvme_detach_controller(args.client,
                                                 name=args.name)
        p = s.add_parser('bdev_nvme_detach_controller', aliases=['delete_nvme_controller'],
                         help='Detach an NVMe controller and delete any associated bdevs')
        p.add_argument('name', help="Name of the controller")
        p.set_defaults(func=do_bdev_nvme_detach_controller)

    @subcommand
    def bdev_nvme_cuse_register(s):
        def do_bdev_nvme_cuse_register(args):
            rpc.bdev.bdev_nvme_cuse_register(args.client,
                                             name=args.name,
                                             dev_path=args.dev_path)
        p = s.add_parser('bdev_nvme_cuse_register',
        ...
        p.set_defaults(func=do_bdev_nvme_cuse_register)

    @subcommand
    def bdev_nvme_cuse_unregister(s):
        def do_bdev_nvme_cuse_unregister(args):
            rpc.bdev.bdev_nvme_cuse_unregister(args.client,
                                               name=args.name)
        p = s.add_parser('bdev_nvme_cuse_unregister',
                         help='Unregister CUSE devices on NVMe controller')
        p.add_argument('-n', '--name',
                       help='Name of the NVMe controller. Example: Nvme0', required=True)
        p.set_defaults(func=do_bdev_nvme_cuse_unregister)

    @subcommand
    def bdev_zone_block_create(s):
        def do_bdev_zone_block_create(args):
        ...
        p = s.add_parser('bdev_zone_block_create',
        ...
        p.set_defaults(func=do_bdev_zone_block_create)

    @subcommand
    def bdev_zone_block_delete(s):
        def do_bdev_zone_block_delete(args):
            rpc.bdev.bdev_zone_block_delete(args.client,
                                            name=args.name)
        p = s.add_parser('bdev_zone_block_delete', help='Delete a virtual zone namespace device')
        p.add_argument('name', help='Virtual zone bdev name')
        p.set_defaults(func=do_bdev_zone_block_delete)

    @subcommand
    def bdev_rbd_create(s):
        def do_bdev_rbd_create(args):
            config = None
            if args.config:
                config = {}
            ...
            print_json(rpc.bdev.bdev_rbd_create(args.client,
                                                ...
                                                block_size=args.block_size))
        p = s.add_parser('bdev_rbd_create', aliases=['construct_rbd_bdev'],
        ...
        p.set_defaults(func=do_bdev_rbd_create)

    @subcommand
    def bdev_rbd_delete(s):
        def do_bdev_rbd_delete(args):
            rpc.bdev.bdev_rbd_delete(args.client,
                                     name=args.name)
        p = s.add_parser('bdev_rbd_delete', aliases=['delete_rbd_bdev'],
                         help='Delete a rbd bdev')
        p.add_argument('name', help='rbd bdev name')
        p.set_defaults(func=do_bdev_rbd_delete)

    @subcommand
    def bdev_delay_create(s):
        def do_bdev_delay_create(args):
        ...
        p = s.add_parser('bdev_delay_create', help='Add a delay bdev on existing bdev')
        ...
        p.set_defaults(func=do_bdev_delay_create)

    @subcommand
    def bdev_delay_delete(s):
        def do_bdev_delay_delete(args):
            rpc.bdev.bdev_delay_delete(args.client,
                                       name=args.name)
        p = s.add_parser('bdev_delay_delete', help='Delete a delay bdev')
        p.add_argument('name', help='delay bdev name')
        p.set_defaults(func=do_bdev_delay_delete)

    @subcommand
    def bdev_delay_update_latency(s):
        def do_bdev_delay_update_latency(args):
        ...
        p = s.add_parser('bdev_delay_update_latency',
        ...
        p.set_defaults(func=do_bdev_delay_update_latency)

    @subcommand
    def bdev_error_create(s):
        def do_bdev_error_create(args):
            print_json(rpc.bdev.bdev_error_create(args.client,
                                                  base_name=args.base_name))
        p = s.add_parser('bdev_error_create', aliases=['construct_error_bdev'],
                         help='Add bdev with error injection backend')
        p.add_argument('base_name', help='base bdev name')
        p.set_defaults(func=do_bdev_error_create)

    @subcommand
    def bdev_error_delete(s):
        def do_bdev_error_delete(args):
            rpc.bdev.bdev_error_delete(args.client,
                                       name=args.name)
        p = s.add_parser('bdev_error_delete', aliases=['delete_error_bdev'],
                         help='Delete an error bdev')
        p.add_argument('name', help='error bdev name')
        p.set_defaults(func=do_bdev_error_delete)

    @subcommand
    def bdev_iscsi_create(s):
        def do_bdev_iscsi_create(args):
            print_json(rpc.bdev.bdev_iscsi_create(args.client,
                                                  name=args.name,
                                                  url=args.url,
                                                  initiator_iqn=args.initiator_iqn))
        p = s.add_parser('bdev_iscsi_create', aliases=['construct_iscsi_bdev'],
        ...
        p.set_defaults(func=do_bdev_iscsi_create)

    @subcommand
    def bdev_iscsi_delete(s):
        def do_bdev_iscsi_delete(args):
            rpc.bdev.bdev_iscsi_delete(args.client,
                                       name=args.name)
        p = s.add_parser('bdev_iscsi_delete', aliases=['delete_iscsi_bdev'],
                         help='Delete an iSCSI bdev')
        p.add_argument('name', help='iSCSI bdev name')
        p.set_defaults(func=do_bdev_iscsi_delete)

    @subcommand
    def bdev_pmem_create(s):
        def do_bdev_pmem_create(args):
            print_json(rpc.bdev.bdev_pmem_create(args.client,
                                                 pmem_file=args.pmem_file,
                                                 name=args.name))
        p = s.add_parser('bdev_pmem_create', aliases=['construct_pmem_bdev'],
                         help='Add a bdev with pmem backend')
        p.add_argument('pmem_file', help='Path to pmemblk pool file')
        p.add_argument('-n', '--name', help='Block device name', required=True)
        p.set_defaults(func=do_bdev_pmem_create)

    @subcommand
    def bdev_pmem_delete(s):
        def do_bdev_pmem_delete(args):
            rpc.bdev.bdev_pmem_delete(args.client,
                                      name=args.name)
        p = s.add_parser('bdev_pmem_delete', aliases=['delete_pmem_bdev'],
                         help='Delete a pmem bdev')
        p.add_argument('name', help='pmem bdev name')
        p.set_defaults(func=do_bdev_pmem_delete)

    @subcommand
    def bdev_passthru_create(s):
        def do_bdev_passthru_create(args):
            print_json(rpc.bdev.bdev_passthru_create(args.client,
                                                     base_bdev_name=args.base_bdev_name,
                                                     name=args.name))
        p = s.add_parser('bdev_passthru_create', aliases=['construct_passthru_bdev'],
                         help='Add a pass through bdev on existing bdev')
        p.add_argument('-b', '--base-bdev-name', help="Name of the existing bdev", required=True)
        p.add_argument('-p', '--name', help="Name of the pass through bdev", required=True)
        p.set_defaults(func=do_bdev_passthru_create)

    @subcommand
    def bdev_passthru_delete(s):
        def do_bdev_passthru_delete(args):
            rpc.bdev.bdev_passthru_delete(args.client,
                                          name=args.name)
        p = s.add_parser('bdev_passthru_delete', aliases=['delete_passthru_bdev'],
                         help='Delete a pass through bdev')
        p.add_argument('name', help='pass through bdev name')
        p.set_defaults(func=do_bdev_passthru_delete)

    @subcommand
    def bdev_get_bdevs(s):
        def do_bdev_get_bdevs(args):
            print_dict(rpc.bdev.bdev_get_bdevs(args.client,
                                               name=args.name))
        p = s.add_parser('bdev_get_bdevs', aliases=['get_bdevs'],
                         help='Display current blockdev list or required blockdev')
        p.add_argument('-b', '--name', help="Name of the Blockdev. Example: Nvme0n1", required=False)
        p.set_defaults(func=do_bdev_get_bdevs)

    @subcommand
    def bdev_get_iostat(s):
        def do_bdev_get_iostat(args):
            print_dict(rpc.bdev.bdev_get_iostat(args.client,
                                                name=args.name))
        p = s.add_parser('bdev_get_iostat', aliases=['get_bdevs_iostat'],
                         help='Display current I/O statistics of all the blockdevs or required blockdev.')
        p.add_argument('-b', '--name', help="Name of the Blockdev. Example: Nvme0n1", required=False)
        p.set_defaults(func=do_bdev_get_iostat)

    @subcommand
    def bdev_enable_histogram(s):
        def do_bdev_enable_histogram(args):
            rpc.bdev.bdev_enable_histogram(args.client, name=args.name, enable=args.enable)
        p = s.add_parser('bdev_enable_histogram', aliases=['enable_bdev_histogram'],
        ...
        p.add_argument('name', help='bdev name')
        p.set_defaults(func=do_bdev_enable_histogram)

    @subcommand
    def bdev_get_histogram(s):
        def do_bdev_get_histogram(args):
            print_dict(rpc.bdev.bdev_get_histogram(args.client, name=args.name))
        p = s.add_parser('bdev_get_histogram', aliases=['get_bdev_histogram'],
                         help='Get histogram for specified bdev')
        p.add_argument('name', help='bdev name')
        p.set_defaults(func=do_bdev_get_histogram)

    @subcommand
    def bdev_set_qd_sampling_period(s):
        def do_bdev_set_qd_sampling_period(args):
            rpc.bdev.bdev_set_qd_sampling_period(args.client,
                                                 name=args.name,
                                                 period=args.period)
        p = s.add_parser('bdev_set_qd_sampling_period', aliases=['set_bdev_qd_sampling_period'],
        ...
        p.add_argument('period', help='Period with which to poll the block device queue depth in microseconds.'
                       ' If set to 0, polling will be disabled.', type=int)
        p.set_defaults(func=do_bdev_set_qd_sampling_period)

    @subcommand
    def bdev_set_qos_limit(s):
        def do_bdev_set_qos_limit(args):
        ...
        p = s.add_parser('bdev_set_qos_limit', aliases=['set_bdev_qos_limit'],
        ...
        p.add_argument('--w_mbytes_per_sec',
                       ...
                       type=int, required=False)
        p.set_defaults(func=do_bdev_set_qos_limit)

    @subcommand
    def bdev_error_inject_error(s):
        def do_bdev_error_inject_error(args):
            rpc.bdev.bdev_error_inject_error(args.client,
                                             name=args.name,
                                             io_type=args.io_type,
                                             error_type=args.error_type,
                                             num=args.num)
        p = s.add_parser('bdev_error_inject_error', aliases=['bdev_inject_error'],
        ...
        p.set_defaults(func=do_bdev_error_inject_error)

    @subcommand
    def bdev_nvme_apply_firmware(s):
        def do_bdev_nvme_apply_firmware(args):
            print_dict(rpc.bdev.bdev_nvme_apply_firmware(args.client,
                                                         bdev_name=args.bdev_name,
                                                         filename=args.filename))
        p = s.add_parser('bdev_nvme_apply_firmware', aliases=['apply_firmware'],
        ...
        p.set_defaults(func=do_bdev_nvme_apply_firmware)

    # iSCSI
    @subcommand
    def iscsi_set_options(s):
        def do_iscsi_set_options(args):
        ...
        p = s.add_parser('iscsi_set_options', aliases=['set_iscsi_options'],
        ...
        *** Authentication group must be precreated ***""", type=int)
        ...
        p.set_defaults(func=do_iscsi_set_options)

    @subcommand
    def iscsi_set_discovery_auth(s):
        def do_iscsi_set_discovery_auth(args):
            rpc.iscsi.iscsi_set_discovery_auth(
                ...
                mutual_chap=args.mutual_chap,
                chap_group=args.chap_group)
        p = s.add_parser('iscsi_set_discovery_auth', aliases=['set_iscsi_discovery_auth'],
        ...
        *** Mutually exclusive with --disable-chap""", action='store_true')
        ...
        p.add_argument('-g', '--chap-group', help="""Authentication group ID for discovery session.
        *** Authentication group must be precreated ***""", type=int)
        p.set_defaults(func=do_iscsi_set_discovery_auth)

    @subcommand
    def iscsi_create_auth_group(s):
        def do_iscsi_create_auth_group(args):
            secrets = None
            if args.secrets:
        ...
        p = s.add_parser('iscsi_create_auth_group', aliases=['add_iscsi_auth_group'],
    ...
    Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
        p.set_defaults(func=do_iscsi_create_auth_group)

    @subcommand
    def iscsi_delete_auth_group(s):
        def do_iscsi_delete_auth_group(args):
            rpc.iscsi.iscsi_delete_auth_group(args.client, tag=args.tag)
        p = s.add_parser('iscsi_delete_auth_group', aliases=['delete_iscsi_auth_group'],
                         help='Delete an authentication group.')
        p.add_argument('tag', help='Authentication group tag', type=int)
        p.set_defaults(func=do_iscsi_delete_auth_group)

    @subcommand
    def iscsi_auth_group_add_secret(s):
        def do_iscsi_auth_group_add_secret(args):
        ...
        p = s.add_parser('iscsi_auth_group_add_secret', aliases=['add_secret_to_iscsi_auth_group'],
        ...
        p.set_defaults(func=do_iscsi_auth_group_add_secret)

    @subcommand
    def iscsi_auth_group_remove_secret(s):
        def do_iscsi_auth_group_remove_secret(args):
            rpc.iscsi.iscsi_auth_group_remove_secret(args.client, tag=args.tag, user=args.user)
        p = s.add_parser('iscsi_auth_group_remove_secret', aliases=['delete_secret_from_iscsi_auth_group'],
                         help='Remove a secret from an authentication group.')
        p.add_argument('tag', help='Authentication group tag', type=int)
        p.add_argument('-u', '--user', help='User name for one-way CHAP authentication', required=True)
        p.set_defaults(func=do_iscsi_auth_group_remove_secret)

    @subcommand
    def iscsi_get_auth_groups(s):
        def do_iscsi_get_auth_groups(args):
            print_dict(rpc.iscsi.iscsi_get_auth_groups(args.client))
        p = s.add_parser('iscsi_get_auth_groups', aliases=['get_iscsi_auth_groups'],
                         help='Display current authentication group configuration')
        p.set_defaults(func=do_iscsi_get_auth_groups)

    @subcommand
    def iscsi_get_portal_groups(s):
        def do_iscsi_get_portal_groups(args):
            print_dict(rpc.iscsi.iscsi_get_portal_groups(args.client))
        p = s.add_parser('iscsi_get_portal_groups', aliases=['get_portal_groups'],
                         help='Display current portal group configuration')
        p.set_defaults(func=do_iscsi_get_portal_groups)

    @subcommand
    def iscsi_get_initiator_groups(s):
        def do_iscsi_get_initiator_groups(args):
            print_dict(rpc.iscsi.iscsi_get_initiator_groups(args.client))

        p = s.add_parser('iscsi_get_initiator_groups', aliases=['get_initiator_groups'],
                         help='Display current initiator group configuration')
        p.set_defaults(func=do_iscsi_get_initiator_groups)

    @subcommand
    def iscsi_get_target_nodes(s):
        def do_iscsi_get_target_nodes(args):
            print_dict(rpc.iscsi.iscsi_get_target_nodes(args.client))
        p = s.add_parser('iscsi_get_target_nodes', aliases=['get_target_nodes'],
                         help='Display target nodes')
        p.set_defaults(func=do_iscsi_get_target_nodes)

    @subcommand
    def iscsi_create_target_node(s):
        def do_iscsi_create_target_node(args):
            luns = []
        ...
        p = s.add_parser('iscsi_create_target_node', aliases=['construct_target_node'],
        ...
        *** Mutually exclusive with --disable-chap ***""", action='store_true')
        p.add_argument('-m', '--mutual-chap',
                       help='CHAP authentication should be mutual/bidirectional.', action='store_true')
        ...
        p.set_defaults(func=do_iscsi_create_target_node)

    @subcommand
    def iscsi_target_node_add_lun(s):
        def do_iscsi_target_node_add_lun(args):
            rpc.iscsi.iscsi_target_node_add_lun(
                args.client,
                name=args.name,
                bdev_name=args.bdev_name,
                lun_id=args.lun_id)
        p = s.add_parser('iscsi_target_node_add_lun', aliases=['target_node_add_lun'],
        ...
        *** If LUN ID is omitted or -1, the lowest free one is assigned ***""", type=int, required=False)
        p.set_defaults(func=do_iscsi_target_node_add_lun)

    @subcommand
    def iscsi_target_node_set_auth(s):
        def do_iscsi_target_node_set_auth(args):
            rpc.iscsi.iscsi_target_node_set_auth(
                ...
                require_chap=args.require_chap,
                mutual_chap=args.mutual_chap)
        p = s.add_parser('iscsi_target_node_set_auth', aliases=['set_iscsi_target_node_auth'],
        ...
        *** Mutually exclusive with --disable-chap ***""", action='store_true')
        p.add_argument('-m', '--mutual-chap', help='CHAP authentication should be mutual/bidirectional.',
                       action='store_true')
        p.set_defaults(func=do_iscsi_target_node_set_auth)

    @subcommand
    def iscsi_target_node_add_pg_ig_maps(s):
        def do_iscsi_target_node_add_pg_ig_maps(args):
            pg_ig_maps = []
            for u in args.pg_ig_mappings.strip().split(" "):
                pg, ig = u.split(":")
                pg_ig_maps.append({"pg_tag": int(pg), "ig_tag": int(ig)})
            rpc.iscsi.iscsi_target_node_add_pg_ig_maps(
                ...
                pg_ig_maps=pg_ig_maps,
                name=args.name)
        p = s.add_parser('iscsi_target_node_add_pg_ig_maps', aliases=['add_pg_ig_maps'],
        ...
        Example: '1:1 2:2 2:1'
        *** The Portal/Initiator Groups must be precreated ***""")
        p.set_defaults(func=do_iscsi_target_node_add_pg_ig_maps)

    @subcommand
    def iscsi_target_node_remove_pg_ig_maps(s):
        def do_iscsi_target_node_remove_pg_ig_maps(args):
            pg_ig_maps = []
            for u in args.pg_ig_mappings.strip().split(" "):
                pg, ig = u.split(":")
                pg_ig_maps.append({"pg_tag": int(pg), "ig_tag": int(ig)})
        ...
        p = s.add_parser('iscsi_target_node_remove_pg_ig_maps', aliases=['delete_pg_ig_maps'],
        ...
        Example: '1:1 2:2 2:1'
        *** The Portal/Initiator Groups must be precreated ***""")
        p.set_defaults(func=do_iscsi_target_node_remove_pg_ig_maps)

    @subcommand
    def iscsi_create_portal_group(s):
        def do_iscsi_create_portal_group(args):
            portals = []
            ...
            rpc.iscsi.iscsi_create_portal_group(
                ...
                portals=portals,
                tag=args.tag)
        p = s.add_parser('iscsi_create_portal_group', aliases=['add_portal_group'],
        ...
        p.set_defaults(func=do_iscsi_create_portal_group)

    @subcommand
    def iscsi_create_initiator_group(s):
        def do_iscsi_create_initiator_group(args):
            initiators = []
            netmasks = []
        ...
        p = s.add_parser('iscsi_create_initiator_group', aliases=['add_initiator_group'],
        ...
        p.set_defaults(func=do_iscsi_create_initiator_group)

    @subcommand
    def iscsi_initiator_group_add_initiators(s):
        def do_iscsi_initiator_group_add_initiators(args):
            initiators = None
            netmasks = None
            ...
            rpc.iscsi.iscsi_initiator_group_add_initiators(
                ...
                initiators=initiators,
                netmasks=netmasks)
        p = s.add_parser('iscsi_initiator_group_add_initiators', aliases=['add_initiators_to_initiator_group'],
        ...
        p.set_defaults(func=do_iscsi_initiator_group_add_initiators)

    @subcommand
    def iscsi_initiator_group_remove_initiators(s):
        def do_iscsi_initiator_group_remove_initiators(args):
            initiators = None
            netmasks = None
            if args.initiator_list:
                initiators = []
            ...
            rpc.iscsi.iscsi_initiator_group_remove_initiators(
                ...
                initiators=initiators,
                netmasks=netmasks)
        p = s.add_parser('iscsi_initiator_group_remove_initiators', aliases=['delete_initiators_from_initiator_group'],
        ...
        p.add_argument('-m', dest='netmask_list', help="""Whitespace-separated list of initiator netmasks enclosed in quotes.
        This parameter can be omitted.  Example: '255.255.0.0 255.248.0.0' etc""", required=False)
        p.set_defaults(func=do_iscsi_initiator_group_remove_initiators)

    @subcommand
    def iscsi_delete_target_node(s):
        def do_iscsi_delete_target_node(args):
            rpc.iscsi.iscsi_delete_target_node(
                args.client, target_node_name=args.target_node_name)
        p = s.add_parser('iscsi_delete_target_node', aliases=['delete_target_node'],
        ...
        p.set_defaults(func=do_iscsi_delete_target_node)

    @subcommand
    def iscsi_delete_portal_group(s):
        def do_iscsi_delete_portal_group(args):
            rpc.iscsi.iscsi_delete_portal_group(args.client, tag=args.tag)
        p = s.add_parser('iscsi_delete_portal_group', aliases=['delete_portal_group'],
                         help='Delete a portal group')
        p.add_argument(
            'tag', help='Portal group tag (unique, integer > 0)', type=int)
        p.set_defaults(func=do_iscsi_delete_portal_group)

    @subcommand
    def iscsi_delete_initiator_group(s):
        def do_iscsi_delete_initiator_group(args):
            rpc.iscsi.iscsi_delete_initiator_group(args.client, tag=args.tag)

        p = s.add_parser('iscsi_delete_initiator_group', aliases=['delete_initiator_group'],
                         help='Delete an initiator group')
        p.add_argument(
            'tag', help='Initiator group tag (unique, integer > 0)', type=int)
        p.set_defaults(func=do_iscsi_delete_initiator_group)

    @subcommand
    def iscsi_portal_group_set_auth(s):
        def do_iscsi_portal_group_set_auth(args):
        ...
        p = s.add_parser('iscsi_portal_group_set_auth',
        ...
        *** Mutually exclusive with --disable-chap ***""", action='store_true')
        p.add_argument('-m', '--mutual-chap', help='CHAP authentication should be mutual/bidirectional.',
                       action='store_true')
        p.set_defaults(func=do_iscsi_portal_group_set_auth)

    @subcommand
    def iscsi_get_connections(s):
        def do_iscsi_get_connections(args):
            print_dict(rpc.iscsi.iscsi_get_connections(args.client))
        p = s.add_parser('iscsi_get_connections', aliases=['get_iscsi_connections'],
                         help='Display iSCSI connections')
        p.set_defaults(func=do_iscsi_get_connections)

    @subcommand
    def iscsi_get_options(s):
        def do_iscsi_get_options(args):
            print_dict(rpc.iscsi.iscsi_get_options(args.client))
        p = s.add_parser('iscsi_get_options', aliases=['get_iscsi_global_params'],
                         help='Display iSCSI global parameters')
        p.set_defaults(func=do_iscsi_get_options)

    @subcommand
    def scsi_get_devices(s):
        def do_scsi_get_devices(args):
            print_dict(rpc.iscsi.scsi_get_devices(args.client))
        p = s.add_parser('scsi_get_devices', aliases=['get_scsi_devices'],
                         help='Display SCSI devices')
        p.set_defaults(func=do_scsi_get_devices)

    # trace
    @subcommand
    def trace_enable_tpoint_group(s):
        def do_trace_enable_tpoint_group(args):
            rpc.trace.trace_enable_tpoint_group(args.client, name=args.name)
        p = s.add_parser('trace_enable_tpoint_group', aliases=['enable_tpoint_group'],
        ...
        p.set_defaults(func=do_trace_enable_tpoint_group)

    @subcommand
    def trace_disable_tpoint_group(s):
        def do_trace_disable_tpoint_group(args):
            rpc.trace.trace_disable_tpoint_group(args.client, name=args.name)
        p = s.add_parser('trace_disable_tpoint_group', aliases=['disable_tpoint_group'],
                         help='disable trace on a specific tpoint group')
        p.add_argument(
            'name', help="""trace group name we want to disable in tpoint_group_mask.
            (for example "bdev" for bdev trace group, "all" for all trace groups).""")
        p.set_defaults(func=do_trace_disable_tpoint_group)

    @subcommand
    def trace_get_tpoint_group_mask(s):
        def do_trace_get_tpoint_group_mask(args):
            print_dict(rpc.trace.trace_get_tpoint_group_mask(args.client))
        p = s.add_parser('trace_get_tpoint_group_mask', aliases=['get_tpoint_group_mask'],
                         help='get trace point group mask')
        p.set_defaults(func=do_trace_get_tpoint_group_mask)

    # log
    @subcommand
    def log_set_flag(s):
        def do_log_set_flag(args):
            rpc.log.log_set_flag(args.client, flag=args.flag)
        p = s.add_parser('log_set_flag', help='set log flag', aliases=['set_log_flag'])
        p.add_argument(
            'flag', help='log flag we want to set. (for example "nvme").')
        p.set_defaults(func=do_log_set_flag)

    @subcommand
    def log_clear_flag(s):
        def do_log_clear_flag(args):
            rpc.log.log_clear_flag(args.client, flag=args.flag)
        p = s.add_parser('log_clear_flag', help='clear log flag', aliases=['clear_log_flag'])
        p.add_argument(
            'flag', help='log flag we want to clear. (for example "nvme").')
        p.set_defaults(func=do_log_clear_flag)

    @subcommand
    def log_get_flags(s):
        def do_log_get_flags(args):
            print_dict(rpc.log.log_get_flags(args.client))
        p = s.add_parser('log_get_flags', help='get log flags', aliases=['get_log_flags'])
        p.set_defaults(func=do_log_get_flags)

    @subcommand
    def log_set_level(s):
        def do_log_set_level(args):
            rpc.log.log_set_level(args.client, level=args.level)
        p = s.add_parser('log_set_level', aliases=['set_log_level'],
                         help='set log level')
        p.add_argument('level', help='log level we want to set. (for example "DEBUG").')
        p.set_defaults(func=do_log_set_level)

    @subcommand
    def log_get_level(s):
        def do_log_get_level(args):
            print_dict(rpc.log.log_get_level(args.client))
        p = s.add_parser('log_get_level', aliases=['get_log_level'],
                         help='get log level')
        p.set_defaults(func=do_log_get_level)

    @subcommand
    def log_set_print_level(s):
        def do_log_set_print_level(args):
            rpc.log.log_set_print_level(args.client, level=args.level)
        p = s.add_parser('log_set_print_level', aliases=['set_log_print_level'],
                         help='set log print level')
        p.add_argument('level', help='log print level we want to set. (for example "DEBUG").')
        p.set_defaults(func=do_log_set_print_level)

    @subcommand
    def log_get_print_level(s):
        def do_log_get_print_level(args):
            print_dict(rpc.log.log_get_print_level(args.client))
        p = s.add_parser('log_get_print_level', aliases=['get_log_print_level'],
                         help='get log print level')
        p.set_defaults(func=do_log_get_print_level)

    # lvol
    @subcommand
    def bdev_lvol_create_lvstore(s):
        def do_bdev_lvol_create_lvstore(args):
            print_json(rpc.lvol.bdev_lvol_create_lvstore(args.client,
                                                         bdev_name=args.bdev_name,
                                                         lvs_name=args.lvs_name,
                                                         cluster_sz=args.cluster_sz,
                                                         clear_method=args.clear_method))
        p = s.add_parser('bdev_lvol_create_lvstore', aliases=['construct_lvol_store'],
        ...
        p.set_defaults(func=do_bdev_lvol_create_lvstore)

    @subcommand
    def bdev_lvol_rename_lvstore(s):
        def do_bdev_lvol_rename_lvstore(args):
            rpc.lvol.bdev_lvol_rename_lvstore(args.client,
                                              old_name=args.old_name,
                                              new_name=args.new_name)
        p = s.add_parser('bdev_lvol_rename_lvstore', aliases=['rename_lvol_store'],
                         help='Change logical volume store name')
        p.add_argument('old_name', help='old name')
        p.add_argument('new_name', help='new name')
        p.set_defaults(func=do_bdev_lvol_rename_lvstore)

    @subcommand
    def bdev_lvol_create(s):
        def do_bdev_lvol_create(args):
            print_json(rpc.lvol.bdev_lvol_create(args.client,
                                                 ...
                                                 uuid=args.uuid,
                                                 lvs_name=args.lvs_name))
        p = s.add_parser('bdev_lvol_create', aliases=['construct_lvol_bdev'],
        ...
        p.set_defaults(func=do_bdev_lvol_create)

    @subcommand
    def bdev_lvol_snapshot(s):
        def do_bdev_lvol_snapshot(args):
            print_json(rpc.lvol.bdev_lvol_snapshot(args.client,
                                                   lvol_name=args.lvol_name,
                                                   snapshot_name=args.snapshot_name))
        p = s.add_parser('bdev_lvol_snapshot', aliases=['snapshot_lvol_bdev'],
        ...
        p.set_defaults(func=do_bdev_lvol_snapshot)

    @subcommand
    def bdev_lvol_clone(s):
        def do_bdev_lvol_clone(args):
            print_json(rpc.lvol.bdev_lvol_clone(args.client,
                                                snapshot_name=args.snapshot_name,
                                                clone_name=args.clone_name))
        p = s.add_parser('bdev_lvol_clone', aliases=['clone_lvol_bdev'],
                         help='Create a clone of an lvol snapshot')
        p.add_argument('snapshot_name', help='lvol snapshot name')
        p.add_argument('clone_name', help='lvol clone name')
        p.set_defaults(func=do_bdev_lvol_clone)

    @subcommand
    def bdev_lvol_rename(s):
        def do_bdev_lvol_rename(args):
            rpc.lvol.bdev_lvol_rename(args.client,
                                      old_name=args.old_name,
                                      new_name=args.new_name)
        p = s.add_parser('bdev_lvol_rename', aliases=['rename_lvol_bdev'],
                         help='Change lvol bdev name')
        p.add_argument('old_name', help='lvol bdev name')
        p.add_argument('new_name', help='new lvol name')
        p.set_defaults(func=do_bdev_lvol_rename)

    @subcommand
    def bdev_lvol_inflate(s):
        def do_bdev_lvol_inflate(args):
            rpc.lvol.bdev_lvol_inflate(args.client,
                                       name=args.name)
        p = s.add_parser('bdev_lvol_inflate', aliases=['inflate_lvol_bdev'],
                         help='Make thin provisioned lvol a thick provisioned lvol')
        p.add_argument('name', help='lvol bdev name')
        p.set_defaults(func=do_bdev_lvol_inflate)

    @subcommand
    def bdev_lvol_decouple_parent(s):
        def do_bdev_lvol_decouple_parent(args):
            rpc.lvol.bdev_lvol_decouple_parent(args.client,
                                               name=args.name)
        p = s.add_parser('bdev_lvol_decouple_parent', aliases=['decouple_parent_lvol_bdev'],
                         help='Decouple parent of lvol')
        p.add_argument('name', help='lvol bdev name')
        p.set_defaults(func=do_bdev_lvol_decouple_parent)

    @subcommand
    def bdev_lvol_resize(s):
        def do_bdev_lvol_resize(args):
            rpc.lvol.bdev_lvol_resize(args.client,
                                      name=args.name,
                                      size=args.size * 1024 * 1024)
        p = s.add_parser('bdev_lvol_resize', aliases=['resize_lvol_bdev'],
                         help='Resize existing lvol bdev')
        p.add_argument('name', help='lvol bdev name')
        p.add_argument('size', help='new size in MiB for this bdev', type=int)
        p.set_defaults(func=do_bdev_lvol_resize)

    @subcommand
    def bdev_lvol_set_read_only(s):
        def do_bdev_lvol_set_read_only(args):
            rpc.lvol.bdev_lvol_set_read_only(args.client,
                                             name=args.name)
        p = s.add_parser('bdev_lvol_set_read_only', aliases=['set_read_only_lvol_bdev'],
                         help='Mark lvol bdev as read only')
        p.add_argument('name', help='lvol bdev name')
        p.set_defaults(func=do_bdev_lvol_set_read_only)

    @subcommand
    def bdev_lvol_delete(s):
        def do_bdev_lvol_delete(args):
            rpc.lvol.bdev_lvol_delete(args.client,
                                      name=args.name)
        p = s.add_parser('bdev_lvol_delete', aliases=['destroy_lvol_bdev'],
                         help='Destroy a logical volume')
        p.add_argument('name', help='lvol bdev name')
        p.set_defaults(func=do_bdev_lvol_delete)

    @subcommand
    def bdev_lvol_delete_lvstore(s):
        def do_bdev_lvol_delete_lvstore(args):
            rpc.lvol.bdev_lvol_delete_lvstore(args.client,
                                              uuid=args.uuid,
                                              lvs_name=args.lvs_name)
        p = s.add_parser('bdev_lvol_delete_lvstore', aliases=['destroy_lvol_store'],
                         help='Destroy an logical volume store')
        p.add_argument('-u', '--uuid', help='lvol store UUID', required=False)
        p.add_argument('-l', '--lvs-name', help='lvol store name', required=False)
        p.set_defaults(func=do_bdev_lvol_delete_lvstore)

    @subcommand
    def bdev_lvol_get_lvstores(s):
        def do_bdev_lvol_get_lvstores(args):
            print_dict(rpc.lvol.bdev_lvol_get_lvstores(args.client,
                                                       uuid=args.uuid,
                                                       lvs_name=args.lvs_name))
        p = s.add_parser('bdev_lvol_get_lvstores', aliases=['get_lvol_stores'],
                         help='Display current logical volume store list')
        p.add_argument('-u', '--uuid', help='lvol store UUID', required=False)
        p.add_argument('-l', '--lvs-name', help='lvol store name', required=False)
        p.set_defaults(func=do_bdev_lvol_get_lvstores)

    @subcommand
    def bdev_raid_get_bdevs(s):
        def do_bdev_raid_get_bdevs(args):
            print_array(rpc.bdev.bdev_raid_get_bdevs(args.client,
                                                     category=args.category))
        p = s.add_parser('bdev_raid_get_bdevs', aliases=['get_raid_bdevs'],
        ...
        p.set_defaults(func=do_bdev_raid_get_bdevs)

    @subcommand
    def bdev_raid_create(s):
        def do_bdev_raid_create(args):
            base_bdevs = []
        ...
        p = s.add_parser('bdev_raid_create', aliases=['construct_raid_bdev'],
        ...
        p.set_defaults(func=do_bdev_raid_create)

    @subcommand
    def bdev_raid_delete(s):
        def do_bdev_raid_delete(args):
            rpc.bdev.bdev_raid_delete(args.client,
                                      name=args.name)
        p = s.add_parser('bdev_raid_delete', aliases=['destroy_raid_bdev'],
                         help='Delete existing raid bdev')
        p.add_argument('name', help='raid bdev name')
        p.set_defaults(func=do_bdev_raid_delete)

    # split
    @subcommand
    def bdev_split_create(s):
        def do_bdev_split_create(args):
            print_array(rpc.bdev.bdev_split_create(args.client,
                                                   base_bdev=args.base_bdev,
                                                   split_count=args.split_count,
                                                   split_size_mb=args.split_size_mb))
        p = s.add_parser('bdev_split_create', aliases=['construct_split_vbdev'],
        ...
        p.set_defaults(func=do_bdev_split_create)

    @subcommand
    def bdev_split_delete(s):
        def do_bdev_split_delete(args):
            rpc.bdev.bdev_split_delete(args.client,
                                       base_bdev=args.base_bdev)
        p = s.add_parser('bdev_split_delete', aliases=['destruct_split_vbdev'],
                         help="""Delete split config with all created splits.""")
        p.add_argument('base_bdev', help='base bdev name')
        p.set_defaults(func=do_bdev_split_delete)

    # ftl
    ftl_valid_limits = ('crit', 'high', 'low', 'start')

    @subcommand
    def bdev_ftl_create(s):
        def do_bdev_ftl_create(args):
            ...
            print_dict(rpc.bdev.bdev_ftl_create(args.client,
                                                ...
                                                **arg_limits))
        p = s.add_parser('bdev_ftl_create', aliases=['construct_ftl_bdev'],
                         help='Add FTL bdev')
        p.add_argument('-b', '--name', help="Name of the bdev", required=True)
        p.add_argument('-t', '--trtype',
        ...
        p.set_defaults(func=do_bdev_ftl_create)

    @subcommand
    def bdev_ftl_delete(s):
        def do_bdev_ftl_delete(args):
            print_dict(rpc.bdev.bdev_ftl_delete(args.client, name=args.name))
        p = s.add_parser('bdev_ftl_delete', aliases=['delete_ftl_bdev'],
                         help='Delete FTL bdev')
        p.add_argument('-b', '--name', help="Name of the bdev", required=True)
        p.set_defaults(func=do_bdev_ftl_delete)

    # vmd
    @subcommand
    def enable_vmd(s):
        def do_enable_vmd(args):
            print_dict(rpc.vmd.enable_vmd(args.client))
        p = s.add_parser('enable_vmd', help='Enable VMD enumeration')
        p.set_defaults(func=do_enable_vmd)

    # nbd
    @subcommand
    def nbd_start_disk(s):
        def do_nbd_start_disk(args):
            print(rpc.nbd.nbd_start_disk(args.client,
                                         bdev_name=args.bdev_name,
                                         nbd_device=args.nbd_device))
        p = s.add_parser('nbd_start_disk', aliases=['start_nbd_disk'],
        ...
        p.set_defaults(func=do_nbd_start_disk)

    @subcommand
    def nbd_stop_disk(s):
        def do_nbd_stop_disk(args):
            rpc.nbd.nbd_stop_disk(args.client,
                                  nbd_device=args.nbd_device)
        p = s.add_parser('nbd_stop_disk', aliases=['stop_nbd_disk'],
                         help='Stop an nbd disk')
        p.add_argument('nbd_device', help='Nbd device name to be stopped. Example: /dev/nbd0.')
        p.set_defaults(func=do_nbd_stop_disk)

    @subcommand
    def nbd_get_disks(s):
        def do_nbd_get_disks(args):
            print_dict(rpc.nbd.nbd_get_disks(args.client,
                                             nbd_device=args.nbd_device))
        p = s.add_parser('nbd_get_disks', aliases=['get_nbd_disks'],
        ...
        p.set_defaults(func=do_nbd_get_disks)

    # net
    @subcommand
    def net_interface_add_ip_address(s):
        def do_net_interface_add_ip_address(args):
            rpc.net.net_interface_add_ip_address(args.client, ifc_index=args.ifc_index, ip_addr=args.ip_addr)
        p = s.add_parser('net_interface_add_ip_address', aliases=['add_ip_address'],
                         help='Add IP address')
        p.add_argument('ifc_index', help='ifc index of the nic device.', type=int)
        p.add_argument('ip_addr', help='ip address will be added.')
        p.set_defaults(func=do_net_interface_add_ip_address)

    @subcommand
    def net_interface_delete_ip_address(s):
        def do_net_interface_delete_ip_address(args):
            rpc.net.net_interface_delete_ip_address(args.client, ifc_index=args.ifc_index, ip_addr=args.ip_addr)
        p = s.add_parser('net_interface_delete_ip_address', aliases=['delete_ip_address'],
                         help='Delete IP address')
        p.add_argument('ifc_index', help='ifc index of the nic device.', type=int)
        p.add_argument('ip_addr', help='ip address will be deleted.')
        p.set_defaults(func=do_net_interface_delete_ip_address)

    @subcommand
    def net_get_interfaces(s):
        def do_net_get_interfaces(args):
        ...
        p.set_defaults(func=do_net_get_interfaces)

    # NVMe-oF
    @subcommand
    def nvmf_set_max_subsystems(s):
        def do_nvmf_set_max_subsystems(args):
            rpc.nvmf.nvmf_set_max_subsystems(args.client,
                                             max_subsystems=args.max_subsystems)
        p = s.add_parser('nvmf_set_max_subsystems', aliases=['set_nvmf_target_max_subsystems'],
        ...
        p.set_defaults(func=do_nvmf_set_max_subsystems)

    @subcommand
    def nvmf_set_config(s):
        def do_nvmf_set_config(args):
            rpc.nvmf.nvmf_set_config(args.client,
                                     acceptor_poll_rate=args.acceptor_poll_rate,
                                     conn_sched=args.conn_sched)
        p = s.add_parser('nvmf_set_config', aliases=['set_nvmf_target_config'],
        ...
        p.set_defaults(func=do_nvmf_set_config)

    @subcommand
    def nvmf_create_transport(s):
        def do_nvmf_create_transport(args):
        ...
        p = s.add_parser('nvmf_create_transport', help='Create NVMf transport')
        ...
        p.add_argument('-n', '--num-shared-buffers',
                       help='The number of pooled data buffers available to the transport', type=int)
        p.add_argument('-b', '--buf-cache-size',
                       help='The number of shared buffers to reserve for each poll group', type=int)
        p.add_argument('-s', '--max-srq-depth',
                       help='Max number of outstanding I/O per SRQ. Relevant only for RDMA transport', type=int)
        p.add_argument('-r', '--no-srq', action='store_true',
                       help='Disable per-thread shared receive queue. Relevant only for RDMA transport')
        p.add_argument('-o', '--c2h-success', action='store_false',
                       help='Disable C2H success optimization. Relevant only for TCP transport')
        p.add_argument('-f', '--dif-insert-or-strip', action='store_true',
                       help='Enable DIF insert/strip. Relevant only for TCP transport')
        p.add_argument('-y', '--sock-priority',
                       help='The sock priority of the tcp connection. Relevant only for TCP transport', type=int)
        p.set_defaults(func=do_nvmf_create_transport)

    @subcommand
    def nvmf_get_transports(s):
        def do_nvmf_get_transports(args):
            print_dict(rpc.nvmf.nvmf_get_transports(args.client, tgt_name=args.tgt_name))
        p = s.add_parser('nvmf_get_transports', aliases=['get_nvmf_transports'],
                         help='Display nvmf transports')
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_get_transports)

    @subcommand
    def nvmf_get_subsystems(s):
        def do_nvmf_get_subsystems(args):
            print_dict(rpc.nvmf.nvmf_get_subsystems(args.client, tgt_name=args.tgt_name))
        p = s.add_parser('nvmf_get_subsystems', aliases=['get_nvmf_subsystems'],
                         help='Display nvmf subsystems')
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_get_subsystems)

    @subcommand
    def nvmf_create_subsystem(s):
        def do_nvmf_create_subsystem(args):
        ...
        p = s.add_parser('nvmf_create_subsystem', aliases=['nvmf_subsystem_create'],
        ...
        p.add_argument("-m", "--max-namespaces", help="Maximum number of namespaces allowed",
                       type=int, default=0)
        p.set_defaults(func=do_nvmf_create_subsystem)

    @subcommand
    def nvmf_delete_subsystem(s):
        def do_nvmf_delete_subsystem(args):
            rpc.nvmf.nvmf_delete_subsystem(args.client,
                                           nqn=args.subsystem_nqn,
                                           tgt_name=args.tgt_name)
        p = s.add_parser('nvmf_delete_subsystem', aliases=['delete_nvmf_subsystem'],
        ...
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_delete_subsystem)

    @subcommand
    def nvmf_subsystem_add_listener(s):
        def do_nvmf_subsystem_add_listener(args):
            rpc.nvmf.nvmf_subsystem_add_listener(args.client,
                                                 ...
                                                 adrfam=args.adrfam,
                                                 trsvcid=args.trsvcid)
        p = s.add_parser('nvmf_subsystem_add_listener', help='Add a listener to an NVMe-oF subsystem')
        ...
        p.set_defaults(func=do_nvmf_subsystem_add_listener)

    @subcommand
    def nvmf_subsystem_remove_listener(s):
        def do_nvmf_subsystem_remove_listener(args):
            rpc.nvmf.nvmf_subsystem_remove_listener(args.client,
                                                    ...
                                                    adrfam=args.adrfam,
                                                    trsvcid=args.trsvcid)
        p = s.add_parser('nvmf_subsystem_remove_listener', help='Remove a listener from an NVMe-oF subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        p.add_argument('-t', '--trtype', help='NVMe-oF transport type: e.g., rdma', required=True)
        p.add_argument('-a', '--traddr', help='NVMe-oF transport address: e.g., an ip address', required=True)
        p.add_argument('-p', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.add_argument('-f', '--adrfam', help='NVMe-oF transport adrfam: e.g., ipv4, ipv6, ib, fc, intra_host')
        p.add_argument('-s', '--trsvcid', help='NVMe-oF transport service id: e.g., a port number')
        p.set_defaults(func=do_nvmf_subsystem_remove_listener)

    @subcommand
    def nvmf_subsystem_add_ns(s):
        def do_nvmf_subsystem_add_ns(args):
            rpc.nvmf.nvmf_subsystem_add_ns(args.client,
                                           ...
                                           eui64=args.eui64,
                                           uuid=args.uuid)
        p = s.add_parser('nvmf_subsystem_add_ns', help='Add a namespace to an NVMe-oF subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        ...
        p.set_defaults(func=do_nvmf_subsystem_add_ns)

    @subcommand
    def nvmf_subsystem_remove_ns(s):
        def do_nvmf_subsystem_remove_ns(args):
            rpc.nvmf.nvmf_subsystem_remove_ns(args.client,
                                              nqn=args.nqn,
                                              nsid=args.nsid,
                                              tgt_name=args.tgt_name)
        p = s.add_parser('nvmf_subsystem_remove_ns', help='Remove a namespace to an NVMe-oF subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        p.add_argument('nsid', help='The requested NSID', type=int)
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_subsystem_remove_ns)

    @subcommand
    def nvmf_subsystem_add_host(s):
        def do_nvmf_subsystem_add_host(args):
            rpc.nvmf.nvmf_subsystem_add_host(args.client,
                                             nqn=args.nqn,
                                             host=args.host,
                                             tgt_name=args.tgt_name)
        p = s.add_parser('nvmf_subsystem_add_host', help='Add a host to an NVMe-oF subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        p.add_argument('host', help='Host NQN to allow')
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_subsystem_add_host)

    @subcommand
    def nvmf_subsystem_remove_host(s):
        def do_nvmf_subsystem_remove_host(args):
            rpc.nvmf.nvmf_subsystem_remove_host(args.client,
                                                nqn=args.nqn,
                                                host=args.host,
                                                tgt_name=args.tgt_name)
        p = s.add_parser('nvmf_subsystem_remove_host', help='Remove a host from an NVMe-oF subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        p.add_argument('host', help='Host NQN to remove')
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_subsystem_remove_host)

    @subcommand
    def nvmf_subsystem_allow_any_host(s):
        def do_nvmf_subsystem_allow_any_host(args):
            rpc.nvmf.nvmf_subsystem_allow_any_host(args.client,
                                                   nqn=args.nqn,
                                                   disable=args.disable,
                                                   tgt_name=args.tgt_name)
        p = s.add_parser('nvmf_subsystem_allow_any_host', help='Allow any host to connect to the subsystem')
        p.add_argument('nqn', help='NVMe-oF subsystem NQN')
        ...
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_subsystem_allow_any_host)

    @subcommand
    def nvmf_get_stats(s):
        def do_nvmf_get_stats(args):
        ...
        p.add_argument('-t', '--tgt_name', help='The name of the parent NVMe-oF target (optional)', type=str)
        p.set_defaults(func=do_nvmf_get_stats)

    # pmem
    @subcommand
    def bdev_pmem_create_pool(s):
        def do_bdev_pmem_create_pool(args):
            num_blocks = int((args.total_size * 1024 * 1024) / args.block_size)
            rpc.pmem.bdev_pmem_create_pool(args.client,
                                           pmem_file=args.pmem_file,
                                           num_blocks=num_blocks,
                                           block_size=args.block_size)
        p = s.add_parser('bdev_pmem_create_pool', aliases=['create_pmem_pool'],
        ...
        p.set_defaults(func=do_bdev_pmem_create_pool)

    @subcommand
    def bdev_pmem_get_pool_info(s):
        def do_bdev_pmem_get_pool_info(args):
            print_dict(rpc.pmem.bdev_pmem_get_pool_info(args.client,
                                                        pmem_file=args.pmem_file))
        p = s.add_parser('bdev_pmem_get_pool_info', aliases=['pmem_pool_info'],
                         help='Display pmem pool info and check consistency')
        p.add_argument('pmem_file', help='Path to pmemblk pool file')
        p.set_defaults(func=do_bdev_pmem_get_pool_info)

    @subcommand
    def bdev_pmem_delete_pool(s):
        def do_bdev_pmem_delete_pool(args):
            rpc.pmem.bdev_pmem_delete_pool(args.client,
                                           pmem_file=args.pmem_file)
        p = s.add_parser('bdev_pmem_delete_pool', aliases=['delete_pmem_pool'],
                         help='Delete pmem pool')
        p.add_argument('pmem_file', help='Path to pmemblk pool file')
        p.set_defaults(func=do_bdev_pmem_delete_pool)

    # subsystem
    @subcommand
    def framework_get_subsystems(s):
        def do_framework_get_subsystems(args):
            print_dict(rpc.subsystem.framework_get_subsystems(args.client))
        p = s.add_parser('framework_get_subsystems', aliases=['get_subsystems'],
        ...
        p.set_defaults(func=do_framework_get_subsystems)

    @subcommand
    def framework_get_config(s):
        def do_framework_get_config(args):
            print_dict(rpc.subsystem.framework_get_config(args.client, args.name))
        p = s.add_parser('framework_get_config', aliases=['get_subsystem_config'],
                         help="""Print subsystem configuration""")
        p.add_argument('name', help='Name of subsystem to query')
        p.set_defaults(func=do_framework_get_config)

    # vhost
    @subcommand
    def vhost_controller_set_coalescing(s):
        def do_vhost_controller_set_coalescing(args):
            rpc.vhost.vhost_controller_set_coalescing(args.client,
                                                      ctrlr=args.ctrlr,
                                                      delay_base_us=args.delay_base_us,
                                                      iops_threshold=args.iops_threshold)
        p = s.add_parser('vhost_controller_set_coalescing', aliases=['set_vhost_controller_coalescing'],
        ...
        p.set_defaults(func=do_vhost_controller_set_coalescing)

    @subcommand
    def vhost_create_scsi_controller(s):
        def do_vhost_create_scsi_controller(args):
            rpc.vhost.vhost_create_scsi_controller(args.client,
                                                   ctrlr=args.ctrlr,
                                                   cpumask=args.cpumask)
        p = s.add_parser('vhost_create_scsi_controller', aliases=['construct_vhost_scsi_controller'],
                         help='Add new vhost controller')
        p.add_argument('ctrlr', help='controller name')
        p.add_argument('--cpumask', help='cpu mask for this controller')
        p.set_defaults(func=do_vhost_create_scsi_controller)

    @subcommand
    def vhost_scsi_controller_add_target(s):
        def do_vhost_scsi_controller_add_target(args):
            print_json(rpc.vhost.vhost_scsi_controller_add_target(args.client,
                                                                  ctrlr=args.ctrlr,
                                                                  scsi_target_num=args.scsi_target_num,
                                                                  bdev_name=args.bdev_name))
        p = s.add_parser('vhost_scsi_controller_add_target', aliases=['add_vhost_scsi_lun'],
        ...
        p.set_defaults(func=do_vhost_scsi_controller_add_target)

    @subcommand
    def vhost_scsi_controller_remove_target(s):
        def do_vhost_scsi_controller_remove_target(args):
            rpc.vhost.vhost_scsi_controller_remove_target(args.client,
                                                          ctrlr=args.ctrlr,
                                                          scsi_target_num=args.scsi_target_num)
        p = s.add_parser('vhost_scsi_controller_remove_target', aliases=['remove_vhost_scsi_target'],
                         help='Remove target from vhost controller')
        p.add_argument('ctrlr', help='controller name to remove target from')
        p.add_argument('scsi_target_num', help='scsi_target_num', type=int)
        p.set_defaults(func=do_vhost_scsi_controller_remove_target)

    @subcommand
    def vhost_create_blk_controller(s):
        def do_vhost_create_blk_controller(args):
            rpc.vhost.vhost_create_blk_controller(args.client,
                                                  ctrlr=args.ctrlr,
                                                  dev_name=args.dev_name,
                                                  cpumask=args.cpumask,
                                                  readonly=args.readonly)
        p = s.add_parser('vhost_create_blk_controller', aliases=['remove_vhost_scsi_target'],
        ...
        p.set_defaults(func=do_vhost_create_blk_controller)

    @subcommand
    def vhost_create_nvme_controller(s):
        def do_vhost_create_nvme_controller(args):
            rpc.vhost.vhost_create_nvme_controller(args.client,
                                                   ctrlr=args.ctrlr,
                                                   io_queues=args.io_queues,
                                                   cpumask=args.cpumask)
        p = s.add_parser('vhost_create_nvme_controller', aliases=['vhost_create_nvme_controller'],
                         help='Add new vhost controller')
        p.add_argument('ctrlr', help='controller name')
        p.add_argument('io_queues', help='number of IO queues for the controller', type=int)
        p.add_argument('--cpumask', help='cpu mask for this controller')
        p.set_defaults(func=do_vhost_create_nvme_controller)

    @subcommand
    def vhost_nvme_controller_add_ns(s):
        def do_vhost_nvme_controller_add_ns(args):
            rpc.vhost.vhost_nvme_controller_add_ns(args.client,
                                                   ctrlr=args.ctrlr,
                                                   bdev_name=args.bdev_name)
        p = s.add_parser('vhost_nvme_controller_add_ns', aliases=['add_vhost_nvme_ns'],
        ...
        p.set_defaults(func=do_vhost_nvme_controller_add_ns)

    @subcommand
    def vhost_get_controllers(s):
        def do_vhost_get_controllers(args):
            print_dict(rpc.vhost.vhost_get_controllers(args.client, args.name))
        p = s.add_parser('vhost_get_controllers', aliases=['get_vhost_controllers'],
                         help='List all or specific vhost controller(s)')
        p.add_argument('-n', '--name', help="Name of vhost controller", required=False)
        p.set_defaults(func=do_vhost_get_controllers)

    @subcommand
    def vhost_delete_controller(s):
        def do_vhost_delete_controller(args):
            rpc.vhost.vhost_delete_controller(args.client,
                                              ctrlr=args.ctrlr)
        p = s.add_parser('vhost_delete_controller', aliases=['remove_vhost_controller'],
                         help='Delete a vhost controller')
        p.add_argument('ctrlr', help='controller name')
        p.set_defaults(func=do_vhost_delete_controller)

    @subcommand
    def bdev_virtio_attach_controller(s):
        def do_bdev_virtio_attach_controller(args):
        ...
        p = s.add_parser('bdev_virtio_attach_controller', aliases=['construct_virtio_dev'],
        ...
        p.set_defaults(func=do_bdev_virtio_attach_controller)

    @subcommand
    def bdev_virtio_scsi_get_devices(s):
        def do_bdev_virtio_scsi_get_devices(args):
            print_dict(rpc.vhost.bdev_virtio_scsi_get_devices(args.client))
        p = s.add_parser('bdev_virtio_scsi_get_devices', aliases=['get_virtio_scsi_devs'],
                         help='List all Virtio-SCSI devices.')
        p.set_defaults(func=do_bdev_virtio_scsi_get_devices)

    @subcommand
    def bdev_virtio_detach_controller(s):
        def do_bdev_virtio_detach_controller(args):
            rpc.vhost.bdev_virtio_detach_controller(args.client,
                                                    name=args.name)
        p = s.add_parser('bdev_virtio_detach_controller', aliases=['remove_virtio_bdev'],
        ...
        p.set_defaults(func=do_bdev_virtio_detach_controller)

    # ioat
    @subcommand
    def ioat_scan_copy_engine(s):
        def do_ioat_scan_copy_engine(args):
            pci_whitelist = []
        ...
        p = s.add_parser('ioat_scan_copy_engine', aliases=['scan_ioat_copy_engine'],
        ...
        p.set_defaults(func=do_ioat_scan_copy_engine)

    # opal
    @subcommand
    def bdev_nvme_opal_init(s):
        def do_bdev_nvme_opal_init(args):
            rpc.nvme.bdev_nvme_opal_init(args.client,
                                         nvme_ctrlr_name=args.nvme_ctrlr_name,
                                         password=args.password)
        p = s.add_parser('bdev_nvme_opal_init', help='take ownership and activate')
        p.add_argument('-b', '--nvme-ctrlr-name', help='nvme ctrlr name')
        p.add_argument('-p', '--password', help='password for admin')
        p.set_defaults(func=do_bdev_nvme_opal_init)

    @subcommand
    def bdev_nvme_opal_revert(s):
        def do_bdev_nvme_opal_revert(args):
            rpc.nvme.bdev_nvme_opal_revert(args.client,
                                           nvme_ctrlr_name=args.nvme_ctrlr_name,
                                           password=args.password)
        p = s.add_parser('bdev_nvme_opal_revert', help='Revert to default factory settings')
        p.add_argument('-b', '--nvme-ctrlr-name', help='nvme ctrlr name')
        p.add_argument('-p', '--password', help='password')
        p.set_defaults(func=do_bdev_nvme_opal_revert)

    @subcommand
    def bdev_opal_create(s):
        def do_bdev_opal_create(args):
        ...
        p = s.add_parser('bdev_opal_create', help="""Create opal bdev on specified NVMe controller""")
        ...
        p.set_defaults(func=do_bdev_opal_create)

    @subcommand
    def bdev_opal_get_info(s):
        def do_bdev_opal_get_info(args):
            print_dict(rpc.bdev.bdev_opal_get_info(args.client,
                                                   bdev_name=args.bdev_name,
                                                   password=args.password))
        p = s.add_parser('bdev_opal_get_info', help='get opal locking range info for this bdev')
        p.add_argument('-b', '--bdev-name', help='opal bdev')
        p.add_argument('-p', '--password', help='password')
        p.set_defaults(func=do_bdev_opal_get_info)

    @subcommand
    def bdev_opal_delete(s):
        def do_bdev_opal_delete(args):
            rpc.bdev.bdev_opal_delete(args.client,
                                      bdev_name=args.bdev_name,
                                      password=args.password)
        p = s.add_parser('bdev_opal_delete', help="""delete a virtual opal bdev""")
        p.add_argument('-b', '--bdev-name', help='opal virtual bdev', required=True)
        p.add_argument('-p', '--password', help='admin password', required=True)
        p.set_defaults(func=do_bdev_opal_delete)

    @subcommand
    def bdev_opal_new_user(s):
        def do_bdev_opal_new_user(args):
            rpc.bdev.bdev_opal_new_user(args.client,
                                        bdev_name=args.bdev_name,
                                        admin_password=args.admin_password,
                                        user_id=args.user_id,
                                        user_password=args.user_password)
        p = s.add_parser('bdev_opal_new_user', help="""Add a user to opal bdev who can set lock state for this bdev""")
        ...
        p.set_defaults(func=do_bdev_opal_new_user)

    @subcommand
    def bdev_opal_set_lock_state(s):
        def do_bdev_opal_set_lock_state(args):
            rpc.bdev.bdev_opal_set_lock_state(args.client,
                                              bdev_name=args.bdev_name,
                                              user_id=args.user_id,
                                              password=args.password,
                                              lock_state=args.lock_state)
        p = s.add_parser('bdev_opal_set_lock_state', help="""set lock state for an opal bdev""")
        p.add_argument('-b', '--bdev-name', help='opal bdev', required=True)
        ...
        p.set_defaults(func=do_bdev_opal_set_lock_state)

    # bdev_nvme_send_cmd
    @subcommand
    def bdev_nvme_send_cmd(s):
        def do_bdev_nvme_send_cmd(args):
        ...
        p = s.add_parser('bdev_nvme_send_cmd', aliases=['send_nvme_cmd'], help='NVMe passthrough cmd.')
        ...
        p.set_defaults(func=do_bdev_nvme_send_cmd)

    # Notifications
    @subcommand
    def notify_get_types(s):
        def do_notify_get_types(args):
            print_dict(rpc.notify.notify_get_types(args.client))
        p = s.add_parser('notify_get_types', aliases=['get_notification_types'],
                         help='List available notifications that user can subscribe to.')
        p.set_defaults(func=do_notify_get_types)

    @subcommand
    def notify_get_notifications(s):
        def do_notify_get_notifications(args):
            ret = rpc.notify.notify_get_notifications(args.client,
                                                      id=args.id,
                                                      max=args.max)
            print_dict(ret)
        p = s.add_parser('notify_get_notifications', aliases=['get_notifications'],
        ...
        p.set_defaults(func=do_notify_get_notifications)

    @subcommand
    def thread_get_stats(s):
        def do_thread_get_stats(args):
            print_dict(rpc.app.thread_get_stats(args.client))
        p = s.add_parser(
            'thread_get_stats', help='Display current statistics of all the threads')
        p.set_defaults(func=do_thread_get_stats)

    # blobfs
    @subcommand
    def blobfs_detect(s):
        def do_blobfs_detect(args):
            print(rpc.blobfs.blobfs_detect(args.client,
                                           bdev_name=args.bdev_name))
        p = s.add_parser('blobfs_detect', help='Detect whether a blobfs exists on bdev')
        p.add_argument('bdev_name', help='Blockdev name to detect blobfs. Example: Malloc0.')
        p.set_defaults(func=do_blobfs_detect)

    @subcommand
    def blobfs_create(s):
        def do_blobfs_create(args):
            print(rpc.blobfs.blobfs_create(args.client,
                                           bdev_name=args.bdev_name,
                                           cluster_sz=args.cluster_sz))
        p = s.add_parser('blobfs_create', help='Build a blobfs on bdev')
        ...
        p.set_defaults(func=do_blobfs_create)

    @subcommand
    def blobfs_mount(s):
        def do_blobfs_mount(args):
            print(rpc.blobfs.blobfs_mount(args.client,
                                          bdev_name=args.bdev_name,
                                          mountpoint=args.mountpoint))
        p = s.add_parser('blobfs_mount', help='Mount a blobfs on bdev to host path by FUSE')
        ...
        p.set_defaults(func=do_blobfs_mount)

    @subcommand
    def blobfs_set_cache_size(s):
        def do_blobfs_set_cache_size(args):
            print(rpc.blobfs.blobfs_set_cache_size(args.client,
                                                   size_in_mb=args.size_in_mb))
        p = s.add_parser('blobfs_set_cache_size', help='Set cache size for blobfs')
        p.add_argument('size_in_mb', help='Cache size for blobfs in megabytes.', type=int)
        p.set_defaults(func=do_blobfs_set_cache_size)

    def execute_subcommand(subcommand, argv):
        subcommand_name = subcommand
        if subcommand_name in deprecated_aliases:
            subcommand_name = deprecated_aliases[subcommand_name]
            print("{} is deperecated, use {} instead.".format(subcommand, subcommand_name), file=sys.stderr)

        if subcommand_name not in subcommands:
            print(">>> RPC %s not found\n" % subcommand_name)
            parser.print_help()
            return False

        try:
            subcommands[subcommand_name](argv)
    ...
    def execute_script(parser, fd):
        ...
        for rpc_call in map(str.rstrip, fd):
            ...
            split_cmd = shlex.split(rpc_call)
            if not execute_subcommand(split_cmd[0], sys.argv[1:] + split_cmd):
    ...
    if args.rest:
        if args.rest[0] == "list":
            ...
            if not execute_subcommand(args.rest[0], sys.argv[1:]):
                exit(1)
        exit(0)
    ...
    if sys.stdin.isatty():
        ...
        execute_script(parser, sys.stdin)
