...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
    ...
    *** Mutually exclusive with --disable-chap ***""", action='store_true')
    ...
    p.add_argument('-b', '--name', help="Name of the bdev", required=True)
    p.add_argument('-d', '--base_bdev', help='Name of zoned bdev used as underlaying device',
                   required=True)
    ...
    p.add_argument('-r', '--range', help='Parallel unit range (in the form of BEGIN-END)', required=False)
