...
if __name__ == "__main__":
    ...
    @call_cmd
    def enable_bdev_queue_depth_tracking(args):
        rpc.bdev.enable_bdev_queue_depth_tracking(args.client,
                                                  name=args.name,
                                                  period=args.period)

    p = subparsers.add_parser('enable_bdev_queue_depth_tracking', help='Enable tracking of a bdev\'s queue depth.')
    p.add_argument('name', help='Blockdev name. Example: Malloc0', required=False)
    p.add_argument('period', help='Period with which to poll the block device queue depth in microseconds. default 1000',
