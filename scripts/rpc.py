...
if __name__ == "__main__":
    ...
    *** The devices must pre-exist ***""")
    ...
    p.add_argument('-o', '--nop_timeout', help='Timeout in seconds to nop-in request to the initiator', type=int)
    p.add_argument('-n', '--nop-in-interval', help='Time interval in seconds between nop-in requests sent by the target', type=int)
    ...
    *** Authentication group must be precreated ***""", type=int, default=0)
    ...
    def initialize_nbd_subsystem(args):
        rpc.subsystem.initialize_nbd_subsystem(args.client)
