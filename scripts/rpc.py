...
if __name__ == "__main__":
    ...
    def execute_subcommand(subcommand_name):
        if subcommand_name in deprecated_aliases:
            subcommand_name = deprecated_aliases[subcommand_name]
            print("{} is deperecated, use {} instead.".format(args.rpc_name, subcommand_name), file=sys.stderr)
    ...
    def execute_script(parser, client, fd):
        ...
        for rpc_call in map(str.rstrip, fd):
            ...
            if not execute_subcommand(shlex.split(rpc_call)):
    ...
    parser.add_argument("rpc_name",
                        help="The RPC to execute. List RPC names by using 'rpc.py list'."
                        " Get help on specific RPCs by using 'rpc.py <name> -h'")
    ...
    if args.rpc_name == "list":
        list()
        exit(0)

    if execute_subcommand(args.rpc_name):
        exit(0)
