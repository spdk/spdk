...
if __name__ == "__main__":
    ...
    p = subparsers.add_parser('clear_config', help="""Clear SPDK subsystems and targets using JSON RPC""")
    p.set_defaults(func=rpc.clear_config)

    @call_cmd
    def clear_subsystem(args):
        rpc.clear_subsystem(args.client, args)

    p = subparsers.add_parser('clear_subsystem', help="""Clear SPDK subsystems using JSON RPC""")
    p.add_argument('--subsystem', help="""Subsystem name""")
    p.set_defaults(func=rpc.clear_subsystem)
    ...
    *** The devices must pre-exist ***""")
    ...
    def construct_virtio_dev(args):
        print_dict(rpc.vhost.construct_virtio_dev(args))

    p = subparsers.add_parser('construct_virtio_dev', help="""Construct new virtio device using provided
    transport type and device type. In case of SCSI device type this imply scan and add bdevs offered by
    ...
    p.add_argument('-a', '--traddr',
                   help='Transport type specifig target address: e.g. UNIX domain socket path or BDF', required=True)
