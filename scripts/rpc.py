...
if __name__ == "__main__":
    ...
    If no file or file is '-' write configuration to stdout.""")
    p.add_argument('--filename', help="""File where to save JSON configuration to.""")
    p.add_argument('--indent', help="""Indent level. Value less than 0 mean compact mode. If filename is not given default
    indent level is 4. If writing to file of filename is '-' then default is compact mode.""", type=int)
    ...
    p.add_argument('-b', '--name', help="Name of the Blockdev. Example: Nvme0n1", required=False)
    p.set_defaults(func=rpc.bdev.get_bdevs_config)
