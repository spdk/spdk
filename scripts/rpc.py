...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', dest='server_addr',
                        help='RPC server address', default='127.0.0.1')
    parser.add_argument('-p', dest='port',
                        help='RPC port number', default=5260, type=int)
    ...
    p = subparsers.add_parser(
        'get_bdevs', help='Display current blockdev list')
    ...
    p.add_argument(
        'type', help="""type: 'clear' 'read' 'write' 'unmap' 'flush' 'reset' 'all'""")
    p.add_argument(
    ...
    p.set_defaults(func=rpc.iscsi.get_luns)
    ...
    *** Authentication group must be precreated ***""", type=int)
    ...
    p = subparsers.add_parser('construct_nvmf_subsystem',
                              help='Add a nvmf subsystem')
    p.add_argument("-c", "--core", help='The core Nvmf target run on',
                   type=int, default=-1)
    ...
    *** The devices must pre-exist ***""")
    ...
    # vhost
    p = subparsers.add_parser('get_vhost_scsi_controllers',
                              help='List vhost controllers')
    p.set_defaults(func=rpc.vhost.get_vhost_scsi_controllers)
    ...
    p = subparsers.add_parser('remove_vhost_scsi_controller',
                              help='Remove vhost controller')
    p.add_argument('ctrlr', help='controller name')
    p.set_defaults(func=rpc.vhost.remove_vhost_scsi_controller)
    ...
    p = subparsers.add_parser('remove_vhost_scsi_dev',
                              help='Remove device from vhost controller')
    p.add_argument('ctrlr', help='controller name to remove device from')
    p.add_argument('scsi_dev_num', help='scsi_dev_num', type=int)
    p.set_defaults(func=rpc.vhost.remove_vhost_scsi_dev)

    args = parser.parse_args()

    args.client = JSONRPCClient(args.server_addr, args.port)
    args.func(args)
