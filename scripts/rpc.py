...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
    ...
    *** The Portal/Initiator Groups must be precreated ***""")
    ...
    # OCSSD
    def zdev_ocssd_attach_controller(args):
        print_array(rpc.bdev.zdev_ocssd_attach_controller(args.client,
                                                          name=args.name,
                                                          trtype=args.trtype,
                                                          traddr=args.traddr))

    p = subparsers.add_parser('zdev_ocssd_attach_controller',
                              help='Creates zdevs on specified Open Channel controller')
    p.add_argument('-b', '--name', help='Name of the OC NVMe controller, prefix for each zdev name',
                   required=True)
    p.add_argument('-t', '--trtype',
                   help='NVMe-oF target trtype: e.g., rdma, pcie,  default: pcie', default='pcie')
    p.add_argument('-a', '--traddr',
                   help='NVMe-oF target address: e.g., an ip address or BDF', required=True)
    p.set_defaults(func=zdev_ocssd_attach_controller)

    def zdev_ocssd_detach_controller(args):
        rpc.bdev.zdev_ocssd_detach_controller(args.client,
                                              name=args.name)

    p = subparsers.add_parser('zdev_ocssd_detach_controller',
                              help='Detaches Open Channel controller and deletes zdevs created on top of it')
    p.add_argument('-b', '--name', help='Name of the OC NVMe controller', required=True)
    p.set_defaults(func=zdev_ocssd_detach_controller)
