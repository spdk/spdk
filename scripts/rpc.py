    ...
    *** The Portal/Initiator Groups must be precreated ***""")
    ...
    def execute_script(parser, client, fd):
        for rpc_call in map(str.rstrip, fd):
            ...
            if argv[0] != args.func.__name__:
                print("{} is deprecated, use {} instead.".format(argv[0], args.func.__name__), file=sys.stderr)
            args.client = client
            call_rpc_func(args)
    ...
    if hasattr(args, 'func'):
        ...
        if func_name_used != args.func.__name__:
            print("{} is deprecated, use {} instead.".format(func_name_used, args.func.__name__), file=sys.stderr)
        call_rpc_func(args)
