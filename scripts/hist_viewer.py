...
def plot_histogram(x, y, z, diaplay_file):
    ...
    plt.title('Histogram (freq > 0.01%)', fontsize=20)
...
def show_histogram_statistics(out_fp):
    ...
    print >>out_fp, "%-24s: %lu" % ("Min Value", min_value)
    print >>out_fp, "%-24s: %lu" % ("Max Value", max_value)
    print >>out_fp, "%-24s: %.2f" % ("Avg Value", total_values / float(total_num_ios) if total_num_ios != 0 else 0)
    print >>out_fp, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n"
