...
def show_histogram_bins(out_fp):
    ...
    z = []
    percentile = 0.0
    total_ios = long(json_data["total_ios"])
    ...
    for hist_bin in histogram_raw_data:
        ...
        percent = (freq * 100) / float(total_ios) if total_ios != 0 else 0
...
def show_histogram_statistics(out_fp):
    ...
    print >>out_fp, "%-24s: %lu" % ("Total I/O", total_ios)
    print >>out_fp, "%-24s: %lu" % ("Min Value", min_value * if_ticks_to_nsec)
    print >>out_fp, "%-24s: %lu" % ("Max Value", max_value * if_ticks_to_nsec)
    print >>out_fp, "%-24s: %.2f" % ("Avg Value", total_values * if_ticks_to_nsec / float(total_ios) if total_ios != 0 else 0)
    print >>out_fp, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n"
