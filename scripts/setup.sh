...
function configure_linux {
	...
	if [ "$CURENT_SOFT_ULIMIT" != "unlimited" ]; then
		echo ""
		echo "WARNING: soft ulimit for max open files limit is set to $CURENT_SOFT_ULIMIT."
		echo "SPDK won't be able to use more than $CURENT_SOFT_ULIMIT hugepages as a current user."
		echo ""
	fi
	CURENT_HARD_ULIMIT=$(ulimit -Hn)
	if [ "$CURENT_HARD_ULIMIT" != "unlimited" ]; then
		echo ""
		echo "WARNING: hard ulimit for max open files limit is set to $CURENT_HARD_ULIMIT."
		echo "SPDK won't be able to use more than $CURENT_HARD_ULIMIT hugepages as root user."
		echo ""
	fi
