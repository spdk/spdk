...
function usage()
{
	...
	[[ ! -z $2 ]] && ( echo "$2"; echo ""; )
	echo "Helper script for allocating hugepages and binding NVMe, IO/AT and Virtio devices to"
	...
	echo "DESTINATION_USER  User that will own hugepage mountpoint directory and vfio groups."
	echo "                  If unset, the current user will be used."
	exit 0
}
...
function linux_bind_driver() {
	...
	if [ -e "/dev/vfio/$iommu_group" ]; then
		if [ -z "$DESTINATION_USER" ]; then
			chown "$DESTINATION_USER" "/dev/vfio/$iommu_group"
		fi
	fi
}
...
function configure_linux {
	...
	if [ "$driver_name" = "vfio-pci" ]; then
		if [ -z "$DESTINATION_USER" ]; then
			chown "$DESTINATION_USER" "$hugetlbfs_mount"
...
if [ -z "$DESTINATION_USER" ]; then
	DESTINATION_USER="$SUDO_USER"
	if [ -z "$DESTINATION_USER" ]; then
		DESTINATION_USER=`logname 2>/dev/null` || true
	fi
fi
