...
sh -c "ulimit -c unlimited && exec /usr/bin/ceph-mon --mkfs -c ${ceph_conf} -i a --monmap=${base_dir}/ceph/monmap --keyring=${base_dir}/ceph/keyring --mon-data=${mon_dir}"

cp ${base_dir}/ceph/keyring ${mon_dir}/keyring

cp ${base_dir}/ceph.conf /etc/ceph/ceph.conf

cp ${base_dir}/ceph/keyring /etc/ceph/keyring

/usr/bin/ceph-run sh -c "ulimit -n 16384 && ulimit -c unlimited && exec /usr/bin/ceph-mon -c ${ceph_conf} -i a --keyring=${base_dir}/ceph/keyring --pid-file=${base_dir}/ceph/pid/root@`hostname`.pid --mon-data=${mon_dir}"
...
mkdir -p ${pid_dir}
env -i TCMALLOC_MAX_TOTAL_THREAD_CACHE_BYTES=134217728 /usr/bin/ceph-osd -c ${ceph_conf} -i 0 --pid-file=${pid_dir}/ceph-osd.0.pid
