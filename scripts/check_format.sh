...
shellcheck_bin=""
if hash spdk_shellcheck 2>/dev/null; then
	shellcheck_bin="spdk_shellcheck"
...
if [ -n "$shellcheck_bin" ]; then
	...
	echo -n "Using $shellcheck_bin "

	shellcheck_v=$($shellcheck_bin --version | grep -P "version: [0-9\.]+" | cut -d " " -f2)
...
SC2199,SC2206,SC2207,SC2209,SC2214,SC2219,SC2220,SC2223,SC2230,SC2231,SC2233,SC2235"
	...
	error=0
	git ls-files '*.sh' | xargs -P$(nproc) -n1 $shellcheck_bin $SHCH_ARGS > shellcheck.log || error=1
	if [ $error -ne 0 ]; then
