...
OCF virtual bdev is a part of SPDK OCF module which is based on [Open CAS Framework](https://github.com/Open-CAS/) - a high performance block storage caching meta-library.
To enable the module, user needs to provide `--with-ocf=/path/to/ocf/library` for configure script.
OCF bdev can be configured to enable caching for any underlying bdev.

Below is an example command for creating OCF bdev:

`rpc.py construct_ocf_bdev Cache1 wt Malloc0 Nvme0n1`

This command will create new OCF bdev `Cache1` having bdevs `Malloc0` as caching-device and `Nvme0n1` as core-device and initial cache mode `Write-Through`.
By default cache line size is 4KiB and non-volatile metadata is disabled.
Data written to `Cache1` will be present on `Nvme0n1` eventually.

To remove `Cache1`:

`rpc.py delete_ocf_bdev Cache1`

During removal OCF-cache will be stopped and all warm data will be written to core device.

Note that OCF has a per-device RAM requirement of about 56MiB + (58 / _cache line size_) * _cache device size_.
To get more information on OCF please visit [OCF documentation](https://open-cas.github.io/doxygen/ocf/).
