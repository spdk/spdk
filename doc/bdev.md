...
which is consumed by any upper layers. Bdev name is required parameter for this
RPC command and at least one of `rw_ios_per_sec` and `rw_mbytes_per_sec` two
parameters is needed. When both rate limits are enabled, the first met limit will
take effect. Specifically, the value of 0 is to disable the corresponding rate
