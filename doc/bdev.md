...
OCF bdev can be used enable caching for any underlying bdev.

Below is an example command for creating OCF bdev:

`rpc.py construct_ocf_bdev Cache1 wt Malloc0 Nvme0n1`

This command will create new OCF bdev `Cache1` having bdevs `Malloc0` as caching-device
...
During removal OCF-cache will be stopped and all warm data will be written to the core device.
