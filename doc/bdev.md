...
OCF virtual bdev is a part of SPDK OCF module which is based on Open CAS Framework - a high performance block storage caching meta-library.
To enable the module, user needs to provide `--with-ocf=/path/to/ocf/library` for configure script.

OCF bdev can be configured to enable caching for any underlying bdev (assuming that OCF RAM requirement for the core device is met).
