...
relies on an internal SPDK library called  accomplish this, see @ref reduce for detailed
information.
...
only compression modules as well hardware assisted support for the Intel QAT board. At this
...
not point to persistent memory (i.e. a regular filesystem) performance will be severally
impacted.  The vbdev module and reduce libraries were designed to use persistent memory for
any production use.

Example command

`rpc.py construct_compress_bdev -p ~/pm_files -b myLvol`
...
`rpc.py delete_compress_bdev myLvol`
