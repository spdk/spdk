...
and QEMU itself. SPDK provides an accelerated vhost target through application of
the same SPDK userspace polled-mode principles. What's more, its initiators are
told not to send any interrupts on I/O submission, meaning that they will have less
responsibility and lower CPU usage on heavy I/O workloads.
...
The following diagram presents how QEMU-based VM communicates with an SPDK vhost device.
    ...
    +----------------------+             |  |                       |        |  |
                ...
                |                                      |            |
    +-----------v---------------------------------------------------v-----------+
    |                   Virtqueues in shared hugepage memory                    |
    +---------------------------------------------------------------------------+
