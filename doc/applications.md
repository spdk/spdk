...
`RUNTIME` state. When the client finishes configuring the SPDK subsystems also known
as modules it needs to issue the @ref rpc_framework_start_init RPC command to begin
the initialization process. After `rpc_framework_start_init` returns `true` SPDK
