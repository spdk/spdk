...
Similarly to other bdevs, the FTL bdevs can be created either based on JSON config files or via RPC.
...
Zone block bdev is a bdev adapter between regular `bdev` and `bdev_zone`. It emulates zoned
interface on top regular block device.

In order to create FTL on to of regular bdev:
1) Create regular bdev e.g. `bdev_nvme`, `bdev_null`, `bdev_malloc`
2) Create zone block bdev on top regular bdev created in step 1 (user could specify zone capacity
