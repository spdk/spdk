...
geometry of the SSD:
 ...
 - `lba_index` - default LBA format. Supported values:
 ...
 - `laer_thread_sleep` - timeout in ms between asynchronous events requesting the host to relocate the
   data based on media feedback
...
```
$ /path/to/qemu [OTHER PARAMETERS] -drive format=raw,file=/path/to/data/file,if=none,id=ocssd
	-device nvme,drive=ocssd,serial=deadbeef,lver=2,lba_index=3,lnum_ch=1,lnum_lun=8,lnum_pln=4,
