...
include/spdk_internal/bdev.h. In that header a macro is defined that register
a new bdev module named SPDK_BDEV_MODULE_REGISTER. This macro takes as
argument a pointer spdk_bdev_module_if structure that is used to
register new bdev module.

The spdk_bdev_module_if structure describe the module properties like
initialization (`module_init`) and teardown (`module_fini`) functions,
the function that return context size (`get_ctx_size`) - scratch space that
