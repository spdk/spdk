...
This programming guide is intended for developers authoring their own
block device modules to integrate with SPDK's bdev layer. For a guide
on how to use the bdev layer, see @ref bdev_pg.
...
New bdevs are created within the module by calling spdk_bdev_register. The bdev provided is allocated by the module and must be filled out appropriately. The most important field to fill out is `fn_table`, which points at this data structure:
