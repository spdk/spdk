...
2. Developers can integrate the libary in their own NVMe-oF application with or without integrating
...
to connect all avaiable transports and subsystems. For the connections, they will be placed on the
...
NVMe driver) and so on. The general relationship of NVMe-oF target library, bdev libary and nvme
...
places a single poll group on each thread. However, the NVMe-oF library doesn't spawn any threads and
has looser requirements. The primary requirement is that a poll group can only be accessed from the
...
1. Connections can be placed on any thread and all the NVMe I/O commands can be exeucted on any thread.
...
**spdk_nvmf_subsystem_destroy**: destory a NVMe-oF subsystem. A subsystem may only be destroyed
...
**spdk_nvmf_poll_group_create**: create a poll group. All avaiable transports and subsystems will
...
is added based on the round robin mode to pick the avaiable poll group.
