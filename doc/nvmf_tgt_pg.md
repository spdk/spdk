...
2) the NVMe-oF target libraries.
~~~

The target application also invokes to interact with the libraries. This served two purposes:
~~~
1) developers can directly start this standalone application with or without updating the libraries;
...
1. All IO related resources are allocated, retrieved and managed on the one and only one core along
with the IO handling on the same core.
