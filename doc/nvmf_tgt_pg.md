...
The SPDK NVMe-oF target application depends on the NVMe-oF library. Developers may consume this
code using one of two strategies:

1. Using the NVMe-oF target application as-is, and as necessary adding new features (and hopefully
contributing code back to the main SPDK project!).

2. Using the NVMe-oF library inside of their own application.

This guide is applicable to both strategies.
