...
devices to all be located on the same NUMA node. The following parameters in the configuration file
are showed to configure SPDK NVMe-oF target:
...
**Subsystem configurtion:** the [Subsystem] section in the configuration file is used to configure
subysystem for the NVMe-oF target.
~~~{.sh}
[Nvme]
...
Namespace Nvme1n1 1
~~~
Currently, SPDK spreads the execution of requests for a single subsystem across all available cores 
