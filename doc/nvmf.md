...
It requires an RDMA-capable NIC with its corresponding OFED software package
installed to run. The target should work on all flavors of RDMA, but it is currently tested against
Mellanox NICs (RoCEv2) and Chelsio NICs (iWARP). And we will also test Intel NICs (iWARP/RoCEv2) later.

### Prerequisites {#nvmf_rdma_prereqs}
...
TCP transport is built in nvmf_tgt by default, and it does not need any special
libraries.
