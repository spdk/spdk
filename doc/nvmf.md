...
The SPDK NVMe over Fabrics target is a user space application that presents block devices over the
fabrics such as Ethernet, Infiniband, Fibre Channel, which are defined as different Transports in
NVMe-oF spec. Currently, SPDK supports RDMA and TCP transports.
...
It requires an RDMA-capable NIC with its corresponding OFED software package installed to run. The target
should work on all flavors of RDMA, but it is currently tested against Mellanox NICs (RoCEv2) and Chelsio
NICs (iWARP). And we will also test Intel NICs (iWARP/RoCEv2) later.

### Prerequisites {#nvmf_prereqs}

To build nvmf_tgt with RDMA transport, there are some additional dependencies.
...
Before starting our NVMe-oF target with RDMA trasnport we must load the InfiniBand and RDMA modules
...
TCP transport is built in nvmf_tgt by default, and it does not need any special libraries.
...
For example, you use rdma transport to discover, connect and disconnect. Usually,
if you want to use tcp transport, you can replace rdma with tcp.
