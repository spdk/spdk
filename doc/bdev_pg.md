...
calling spdk_bdev_open. This will return a descriptor. Write permission to a
block device is an exclusive operation, so the call to open the bdev with
write permissions will fail if any other descriptors exist. Multiple users may
open a bdev in read-only mode simultaneously, however.
...
descriptor so other memory may be freed.
...
spdk_bdev_io_get_scsi_status to obtain error information in the format of their choosing.

The I/O submission functions, such as spdk_bdev_read, may fail in one of two
ways. First, they may fail immediately and return an error code. In that case,
the provided callback will not be called. Second, they may fail
asynchronously. In that case, the associated spdk_bdev_io will be passed to
the callback and it will report error information.

Some I/O request types are optional and may not be universally supported. To
query a bdev for the I/O request types it supports, call
