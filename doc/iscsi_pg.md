...
1 Pin iSCSI connections to dedicated CPU core: All iSCSI connections of a target is handled by one
CPU core. Which CPU core is used for the target is determined automatically based on the simple load
balancing algorithm. The only case is connection migration for load balance purpose.
