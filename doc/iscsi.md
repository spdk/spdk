...
VPP configuration can be done using VPP startup file (placed in `/etc/vpp/startup.conf`
by default, or passed to vpp command with `-c` argument) and `vppctl` command.
...
In the VPP startup file (e.g. /etc/vpp/startup.conf) whitelist used interfaces
