...
A detailed instructions for **simplified steps 1-3** below, can be found on VPP [Quick Start Guide](https://wiki.fd.io/view/VPP).
...
## 3. Running VPP
VPP takes over any network interfaces that were bound to userspace driver, for details please see DPDK guide on [Binding and Unbinding Network Ports to/from the Kernel Modules](http://dpdk.org/doc/guides/linux_gsg/linux_drivers.html#binding-and-unbinding-network-ports-to-from-the-kernel-modules).

### Example: Tap interfaces on single host
For functional test purpose a tap interface with kernel can be created. This will allow network communication between SPDK iSCSI target using VPP end of tap and kernel iSCSI initiator using the kernel part of tap. A single host is used in this scenario.
...
## 5. Running SPDK
VPP application has to be started before SPDK, in order to enable usage of network interfaces.
