...
By default when deleting lvol bdev or resizing down, allocated clusters are unmaped. Optional --clear-method parameter can be passed on creation to change that behavior to writing zeroes or performing no operation.
...
```
construct_lvol_bdev [-h] [-u UUID] [-l LVS_NAME] [-t] [-c] lvol_name size
    ...
    optional arguments:
    -h, --help  show help
    -c, --clear-method Change default data clusters clear method.
get_bdevs [-h] [-b NAME]
