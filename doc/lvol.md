...
Thin provisioned lvols relies on on-demand cluster allocation (e.g. when when first write operation on a cluster is performed), what means that only space required to store data is used and unallocated are obtained from underlaying device (e.g. zeroes_dev).
