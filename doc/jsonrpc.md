...
# Remote access {#jsonrpc_remote}

SPDK provides a sample python script `remote_rpc.py`, that allows user to send JSON objects from remote system via management network. It uses HTTP POST method to receive JSON objects including methods and parameters described in this chapter.
...
`spdk/scripts/remote_rpc.py 192.168.0.2 8000 user password`
