# SPDK Block Compression Algorithm {#compress}

## Overview

This SPDK block compression scheme is based on using SSDs for storing compressed blocks of
storage and persistent memory for metadata.  This metadata includes mappings of logical blocks
requested by a user to the compressed blocks on SSD.
...
best NVMe performance.  These 4KB units are called "backing io units".  They are indexed from 0 to N
...
multiple of at least 2 4KB backing io units.  The number of backing io units per chunk determines
...
example, a 16KB chunk consumes 1, 2, 3 or 4 backing io units.  The number of blocks depends on how
...
logical map is a mapping of offsets into the compressed block device to the corresponding
chunk map.  Each entry in the logical map is a 64-bit value, denoting the associated chunk map.
A special value (for example, 2^64-1) is used if there is no associated chunk map.  At start,
all entries in the logical map have no associated chunk map.  Note that while access to
the backing storage device is in 4KB units, the logical view may allow 4KB or 512B unit access and
...
* The write is not a full chunk size, so we must allocate a 16KB buffer for source data.
...
Unmap operations on a full chunk are achieved by removing the chunk map entry (if any) from
...
Writes zeroes operations are handled similarly to unmap operations.  If a write zeroes
operation covers the entire chunk, we can remove the chunk's entry in the logical map
completely.  Then subsequent reads to that chunk will return all zeroes.

### Restart

When the compressed volume is restarted, the free chunk map list and free backing io unit list
...
until the backing units are actually needed.
