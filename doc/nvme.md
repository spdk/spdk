...
tool to for running some basic benchmarks. See the
[fio start up guide](https://github.com/spdk/spdk/blob/master/examples/nvme/fio_plugin/README.md)
for more details.
...
specification allows for thousands, but most devices support between 32 and
128. The specification makes no guarantees about the performance available from
...
pair to each thread. A further improvement would be to
[pin each thread to a separate CPU core](http://man7.org/linux/man-pages/man3/pthread_setaffinity_np.3.html),
and often the SPDK documentation will use "CPU core" and "thread"
interchangeably because we have this threading model in mind.
...
passing architecture, as opposing to a locking architecture, and will result in
