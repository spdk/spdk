...
for ((i = 0; i < NR_DEV; i++)); do
	if ((INIT == 1)); then
		...
		# dd if=/dev/zero of=/tmp/chunk${i}.qemu bs=32 count=$((NR_PU * NR_CHK))
		# dd if=/dev/zero of=/tmp/meta${i}.qemu bs=20 count=$((NR_PU * NR_CHK * NR_SECT))
		touch /tmp/chunk${i}.qemu /tmp/meta${i}.qemu
	fi

	OPT=(
		lchunktable=/tmp/chunk${i}.qemu
		lmetadata=/tmp/meta${i}.qemu
		lnum_pu=${NR_PU}
		ldebug=1
	)
	IFS=, eval 'LNVMOPT="${OPT[*]}"'
	NVME_OPT+="-drive file=/tmp/blknvme${i},if=none,id=nvme${i} "
	NVME_OPT+="-device nvme,drive=nvme${i},id=ocssd${i},serial=deadbeef${i},${LNVMOPT} "
done
echo ${NVME_OPT}
...
-virtfs local,path=/github,security_model=none,mount_tag=github -monitor stdio
