...
if [ "$1" = "vec_multi_dev" ]; then
	...
	for i in {1..1000}
	do
		MALLOC=`$rpc construct_malloc_bdev 128 8192`
	...
	MALLOC=`$rpc construct_malloc_bdev 128 512`
	GUID=`$rpc construct_lvol_store $MALLOC`
	BLOB=`$rpc construct_lvol_bdev $GUID 25`
	$rpc bdev_open $BLOB
	for i in {1..1000}
	...
	CHAR=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 1 | head -n 1)
	$rpc bdev_writev 1 1 $CHAR
	$rpc bdev_readv 1 1 $CHAR
...
elif [ "$1" = "vec_multi_dev_malloc" ]; then

	for i in {1..1000}
	do
		MALLOC=`$rpc construct_malloc_bdev 128 4096`
		$rpc bdev_open $MALLOC
		CHAR=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 1 | head -n 1)
		$rpc bdev_writev $((($i % 4)+1)) $((($i % 25)+1)) $CHAR
		$rpc bdev_readv $((($i % 4)+1)) $((($i % 25)+1)) $CHAR
		$rpc bdev_close $MALLOC
		$rpc delete_bdev $MALLOC
	done
	$rpc get_bdevs

elif [ "$1" = "vec_single_dev_malloc" ]; then
...
elif [ "$1" = "vec_simple_dev_malloc" ]; then
