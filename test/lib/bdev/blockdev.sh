...
if [ $SPDK_RUN_ASAN -eq 0 ]; then
	...
	if [ $RUN_NIGHTLY -eq 0 ]; then
		LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio $testdir/bdev.fio --iodepth=8 --bs=4k --runtime=10
	else
		LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio $testdir/bdev.fio --iodepth=128 --bs=192k --runtime=60
	fi
	timing_exit fio
fi
