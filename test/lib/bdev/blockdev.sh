...
if [ $(uname -s) = Linux ] && [ -f /usr/sbin/sgdisk ]; then
	...
	#sleep 5 seconds and make sure process is still running
	#Reason: nbd may exist via call spdk_app_stop in nbd_start function,
	# And we can not directly catch this, so sleep 5 seconds.
	sleep 5
