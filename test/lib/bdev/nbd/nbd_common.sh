...
function nbd_dd_data_verify() {
	...
	if [ "$operation" = "write" ]; then
		# data write
		dd if=/dev/urandom of=$tmp_file bs=4k count=256 oflag=direct
		for i in ${nbd_list[@]}; do
			dd if=$tmp_file of=$i bs=4k count=256
		done
...
function nbd_rpc_steps() {
	...
	local bdev_list=("Malloc2p0" "Malloc2p1" "Malloc2p2" \
