...
out_file_pmem=$testdir/spdkcli.test_pmem
out_file_details=$testdir/spdkcli_details.test

function on_error_exit() {
	set +e
	killprocess $spdk_tgt_pid
	rm -f $out_file $out_file_pmem $out_file_details /tmp/sample_aio /tmp/sample_pmem
	print_backtrace
	exit 1
}
...
function run_vhost_test() {
	...
	$SPDK_BUILD_DIR/test/app/match/match -v ${out_file}".match"
	$SPDK_BUILD_DIR/test/app/match/match -v ${out_file_details}".match"
...
function run_pmem_test() {
	...
	$SPDK_BUILD_DIR/scripts/spdkcli.py ll > $out_file_pmem
	$SPDK_BUILD_DIR/test/app/match/match -v ${out_file_pmem}".match"
...
rm -f $out_file $out_file_details /tmp/sample_aio $out_file_pmem
killprocess $spdk_tgt_pid
