...
function waitfortcplisten() {
	...
	echo "Waiting for process to start up and listen on UNIX domain socket $rpc_addr:$3..."
	...
	for (( i = 40; i != 0; i-- )); do
		...
		if $rootdir/scripts/rpc.py -t 1 -s "$rpc_addr" -p $3 rpc_get_methods &>/dev/null; then
			break
		fi
...
$rootdir/app/spdk_tgt/spdk_tgt -m 0x3 -p 0 -s 4096 -r $IP_ADDRESS:$PORT &
spdk_tgt_pid=$!
