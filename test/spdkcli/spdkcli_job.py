...
def load_bdevs():
    ...
    execute_command("/bdevs/malloc create 32 512 Malloc2", "Malloc2", True)
    execute_command("/bdevs/malloc create 32 4096 Malloc3", "Malloc3", True)
    execute_command("/bdevs/error create Malloc1", "EE_Malloc1", True)
    execute_command("/bdevs/null create null_bdev 32 512", "null_bdev", True)
    execute_command("/bdevs/aio create sample /tmp/sample_aio 512", "sample", True)
    nvme_disks = get_nvme_disks()
    nvme_params = nvme_disks[0]['params']
    cmd = "%s %s %s" % (nvme_params['name'], nvme_params['trtype'],
                        nvme_params['traddr'])
    execute_command("/bdevs/nvme create %s" % cmd, "%sn1" % nvme_params['name'], True)
    execute_command("/bdevs/split_disk split_bdev Nvme0n1 4", "Nvme0n1p0", True)


def load_lvols():
    execute_command("/lvol_stores create lvs Malloc0", "lvs", True)
    execute_command("/bdevs/logical_volume create lvol 16 lvs", "lvs/lvol", True)


def load_vhost():
    execute_command("vhost/block create vhost_blk1 Nvme0n1p0", "Nvme0n1p0", True)
    execute_command("vhost/block create vhost_blk2 Nvme0n1p1 0x2 readonly", "Nvme0n1p1", True)
    execute_command("vhost/scsi create vhost_scsi1", "vhost_scsi1", True)
    execute_command("vhost/scsi create vhost_scsi2", "vhost_scsi2", True)
    execute_command("vhost/scsi/vhost_scsi1 add_lun 0 Malloc2", "Malloc2", True)
    execute_command("vhost/scsi/vhost_scsi2 add_lun 0 Malloc3", "Malloc3", True)
    execute_command("vhost/scsi/vhost_scsi2 add_lun 1 Nvme0n1p2", "Nvme0n1p2", True)
    execute_command("vhost/scsi/vhost_scsi2 add_lun 2 Nvme0n1p3", "Nvme0n1p3", True)
...
def clear_spdk_tgt():
    ...
    nvme_disks = get_nvme_disks()
    nvme_params = nvme_disks[0]['params']
    execute_command("/bdevs/nvme delete %sn1" % nvme_params['name'],
                    "%sn1" % nvme_params['name'])
...
def load_spdk_tgt_pmem():
    execute_command("/bdevs/pmemblk create_pmem_pool /tmp/sample_pmem 32 512", "", True)
    execute_command("/bdevs/pmemblk create/tmp/sample_pmem pmem_bdev", "pmem_bdev", True)
...
def load_spdk_tgt_virtio():
    ...
    pci_blk = get_blk_pci()
    if pci_blk:
        execute_command("/bdevs/virtioblk_disk create virtioblk_pci pci 0000:00:07.0", True)
    pci_scsi = get_scsi_pci()
    if pci_scsi:
        execute_command("/bdevs/virtioscsi_disk create virtioscsi_pci pci 0000:00:06.0", True)
    execute_command("/vhost/scsi create sample_scsi", "sample_scsi", True)
    execute_command("/vhost/scsi/sample_scsi add_lun 0 Malloc0", True)
