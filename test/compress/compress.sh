...
function destroy_vols() {
	# gracefully cleanup the vols, deleting the compression vol also deletes the pmem file
...
function create_vols() {
	...
	# use QAT for lv0, by default the vbdev module will use QAT
	# if available and ISAL if not
	compress_bdev1=$($rpc_py construct_compress_bdev -b lvs0/lv0 -p /tmp)
	# force the use of ISAL for lv1 even if QAT is available
