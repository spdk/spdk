...
function compress_cleanup() {
	...
	bdev_svc_pid=$!
	trap "echo TRAP; killprocess $bdev_svc_pid; exit 1" SIGINT SIGTERM
	...
	# we don't want a timeout to kick us out
	set +e
	waitforbdev COMP_lvs0/lv0
	set -e
	bdevs_names=$($rpc_py get_bdevs | jq -r '.[] .name')
        if echo $bdevs_names | grep -qi COMP_lvs0/lv0; then
		$rpc_py delete_compress_bdev COMP_lvs0/lv0
        fi
        if echo $bdevs_names | grep -qi lvs0/lv0; then
		$rpc_py destroy_lvol_store -l lvs0
        fi
	killprocess $bdev_svc_pid
	# in case there was an issue with delete_compress_bdev(), remove pmem file
	rm -rf /tmp/pmem
}
...
$rootdir/test/bdev/bdevperf/bdevperf -z -q $qd  -o $iosize -w verify -t $runtime &bdevperf_pid=$!
