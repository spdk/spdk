...
function replace_defined_variables() {
	...
	for dep in "${bad_values[@]}"; do
		IFS=" " read -r -a dep_def_arr <<< $(cat $libdeps_file | grep -v "#" | grep "${dep}" | cut -d "=" -f 2 | xargs)
		IFS=" " read -r -a new_values <<< $(replace_defined_variables "${dep_def_arr[@]}")
...
function confirm_deps() {
	...
	IFS=" " read -r -a lib_make_deps <<< $(cat $libdeps_file | grep "DEPDIRS-${lib_shortname}" | cut -d "=" -f 2 | xargs)
	IFS=" " read -r -a lib_make_deps <<< $(replace_defined_variables "${lib_make_deps[@]}")
