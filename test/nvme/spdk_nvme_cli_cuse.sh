...
for ns in $(ls /dev/${nvme_name}n1); do
	${NVME_CMD} get-ns-id $ns > ${KERNEL_OUT}.1
	${NVME_CMD} id-ns $ns > ${KERNEL_OUT}.2
	${NVME_CMD} list-ns $ns > ${KERNEL_OUT}.3 || true
done

for ctrlr in $(ls /dev/${nvme_name}); do
...
$rpc_py bdev_nvme_cuse_register -n Nvme0 -p spdk/${nvme_name}
...
for ns in $(ls /dev/spdk/${nvme_name}n1); do
	${NVME_CMD} get-ns-id $ns > ${CUSE_OUT}.1
	${NVME_CMD} id-ns $ns > ${CUSE_OUT}.2
	${NVME_CMD} list-ns $ns > ${CUSE_OUT}.3 || true
done

for ctrlr in $(ls /dev/spdk/${nvme_name}); do
