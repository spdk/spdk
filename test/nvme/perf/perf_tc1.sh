...
#Test case 1: Measure maximum performance in IOPS/CORE on 3 workloads:
	...
	do
		...
		do
			...
			run_spdk_nvme_fio $PLUGIN --filename="$filename" "--runtime=$RUNTIME" "--ramp_time=$RAMP_TIME" "--bs=$BLK_SIZE"\
			 "--rw=$RW" "--rwmixread=${MIX[$i]}" "--iodepth=$qd" "--cpumask=$CPUMASK" "--output=$NVME_FIO_RESULTS" "--time_based=1"
