...
do
	...
	do
		...
		mean_slat_disks[$k]=$((${mean_slat_disks[$k]} + $(get_results mean_slat $MIX)))
		mean_clat_disks[$k]=$((${mean_clat_disks[$k]} + $(get_results mean_clat $MIX)))
...
do
	...
	mean_lat_disks[$k]=$((${mean_lat_disks[$k]} / $REPEAT_NO))
	p99_lat_disks[$k]=$((${p99_lat_disks[$k]} / $REPEAT_NO))
	p99_99_lat_disks[$k]=$((${p99_99_lat_disks[$k]} / $REPEAT_NO))
	stdev_disks[$k]=$((${stdev_disks[$k]} / $REPEAT_NO))
	mean_slat_disks[$k]=$((${mean_slat_disks[$k]} / $REPEAT_NO))
	mean_clat_disks[$k]=$((${mean_clat_disks[$k]} / $REPEAT_NO))

	printf "%s,%s,%s,%s,%s,%s,%s,%s\n" ${k} ${iops_disks[$k]} ${mean_lat_disks[$k]} ${p99_lat_disks[$k]}\
	 ${p99_99_lat_disks[$k]} ${stdev_disks[$k]} ${mean_slat_disks[$k]} ${mean_clat_disks[$k]} >> $result_file
