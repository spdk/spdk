...
result_dir=perf_results_${MIX}_${DRIVER}_${no_cores}cpus_${date}
mkdir -p $BASE_DIR/results/$result_dir
result_file=$BASE_DIR/results/$result_dir/perf_results_${MIX}_${DRIVER}_${no_cores}cpus_${date}.csv
...
do
	#Start with $DISKNO disks and remove 2 disks for each run to avoid preconditioning before each runs
	...
	do
		...
		if [ $DRIVER = "nvme" ] || [ $DRIVER = "bdev" ]; then
			...
			"--write_lat_log=$BASE_DIR/results/$result_dir/perf_lat_${MIX}_${DRIVER}_${no_cores}cpus_${date}_${k}_disks_${j}"
		else
