...
for (( i=0; i < 3; i++ ))
do
	#don't do preconditioning after randread
	if $PRECONDITIONING && [ "$i" -ne "1" ]; then
		preconditioning
	fi
			...
			if [ "$max_iops" = "0" ]; then
				...
				do
					...
					run_spdk_nvme_fio $PLUGIN "--filename=$filename" "--runtime=$RUNTIME" "--ramp_time=$RAMP_TIME" "--bs=$BLK_SIZE"\
					 "--rw=$RW" "--rwmixread=${MIX[$i]}" "--iodepth=$qd" "--cpus_allowed=$cpus_allowed" "--output=$NVME_FIO_RESULTS"\
					 "--time_based=1"
