...
KERNEL_ENGINES=( ["kernel-libaio"]="--ioengine=libaio"
...
function get_numa_node(){
	...
	elif [ "$plugin" = "spdk-plugin-bdev" ] || [ "$plugin" = "spdk-perf-bdev" ]; then
		local bdevs
...
function usage()
{
	...
	echo "[DELETE] --max-disk=INT,ALL    Number of disks to test on, this will run multiple workloads with increasing number of disk each run, if =ALL then test on all found disk. [default=$DISKNO]"
	set -x
}
