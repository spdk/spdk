...
function create_fio_config(){
	...
	local cores_numa=$(get_cores_numa_node $cores)
	...
	if [ "$plugin" != "nvme" ] && [ "$plugin" != "bdev" ]; then
		...
		do
			...
			cpu_used=""
				...
				do
					core_numa=$(lscpu -p=cpu,node | grep "^${cores[$j]}\b" | awk -F ',' '{print $2}')
		...
		do
			core_numa=$(lscpu -p=cpu,node | grep "^${cores[$i]}\b" | awk -F ',' '{print $2}')
...
function usage()
{
	...
	echo "                          'kernel-libaio', 'kernel-classic-polling' or 'kernel-hybrid-polling' for kernel driver. [default=$DRIVER]"
