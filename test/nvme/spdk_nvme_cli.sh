...
if [ $SPDK_TEST_NVME_CUSE -eq 1 ]; then
	...
	for bdf in $bdfs; do
		$rpc_py bdev_nvme_attach_controller -b Nvme${i} -t PCIe -a ${bdf} --cuse
		i=$(( $i + 1 ))
	done

	for bdf in $(ls /dev/spdk/nvme?); do
		${NVME_CMD} list
		${NVME_CMD} id-ctrl $bdf
		${NVME_CMD} list-ctrl $bdf
		${NVME_CMD} get-ns-id $bdf
		${NVME_CMD} id-ns $bdf -n 1
		${NVME_CMD} fw-log $bdf
		${NVME_CMD} smart-log $bdf
		${NVME_CMD} error-log $bdf
		${NVME_CMD} list-ns $bdf -n 1
		${NVME_CMD} get-feature $bdf -f 1 -s 1 -l 100
		${NVME_CMD} get-log $bdf -i 1 -l 100
		${NVME_CMD} reset $bdf
	done
