...
KERNEL_SMART_ERRLOG=$( ${SMARTCTL_CMD} -l error /dev/nvme0 )
...
# Data integity was changed before, now make sure other commads didn't fail
${SMARTCTL_CMD} -i /dev/spdk/nvme0n1
${SMARTCTL_CMD} -c /dev/spdk/nvme0
${SMARTCTL_CMD} -A /dev/spdk/nvme0
${SMARTCTL_CMD} -t select,10-20 /dev/spdk/nvme0n1
${SMARTCTL_CMD} -t select,10+11 /dev/spdk/nvme0n1
