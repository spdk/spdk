...
function opal_init() {
	...
	# ignore bdev_nvme_opal_init failure
	set +e
...
function setup_test_environment() {
	bdf1=$($rootdir/scripts/gen_nvme.sh --json | jq -r '.config[].params | select(.name=="Nvme0").traddr')
	$rpc_py bdev_nvme_attach_controller -b "nvme0" -t "pcie" -a $bdf1
	$rpc_py bdev_opal_create -b nvme0 -i 1 -s 0 -l 1024 -p test
	$rpc_py bdev_opal_create -b nvme0 -i 2 -s 1024 -l 512 -p test
	...
	$rpc_py bdev_opal_create -b nvme0 -i 3 -s 4096 -l 4096 -p test
	$rpc_py bdev_opal_create -b nvme0 -i 1 -s 0 -l 1024 -p test
	$rpc_py bdev_opal_create -b nvme0 -i 2 -s 1024 -l 512 -p test
}
