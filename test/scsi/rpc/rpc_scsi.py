...
def verify_get_bdevs_methods_null(rpc_py):
    ...
    verify(not jsonvalue, "get_bdevs returned:\n {}, \n expected empty".format(jsonvalue))
...
def verify_construct_malloc_bdevs(rpc_py, rpc_param):
    ...
    verify(re.search('^Malloc[0-9]*', output), "construct_malloc_bdev returned {}, expected Malloc_".format(output))
...
def verify_get_bdevs_methods(rpc_py, rpc_param):
    ...
    output = rpc.get_bdevs()
    jsonvalue = json.loads(output)
    verify([str(jsonvalue[0]['block_size']) in str(rpc_param['malloc_block_size']) and jsonvalue[0]['claimed'] is
            rpc_param['claimed']], "get_bdevs returned not correct values")
...
def verify_construct_vhost_scsi_controller(rpc_py, rpc_param):
    ...
    verify(not output, "construct_vhost_scsi_controller returned:\n {}, \n expected empty".format(output))
...
def verify_get_vhost_scsi_controllers(rpc_py, rpc_param, malloc):
    ...
    verify(jsonvalue[0]['ctrlr'] in rpc_param['ctrlr'], "get_vhost_scsi_controllers cpu_mask parameter is "
                                                        "different for value {}".format(rpc_param['ctrlr']))

    if jsonvalue[0]['scsi_devs']:
        verify(jsonvalue[0]['scsi_devs'][0]['luns'][0]['name'] in malloc, "get_vhost_scsi_controllers "
                                                                          "scsi_devs parameter is different "
                                                                          "for value {}".format(malloc))
    else:
        verify(str(jsonvalue[0]['scsi_devs']) in rpc_param['scsi_devs'], "get_vhost_scsi_controllers "
                                                                         "scsi_devs parameter is different "
                                                                         "for value {}".format(rpc_param['scsi_devs']))

    verify(jsonvalue[0]['cpu_mask'] in rpc_param['cpu_mask'], "get_vhost_scsi_controllers cpu_mask parameter is "
...
def verify_remove_vhost_scsi_dev(rpc_py, rpc_param):
    ...
    verify(not output, "remove_vhost_scsi_dev returned:\n {}, \n expected empty".format(output))
...
def verify_delete_bdev(rpc_py, malloc):
    ...
    verify(not output, "delete_bdev returned:\n {}, \n expected empty".format(output))
