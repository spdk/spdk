#!/usr/bin/env python
import sys
from test_cases import *


def check_fail_count(fail_count, num_test):
    if not fail_count:
        print("Test: {num_test} - PASS".format(num_test=num_test))
    else:
        print("Test: {num_test} - FAIL".format(num_test=num_test))

def lvol_test(tc, tc_list):
    tc_failed = []
    if "all" in tc_list:
        for num_test in range(1, test_counter() + 1):
            fail_count = 0
            exec("fail_count += tc.test_case{num_test}"
                 "()".format(num_test=num_test))
            check_fail_count(fail_count, num_test)
            if fail_count:
                tc_failed.append(num_test)

    else:
        for num_test in tc_list:
            fail_count = 0
            exec("fail_count += tc.test_case{num_test}"
                 "()".format(num_test=num_test))
            check_fail_count(fail_count, num_test)
            if fail_count:
                tc_failed.append(num_test)
    return tc_failed

if __name__ == "__main__":
    app_path = None
    base_dir_path = None
    block_size = None
    cluster_size = None
    num_test = None
    rpc_py = None
    tc = None
    tc_failed = None
    total_size = None
    tc_list = []

    tasting = sys.argv[1]
    if (len(sys.argv) <= 9 and tasting in ["yes"]) or \
            (len(sys.argv) <= 8 and tasting in ["no"]):
        rpc_py = sys.argv[2]
        total_size = int(sys.argv[3])
        block_size = int(sys.argv[4])
        cluster_size = int(sys.argv[5])
        base_dir_path = sys.argv[6]
        if tasting in ["yes"]:
            app_path = sys.argv[7]
            tc_list = sys.argv[8].split(',')
            tc = TastingTestCases(rpc_py, total_size, block_size,
                                  cluster_size, base_dir_path, app_path)
        else:
            tc_list = sys.argv[7].split(',')
            tc = TestCases(rpc_py, total_size, block_size, cluster_size,
                           base_dir_path)
    else:
        print("Invalid argument")
        sys.exit(1)

    if len(tc_list) > tc.test_counter():
        print("Invalid argument: Too many test cases")
        sys.exit(1)

    try:
        tc_failed = lvol_test(tc, tc_list)
        if not tc_failed:
            print("RESULT: All test cases - PASS")
        elif tc_failed:
            print("RESULT: Some test cases FAIL")
            print(tc_failed)
            sys.exit(1)
    except:
        print("Test: {num_test} - FAIL".format(num_test=tc_failed))
        sys.exit(1)
