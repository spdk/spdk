...
function lvol_backwards_compatibility() {
	...
	run_fio_test /dev/nbd0 $(( lvol_size / 2 )) $(( lvol_size / 2 )) "write" "0xcc"
	...
	nbd_start_disks "$DEFAULT_RPC_ADDR" "$lvol_uuid2" /dev/nbd0
	run_fio_test /dev/nbd0 $(( lvol_size / 2 )) $(( lvol_size / 2 )) "read" "0xcc"
	run_fio_test /dev/nbd0 0 $(( lvol_size / 2 )) "read" "0x00"
	nbd_stop_disks "$DEFAULT_RPC_ADDR" /dev/nbd0
...
# create lvs + multiple lvols, verify their params
function lvol_forwards_compatibility() {
	$new_spdk &
	spdk_pid=$!
	trap 'killprocess "$spdk_pid"; rm -f $testdir/aio_bdev; exit 1' SIGINT SIGTERM EXIT
	waitforlisten $spdk_pid
	...
	# kill SPDK 20.01 and start SPDK v19.10.1
	killprocess $spdk_pid
	$old_spdk &
	spdk_pid=$!
	waitforlisten $spdk_pid
	...
	# due to higher version in new blobstore, the lvs and lvols should not be loaded
	lvols=$(rpc_cmd bdev_get_bdevs | jq -r '[ .[] | select(.product_name == "Logical Volume") ]')
	[ "$(jq length <<< "$lvols")" == "0" ]
	lvs=$(rpc_cmd bdev_lvol_get_lvstores)
	[ "$(jq length <<< "$lvs")" == "0" ]

	# kill SPDK v19.10.1 and start pre-release SPDK 20.01
	killprocess $spdk_pid
	$new_spdk &
	spdk_pid=$!
	waitforlisten $spdk_pid
...
run_test "lvol_forwards_compatibility" lvol_forwards_compatibility
