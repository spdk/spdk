...
def case_message(func):
    def inner(*args, **kwargs):
        test_name = {
            ...
            # nested destroy_lvol_bdev - positive tests
            350: 'nested_destroy_logical_volume_positive',
...
class TestCases(object):
    ...
    def test_case350(self):
        """
        nested_destroy_logical_volume_positive

        Positive test for destroying lvol bdev with nested lvol store.
        All destroy_lvol_bdev with correct base lvol bdev name.
        """
        # Create malloc bdev
        base_name = self.c.construct_malloc_bdev(self.total_size,
                                                 self.block_size)
        ...
        uuid_bdev_nested = self.c.construct_lvol_bdev(nested_lvs_uuid,
                                                      nested_lbd_name, nested_size)
        lvol_bdev = self.c.check_get_bdevs_methods(uuid_bdev_nested, nested_size)
    ...
    def test_case400(self):
        ...
        lvol_bdev = self.c.check_get_bdevs_methods(nested_uuid_bdev2, nested_size2)

        self.c.destroy_lvol_bdev(nested_uuid_bdev2)
        self.c.destroy_lvol_store(nested_lvs_uuid2)
        self.c.destroy_lvol_bdev(nested_uuid_bdev)
        self.c.destroy_lvol_store(nested_lvs_uuid)
        self.c.destroy_lvol_bdev(uuid_bdev)
        self.c.destroy_lvol_store(lvs_uuid)
        self.c.delete_malloc_bdev(base_name)
