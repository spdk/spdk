...
class TestCases(object):
    ...
    def test_case350(self):
        ...
        fail_count += self.c.check_get_bdevs_methods(uuid_bdev_nested, nested_size)

        if self.c.destroy_lvol_bdev(uuid_bdev) == 1:
            fail_count += 1
        fail_count += self.c.delete_malloc_bdev(base_name)
    ...
    def test_case653(self):
        ...
        fail_count += self.run_fio_test(self.lbd_name, 0, size*MEGABYTE, "write", "0xcc", 0)
        # Save number of free clusters for lvs
        lvs = self.c.get_lvol_stores()[0]
        free_clusters_start = int(lvs['free_clusters'])
        # Resize bdev to full size of lvs
        full_size = self.get_lvs_size()
        ...
        fail_count += self.run_fio_test(self.lbd_name, int(lbd_size * MEGABYTE / 2),
        ...
        if free_clusters_start != free_clusters_resize2:
            fail_count += 1
