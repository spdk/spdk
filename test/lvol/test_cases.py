...
class TestCases(object):
    ...
    def test_case762(self):
        ...
        # Fill first half of lvol bdev
        first_part = int(bdev_size * MEGABYTE / 3)
        lvol_bdev = self.c.get_lvol_bdev_with_name(bdev_name)
        fail_count += self.run_fio_test(nbd_name0, 0, first_part-1, "write", "0xee")

        # Check if snapshot was unchanged
        fail_count += self.c.start_nbd_disk(snapshot_bdev['name'], nbd_name1)
        fail_count += self.run_fio_test(nbd_name1, 0, first_part-1, "read", "0xcc")
        ...
        fail_count += self.run_fio_test(nbd_name2, 0, first_part-1, "read", "0xee")

        # Verify lvol bdev
        fail_count += self.run_fio_test(nbd_name0, 0, first_part-1, "read", "0xee")
        lvol_bdev = self.c.get_lvol_bdev_with_name(bdev_name)
        if lvol_bdev['driver_specific']['lvol']['clone'] is not True:
            fail_count += 1
        ...
        # Fill second one third of lvol bdev
        second_part =  int(bdev_size * MEGABYTE / 3 * 2)
        fail_count += self.run_fio_test(nbd_name0, first_part, second_part-1, "write", "0xdd")

        # Verify snapshots
        fail_count += self.run_fio_test(nbd_name1, first_part, second_part-1, "read", "0xcc")
        fail_count += self.run_fio_test(nbd_name2, first_part, second_part-1, "read", "0xcc")
        ...
        # Check data consistency
        fail_count += self.run_fio_test(nbd_name1, 0, second_part-1, "read", "0xcc")
        fail_count += self.c.stop_nbd_disk(nbd_name1)
        snapshot_bdev = self.c.get_lvol_bdev_with_name(self.lvs_name + "/" + snapshot_name)
        lvol_bdev = self.c.get_lvol_bdev_with_name(bdev_name)
        if lvol_bdev['driver_specific']['lvol']['clone'] is not True:
        ...
        if self.lbd_name not in snapshot_bdev['driver_specific']['lvol']['clones']:
            fail_count += 1
        fail_count += self.run_fio_test(nbd_name0, 0, first_part-1, "read", "0xee")
        fail_count += self.run_fio_test(nbd_name0, first_part, second_part-1, "read", "0xdd")
        fail_count += self.c.stop_nbd_disk(nbd_name0)
