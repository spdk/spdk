...
class TestCases(object):
    ...
    def test_case751(self):
        ...
        lbd_name0 = self.lbd_name + str(0)
        lbd_name1 = self.lbd_name + str(1)
        # Create thin provisioned lvol bdev and simple lvol bdev
        ...
        # Fill thin provisoned lvol bdev
    ...
    def test_case752(self):
        ...
        thread.start()
        time.sleep(2)
        # During fio create snapshot
        fail_count += self.c.snapshot_lvol_bdev(lvol_bdev['name'], snapshot_name)
        output = subprocess.check_output("ps ax", stderr=subprocess.STDOUT, shell=True)
        print ("output: %s" % output)
        print ("thread isalive: %s" % thread.is_alive())
        fio_cmd = "[" + thread.fio_cmd[0] + "]" + thread.fio_cmd[1:]
        print ("ps ax | grep '%s'" % fio_cmd)
        try:
            output = subprocess.check_output("ps ax | grep '%s'" % fio_cmd, stderr=subprocess.STDOUT, shell=True)
            # output = os.system("ps ax | grep '%s'" % fio_cmd)
            print ("Running fio command: %s" % output)
        except Exception as e:
            fail_count += 1
        thread.join()
        print ("thread.rv: %s" % thread.rv)
    ...
    def test_case753(self):
        ...
        # Create snapshot of snapshot and check if it fails
    ...
    def test_case755(self):
        ...
        # Compare snapshot with clone1. Data should be the same
        time.sleep(1)
