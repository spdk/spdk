...
class FioThread(threading.Thread):
    def __init__(self, nbd_disk, offset, size, rw, pattern):
...
class TestCases(object):
    ...
    def test_case750(self):
        ...
        fail_count = self.run_fio_test("/dev/nbd0", 0, size, "write", "0xcc", 0)
        snapshot_name = "snapshot0"
        fail_count + self.c.snapshot_lvol_bdev(lvol_bdev['name'], snapshot_name)
        bdevs = self.get_lvol_bdevs()
        for bdev in bdevs:
            if bdev['name'] == snapshot_name:
                if bdev['readonly'] != 'true':
                    fail_count += 1

        snapshot_bdev = self.c.get_lvol_bdev_with_name(snapshot_name)
        nbd_name1 = "/dev/nbd1"
        rc = self.c.start_nbd_disk(snapshot_bdev['name'], nbd_name1)
    ...
    def test_case751(self):
        ...
        nbd_name0 = "/dev/nbd0"
        self.c.start_nbd_disk(lvol_bdev['name'], nbd_name0)
        ...
        nbd_name1 = "/dev/nbd1"
        self.c.start_nbd_disk(lvol_bdev['name'], nbd_name1)
        ...
        nbd_name2 = "/dev/nbd2"
        self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name0, nbd_name2)
        nbd_name3 = "/dev/nbd3"
        self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name1, nbd_name3)
        rv = self.compare_two_disks(nbd_name0, nbd_name2)
        if rv == 1:
            print("ERROR: Cmp ended with failure")
            fail_count += 1
        rv = self.compare_two_disks(nbd_name1, nbd_name3)
        if rv == 1:
            print("ERROR: Cmp ended with failure")
            fail_count += 1
        ...
        fill_size = int(size * MEGABYTE / 2)
        offset = fill_size
        fail_count = self.run_fio_test(nbd_name0, offset, fill_size, "write", "0xcc", 0)
        rv = self.compare_two_disks(nbd_name0, nbd_name2)
        if rv = 0:
            print("ERROR: Cmp ended with success, but shouldn't")
            fail_count += 1
    ...
    def test_case753(self):
        ...
        fail_count += self.c.snapshot_lvol_bdev(lvol_bdev['name'], snapshot_name)
        snapshot_name1 = "snapshot1"
        if self.c.snapshot_lvol_bdev(snapshot_name, snapshot_name1) == 0:
            print("ERROR: Creating snapshot of snapshot should failed")
            fail_count += 1
        fail_count += self.c.delete_bdev(lvol_bdev['name'])
        fail_count += self.c.delete_bdev(snapshot_name0)
        fail_count += self.c.destroy_lvol_store(uuid_store)
        fail_count += self.c.delete_bdev(base_name)
        footer(753)
        return fail_count
    ...
    def test_case755(self):
        ...
        snapshot_name = "snapshot"
        fail_count += self.c.construct_lvol_snapshot(lvol_bdev['name'], snapshot_name)
