...
class TestCases(object):
    ...
    def test_case757(self):
        ...
        fail_count += self.c.check_get_lvol_stores(base_name, uuid_store,
                                                   self.cluster_size)
        lvs = self.c.get_lvol_stores()
        size = int(int(lvs[0][u'free_clusters'] * lvs[0]['cluster_size']) / 3 / MEGABYTE)
        ...
        if lvol_clone['driver_specific']['lvol']['thin_provision'] != True:
            fail_count +=1
        ...
        fail_count += self.run_fio_test(nbd_name, 0, fill_size, "write", "0xdd", 0)

        # Do inflate
        fail_count += self.c.inflate_lvol_bdev(lvol_clone['name'])
        lvol_clone = self.c.get_lvol_bdev_with_name(self.lvs_name + "/" + self.lbd_name)
        if lvol_clone['driver_specific']['lvol']['thin_provision'] != False:
            fail_count +=1
        ...
        fail_count += self.run_fio_test(nbd_name, 0, fill_size, "read", "0xdd")
        fail_count += self.run_fio_test(nbd_name, fill_size, size * MEGABYTE,
                                        "read", "0xcc")
        # Destroy lvol bdev
        fail_count += self.c.delete_bdev(lvol_bdev['name'])
