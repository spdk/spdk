...
class TestCases(object):
    ...
    def test_case762(self):
        ...
        fail_count += self.run_fio_test(nbd_name2, 0, half_size-1, "read", "0xee")
        ...
        # Fill second half of lvol bdev
        fail_count += self.run_fio_test(nbd_name0, half_size, size-1, "write", "0xdd")

        # Verify snapshots
        fail_count += self.run_fio_test(nbd_name1, half_size, size-1, "read", "0xcc")
        fail_count += self.run_fio_test(nbd_name2, half_size, size-1, "read", "0xcc")
        fail_count += self.c.stop_nbd_disk(nbd_name2)

        # Delete snapshot - should succees
        if snapshot_bdev2['driver_specific']['lvol']['clone'] is not False\
            and snapshot_bdev2['driver_specific']['lvol']['snapshot'] is not True:
            fail_count += 1
        fail_count += self.c.destroy_lvol_bdev(snapshot_bdev2['name'])
        ...
        fail_count += self.run_fio_test(nbd_name1, 0, half_size-1, "read", "0xcc")
        fail_count += self.c.stop_nbd_disk(nbd_name1)
        lvol_bdev = self.c.get_lvol_bdev_with_name(self.lvs_name + "/" + snapshot_name)
        lvol_bdev = self.c.get_lvol_bdev_with_name(bdev_name)
        if lvol_bdev['driver_specific']['lvol']['clone'] is not True:
           fail_count += 1
        fail_count += self.run_fio_test(nbd_name0, 0, half_size-1, "read", "0xee")
