...
class TestCases(object):
    ...
    def test_case850(self):
        ...
        # Check if data on both lvol bdevs remained unchanged.
        # If deleting lvol bdev and lvol store with clear_method=none
        # data on lvol bdev should be untouched
        fail_count += self.c.nbd_start_disk(base_name, nbd_name)
        offset = lvs['cluster_size']
        fail_count += self.run_fio_test("/dev/nbd0", offset, lvs['cluster_size'] * 2, "read", "0xdd")
        fail_count += self.c.nbd_stop_disk(nbd_name)
