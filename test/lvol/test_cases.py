...
class FioThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.rv = 1
        if self.expected_ret_value == self.rv:
            return 0
...
class TestCases(object):
    ...
    def compare_two_disks(self, disk1, disk2):
        cmp_cmd = "cmp %s %s -n 30M" % (disk1, disk2)
        try:
            print "cmp_cmd %s" % cmp_cmd
    ...
    def test_case750(self):
        ...
        #if snapshot_bdev['readonly'] != 'true':
        #    fail_count += 1
    ...
    def test_case751(self):
        ...
        nbd_name1 = "/dev/nbd1"
        fail_count += self.c.start_nbd_disk(lvol_bdev['name'], nbd_name1)
        ...
        nbd_name2 = "/dev/nbd2"
        fail_count += self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name0, nbd_name2)
        nbd_name3 = "/dev/nbd3"
        fail_count += self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name1, nbd_name3)
        print "compare two disks"
        ...
        fail_count += self.c.stop_nbd_disk(nbd_name3)
    ...
    def test_case755(self):
        ...
        fail_count += self.run_fio_test(nbd_name1, 0, fill_size, "write", "0xcc", 0)
        nbd_name2 = "/dev/nbd2"
        fail_count += self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name, nbd_name1)
        nbd_name3 = "/dev/nbd3"
        lvol_clone1 = self.c.get_lvol_bdev_with_name("clone1")
        fail_count += self.c.start_nbd_disk(lvol_clone1['name'], nbd_name3)
