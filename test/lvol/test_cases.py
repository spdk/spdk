...
class TestCases(object):
    ...
    def test_case751(self):
        ...
        fail_count += self.compare_two_disks(nbd_name0, nbd_name2, 0)
        fail_count += self.compare_two_disks(nbd_name1, nbd_name3, 0)

        fill_size = int(size * MEGABYTE / 2)
        offset = fill_size
        fail_count += self.run_fio_test(nbd_name0, offset, fill_size, "write", "0xcc", 0)
        fail_count += self.compare_two_disks(nbd_name0, nbd_name2, 1)
        fail_count += self.c.stop_nbd_disk(nbd_name0)
        fail_count += self.c.stop_nbd_disk(nbd_name1)
    ...
    def test_case755(self):
        ...
        fail_count += self.compare_two_disks(nbd_name2, nbd_name3, 0)
