...
class TestCases(object):
    ...
    def test_case650(self):
        ...
        nbd_name = "/dev/nbd0"
        rc = self.c.start_nbd_disk(bdev_name, nbd_name)
        ...
        print "bdev %s" % lvol_bdev
        offset = int((int(lvol_bdev['num_blocks']) * int(lvol_bdev['block_size']) /
                      free_clusters_create_lvol) * 1.5)
        rv = self.run_fio_test(nbd_name, offset, size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with unexpected failure")
            fail_count += 1
        lvs = self.c.get_lvol_stores(self.lvs_name)[0]
    ...
    def test_case651(self):
        ...
        rv = self.run_fio_test(nbd_name0, 0, size, "write", False)
        if rv == 1:
            print("Fio test ended with unexpected failure")
            fail_count += 1

        size = bdev_size * MEGABYTE
        rv = self.run_fio_test(nbd_name1, 0, size, "read", "0x00")
        if rv == 1:
            print("Fio test ended with unexpected failure")
            fail_count += 1
    ...
    def test_case652(self):
        ...
        size = bdev_size * MEGABYTE
        rv = self.run_fio_test(nbd_name, 0, size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with unexpected failure")
            fail_count += 1
    ...
    def test_case654(self):
        ...
        size = "75%"
        rv = self.run_fio_test(nbd_name0, 0, size, "write", "0xcc")
        if rv == 1:
            print("ERROR: Fio test ended with unexpected failure")
            fail_count += 1

        size = "75%"
        rv = self.run_fio_test(nbd_name1, 0, size, "write", "0xee")
        if rv == 0:
            print("ERROR: Fio test ended with success but shouldn't")
            fail_count += 1

        size = "75%"
        rv = self.run_fio_test(nbd_name0, 0, size, "read", "0xcc")
        if rv == 1:
            print("ERROR: Fio test ended with unexpected failure")
            fail_count += 1
        ...
        size = "25%"
        offset = "75%"
        rv = self.run_fio_test(nbd_name0, offset, size, "read", "0x00")
        if rv == 1:
            print("ERROR: Fio test ended with unexpected failure")
            fail_count += 1
    ...
    def test_case655(self):
        ...
        rv = self.run_fio_test(nbd_name1, 0, size, "write", "0xee")
        if rv == 1:
            print("Fio test ended with enexpected failure")
            fail_count += 1
