...
class TestCases(object):
    ...
    def test_case650(self):
        ...
        size = int(int(lvol_bdev['num_blocks']) * int(lvol_bdev['block_size']) /
        ...
        self.sed_file("filename=", "filename=/dev/nbd0", path.join(self.path, 'fio_job.tmp'))
        self.sed_file("offset=", "offset=%s" % offset, path.join(self.path, 'fio_job.tmp'))
        self.sed_file("size=", "size=%s" % size, path.join(self.path, 'fio_job.tmp'))
        rv = self.run_fio_test("fill_100_percent_with_offset")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        lvs = self.c.get_lvol_store_with_name(self.lvs_name)
        ...
        self.prepare_fio_job(sections)
        size = int(int(lvol_bdev['num_blocks']) * int(lvol_bdev['block_size']) /
                   free_clusters_create_lvol * (free_clusters_create_lvol - 3))
        offset = int(int(lvol_bdev['num_blocks']) * int(lvol_bdev['block_size']) /
    ...
    def test_case653(self):
        ...
        lvs = self.c.get_lvol_store_with_name(self.lvs_name)
        free_clusters_start = int(lvs['free_clusters'])
        lvol_size = int(free_clusters_start / 2)
        bdev_name = self.c.construct_lvol_bdev(uuid_store, self.lbd_name,
                                               lvol_size, thin=True)
        ...
        self.sed_file("filename=", "filename=/dev/nbd0", path.join(self.path, 'fio_job.tmp'))
        self.sed_file("size=", "size=%s" % size, path.join(self.path, 'fio_job.tmp'))
        rv = self.run_fio_test("fill_100_percent_with_verify")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        rc = self.c.stop_nbd_disk(nbd_name)
    ...
    def test_case654(self):
        ...
        sections = ""
        self.prepare_fio_job(sections)
        size = int(int(lvol_bdev0['num_blocks']) * int(lvol_bdev0['block_size']) * 0.75)
        self.sed_file("filename=", "filename=/dev/nbd0", path.join(self.path, 'fio_job.tmp'))
        self.sed_file("size=", "size=%s" % size, path.join(self.path, 'fio_job.tmp'))
        rv = self.run_fio_test("fill_100_percent")
        if rv == 1:
        ...
        sections = ""
        self.prepare_fio_job(sections)
        size = int(int(lvol_bdev1['num_blocks']) * int(lvol_bdev1['block_size']) * 0.75)
        self.sed_file("filename=", "filename=/dev/nbd1", path.join(self.path, 'fio_job.tmp'))
        self.sed_file("size=", "size=%s" % size, path.join(self.path, 'fio_job.tmp'))
