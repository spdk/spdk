...
class FioThread(threading.Thread):
    def __init__(self, nbd_disk, offset, size, write, pattern):
        super(FioThread, self).__init__()
        self.nbd_disk = nbd_disk
        self.fio_cmd = "fio --name=fio_test --verify=md5 --do_verify=1 --verify=pattern "
        self.fio_cmd += " --filename=%s --offset=%s --size=%s" % (nbd_disk, offset, size)
        self.fio_cmd += " --rw=%s" % write
        if pattern:
            self.fio_cmd += " --verify_pattern=%s" % pattern
        self.rv = 1

    def run(self):
        try:
            output_fio = subprocess.check_output(self.fio_cmd, stderr=subprocess.STDOUT, shell=True)
            self.rv = 0
            ...
            self.rv = 1
        output_flush = subprocess.check_output("blockdev --flushbufs %s" % self.nbd_disk,
...
class TestCases(object):
    ...
    def run_fio_test(self, nbd_disk, offset, size, write, pattern):
        fio_cmd = "fio --name=fio_test --verify=md5 --do_verify=1 --verify=pattern "
        fio_cmd += " --filename=%s --offset=%s --size=%s" % (nbd_disk, offset, size)
        fio_cmd += " --rw=%s" % write
        if pattern:
            fio_cmd += " --verify_pattern=%s" % pattern
        try:
            ...
            print("ERROR: Fio test ended with unexpected exception.")
            rv = 1
        output_flush = subprocess.check_output("blockdev --flushbufs %s" % nbd_disk,
                                               stderr=subprocess.STDOUT, shell=True)
    ...
    def test_case800(self):
        ...
        print "asdas: %s" % self.c.get_lvol_stores()[0]
        ...
        print "construct bdev"
        bdev_name = self.c.construct_lvol_bdev(uuid_store, self.lbd_name,
                                               bdev_size)
        print "get bdev"
        lvol_bdev = self.c.get_lvol_bdev_with_name(bdev_name)
        nbd_name0 = "/dev/nbd0"
        print "start nbd: %s" % lvol_bdev['name']
        rc = self.c.start_nbd_disk(lvol_bdev['name'], nbd_name0)
        print "fio test"
        size = size = bdev_size * MEGABYTE
        rv = self.run_fio_test("/dev/nbd0", 0, size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        print "snapshot"
        snapshot_name = "snapshot" + str(0)
        self.c.snapshot_lvol_bdev(lvol_bdev['name'], snapshot_name)
        ...
        lvol_bdev = self.c.get_lvol_bdev_with_name(snapshot_name)
        nbd_name1 = "/dev/nbd1"
        rc = self.c.start_nbd_disk(bdev_name, nbd_name1)

        size = size = bdev_size * MEGABYTE
        rv = self.run_fio_test("/dev/nbd0", 0, size, "write", "0xcc")
        if rv == 0:
            print("Fio test ended with unexpected success")
            fail_count += 1

        self.c.stop_nbd_disk(nbd_name0)
        self.c.stop_nbd_disk(nbd_name1)
        fail_count += self.c.delete_bdev(snapshot_name)
    ...
    def test_case801(self):
        ...
        size = int(int(lvs[0][u'free_clusters'] * lvs[0]['cluster_size']) / 5 / MEGABYTE)
        lbd_name0 = self.lbd_name + str(0)
        lbd_name1 = self.lbd_name + str(1)
        print "construct: %s, %s" % (size, lbd_name0)
        ...
        rv = self.run_fio_test("/dev/nbd0", 0, fill_size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        ...
        nbd_name1 = "/dev/nbd1"
        print "start nbd: %s" % lvol_bdev['name']
        rc = self.c.start_nbd_disk(lvol_bdev['name'], nbd_name1)
        print "fio test"
        fill_size = size * MEGABYTE
        rv = self.run_fio_test("/dev/nbd1", 0, fill_size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1

        print "snapshot0: %s" % uuid_bdev0
        snapshot_name0 = "snapshot" + str(0)
        self.c.snapshot_lvol_bdev(uuid_bdev0, snapshot_name0)
        print "snapshot1"
        snapshot_name1 = "snapshot" + str(1)
        self.c.snapshot_lvol_bdev(uuid_bdev1, snapshot_name1)
        nbd_name2 = "/dev/nbd2"
        self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name0, nbd_name + str(1))
        nbd_name3 = "/dev/nbd3"
        rv = self.compare_two_disks(nbd_name + str(0), nbd_name + str(2))
        if rv == 1:
            print("ERROR: Cmp ended with failure")
            fail_count += 1
        rv = self.compare_two_disks(nbd_name + str(1), nbd_name + str(3))
        if rv == 1:
            print("ERROR: Cmp ended with failure")
            fail_count += 1

        fill_size = int(size * MEGABYTE / 2)
        rv = self.run_fio_test("/dev/nbd0", offset, fill_size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        rc = self.c.stop_nbd_disk(nbd_name0)
        rc = self.c.stop_nbd_disk(nbd_name1)
        rc = self.c.stop_nbd_disk(nbd_name2)
        rc = self.c.stop_nbd_disk(nbd_name3)
    ...
    def test_case802(self):
        ...
        nbd_name = "/dev/nbd0"
        self.c.start_nbd_disk(lvol_bdev['name'], nbd_name)
        print "fio test"
        fill_size = int(size * MEGABYTE)
        thread = FioThread(nbd_name, 0, fill_size, "write", "0xcc")
        print "thread start"
        thread.start()
        print "create snapshot"
        self.c.snapshot_lvol_bdev(lvol_bdev['name'], "snapshot")
        print "thread.join"
        thread.join()
        if thread.rv == 1:
            print("Fio test ended with failure")
    ...
    def test_case803(self):
        ...
        self.c.snapshot_lvol_bdev(lvol_bdev['name'], "snapshot")
        print "thread.join"
        if self.c.snapshot_lvol_bdev("snapshot", "snapshot1") == 0:
            print("ERROR: Creating snapshot of snapshot should failed")
            fail_count += 1
        fail_count += self.c.delete_bdev(lvol_bdev['name'])
        fail_count += self.c.destroy_lvol_store(uuid_store)
        fail_count += self.c.delete_bdev(base_name)
    ...
    def test_case804(self):
        ...
        clone_name0 = "clone0"
        rv = self.c.clone_lvol_bdev(self, lvol_bdev['name'], clone_name)
        if rv == 0:
            print("ERROR: Creating clone of lvol bdev unded with unexpected success")
            fail_count += 1
        fail_count += self.c.snapshot_lvol_bdev(lvol_bdev['name'], "snapshot")
        rv = self.c.clone_lvol_bdev(self, lvol_bdev['name'], clone_name)
        if rv == 0:
            print("ERROR: Creating clone of lvol bdev unded with unexpected success")
            fail_count += 1
        rv = self.c.clone_lvol_bdev("snapshot", clone_name)
        if rv != 0:
    ...
    def test_case805(self):
        ...
        snapshot_name = "snapshot" + str(0)
        self.c.construct_lvol_snapshot(lvol_bdev['name'], snapshot_name)
        fail_count = self.c.clone_lvol_bdev(self, lvol_bdev['name'], "clone0")
        fail_count = self.c.clone_lvol_bdev(self, lvol_bdev['name'], "clone1")

        lvol_clone = self.c.get_lvol_bdev_with_name("clone0")
        nbd_name1 = "/dev/nbd1"
        print "start nbd: %s" % lvol_clone['name']
        rc = self.c.start_nbd_disk(lvol_clone['name'], nbd_name1)
        print "fio test"
        fill_size = int(ize * MEGABYTE / 2)
        rv = self.run_fio_test(nbd_name1, 0, fill_size, "write", "0xcc")
        if rv == 1:
            print("Fio test ended with failure")
            fail_count += 1
        nbd_name2 = "/dev/nbd2"
        self.c.start_nbd_disk(self.lvs_name + "/" + snapshot_name, nbd_name1)
        nbd_name3 = "/dev/nbd3"
        lvol_clone1 = self.c.get_lvol_bdev_with_name("clone1")
        self.c.start_nbd_disk(lvol_clone1['name'], nbd_name3)
        rv = self.compare_two_disks(nbd_name2, nbd_name3)
        if rv == 1:
            print("ERROR: Cmp ended with failure")
            fail_count += 1
