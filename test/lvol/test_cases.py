...
class TestCases(object):
    ...
    def test_case850(self):
        ...
        lbd_name1 = self.lbd_name + "1"
        bdev_uuid1 = self.c.bdev_lvol_create(lvs_uuid,
                                             lbd_name1,
                                             lbd_size)
        lvol_bdev1 = self.c.get_lvol_bdev_with_name(bdev_uuid1)
        lbd_name2 = self.lbd_name + "2"
        bdev_uuid2 = self.c.bdev_lvol_create(lvs_uuid,
                                             lbd_name2,
                                             lbd_size)
        lvol_bdev2 = self.c.get_lvol_bdev_with_name(bdev_uuid2)
        # Bdev lvol delete
        fail_count += self.c.bdev_lvol_delete(bdev_uuid2, clear_method='none')
        ...
        # Check if data on first lvol bdevs changed.
        fail_count += self.run_fio_test("/dev/nbd0", offset, lvs['cluster_size'], "read", "0x00")
        # Check if data on second lvol bdevs remain unchanged.
        fail_count += self.run_fio_test("/dev/nbd0", offset * 2, lvs['cluster_size'], "read", "0xdd")
        fail_count += self.c.nbd_stop_disk(nbd_name)
    ...
    def test_case851(self):
        ...
        Test lvol_store with clear_method equals to unmap
        """
        # Create malloc bdev
        base_name = self.c.bdev_malloc_create(self.total_size,
                                              self.block_size)
        ...
        # Check that second lvol bdev is filled with zeroes
        nbd_name = "/dev/nbd0"
        fail_count = self.c.nbd_start_disk(bdev_uuid2, nbd_name)
        fail_count += self.run_fio_test(nbd_name, 0, lvs['cluster_size'],
                                        "read", "0x00")
        fail_count += self.c.nbd_stop_disk(nbd_name)

        # Fill first lvol bdev with data
        nbd_name = "/dev/nbd0"
        fail_count = self.c.nbd_start_disk(bdev_uuid1, nbd_name)
        fail_count += self.run_fio_test(nbd_name, 0, lvs['cluster_size'],
                                        "write", "0xdd")
        fail_count += self.c.nbd_stop_disk(nbd_name)

        # Fill second lvol bdev with data
        nbd_name = "/dev/nbd0"
        fail_count = self.c.nbd_start_disk(bdev_uuid2, nbd_name)
        fail_count += self.run_fio_test(nbd_name, 0, lvs['cluster_size'],
                                        "write", "0xdd")
        ...
        # Check if data on area between end of metadata
        # and first cluster of lvol bdev are zeroed
        fail_count += self.run_fio_test("/dev/nbd0", offset_metadata_end,
                                        size_metadata_end, "read", "0x00")
        # Check if data on first lvol bdev are zeroed.
        fail_count += self.run_fio_test("/dev/nbd0", offset, lvs['cluster_size'], "read", "0x00")
        # Check if data on second lvol bdev remain unchanged.
        fail_count += self.run_fio_test("/dev/nbd0", offset * 2, lvs['cluster_size'], "read", "0xdd")
        fail_count += self.c.nbd_stop_disk(nbd_name)
