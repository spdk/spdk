...
- construct lvol store on malloc bdev
- construct thin provisioned lvol bdev with size equals to 50% of lvs space
- construct thick provisioned lvol bdev with size equals to 50% of lvs space
- fill first and second lvol bdev with 100% of their space 
- create snapshots of created lvol bdevs and check that they are readonly
- install lvol bdevs and snapshots in kernel using rpc start_nbd_disk
- check using cmp program if data on coressponding lvol bdevs
  and snapshots are the same
- uninstall lvol bdevs and snapshots from kernel using rpc stop_nbd_disk
- destroy lvol bdevs
- destroy lvol store
- destroy malloc bdev
...
- fill lvol bdev with 100% of its space using write operation with known pattern
