...
# Create lvol store on aio bdev. Delete lvs. Delete aio bdev
# and add it again. Check if vhost has no lvol stores.
# Add another aio bdev. Create lvs and lvol bdevs on two aio bdevs.
# Restart vhost, add again aio bdevs and check
...
function test_tasting() {
	...
	# Create 5 lvols on first lvs
        lvol_size_mb=$(( LVS_DEFAULT_CAPACITY_MB / 10 ))
	...
	# Create 5 lvols on second lvs
        lvol2_size_mb=$(( ( AIO_SIZE_MB - 16 ) / 5 ))
        # Round down lvol size to the nearest cluster size boundary
        lvol2_size_mb=$(( lvol2_size_mb / 32 * 32 ))
	...
	# Clean up
        rpc_cmd bdev_lvol_delete_lvstore -u "$lvs_uuid1"
        ! rpc_cmd bdev_lvol_get_lvstores -u "$lvs_uuid1"
	rpc_cmd bdev_lvol_delete_lvstore -u "$lvs_uuid2"
        ! rpc_cmd bdev_lvol_get_lvstores -u "$lvs_uuid2"
	rpc_cmd bdev_aio_delete aio_bdev0
