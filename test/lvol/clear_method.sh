...
hex_dump_lbd_pattern=$(hexdump -n 2048 -s 4194304 testfile)
...
./scripts/setup.sh reset
dd if=/dev/nvme0n1 of=testfile bs=1M count=50
hex_dump_lbd_test=$(hexdump -n 2048 -s 4194304 testfile)
hex_dump_lvs_pattern=$(hexdump -n 1024 testfile)

stty echo
if [ "$hex_dump_lbd_pattern" != "$hex_dump_lbd_test" ]; then
...
killprocess $spdk_tgt_pid
./scripts/setup.sh reset
hex_dump_lvs_test=$(hexdump -n 1024 testfile)

if [[ "$hex_dump_lvs_pattern" != "$hex_dump_lvs_test" ]]; then
