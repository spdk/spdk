...
function test_resize_lvol_negative() {
	...
	! rpc_cmd bdev_lvol_resize "$dummy_uuid" 0
	...
	! rpc_cmd bdev_lvol_resize "$lvol_uuid" "$MALLOC_SIZE_MB"
	# just make the size of the real lvol did not change
	lvol=$(rpc_cmd bdev_get_bdevs -b "$lvol_uuid")
	[ "$(jq -r '.[0].num_blocks' <<< "$lvol")" = "$(( LVS_DEFAULT_CAPACITY / MALLOC_BS ))" ]
...
run_lvol_test test_resize_lvol
run_lvol_test test_resize_lvol_negative
run_lvol_test test_resize_lvol_with_io_traffic
