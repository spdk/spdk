...
function test_bdev_lvol_create_duplicate_names() {
	...
	# round down lvol size to the nearest cluster size boundary
	lvol2_size_mb=$(round_down $lvol2_size_mb)
	...
	block_size="$(jq -r '.[0].block_size' <<< "$lvol1")"
        num_blocks="$(jq -r '.[0].num_blocks' <<< "$lvol1")"
        lvol_1_size_mb=$(( block_size * num_blocks / 1024 / 1024 ))
	[ "$lvol_1_size_mb" = "$LVS_DEFAULT_CAPACITY_MB" ]
	...
	block_size="$(jq -r '.[0].block_size' <<< "$lvol2")"
	num_blocks="$(jq -r '.[0].num_blocks' <<< "$lvol2")"
	lvol_2_size_mb=$(( block_size * num_blocks / 1024 / 1024 ))
	[ "$lvol_2_size_mb" = "$lvol2_size_mb" ]
...
run_lvol_test test_bdev_lvol_create_duplicate_names_positive
