...
case $1 in
	-h|--help)
		...
		echo "  -fls|--fs-integrity-lvol-scsi  for running an integrity test with filesystem and lvol backends"
		echo "  -flb|--fs-integrity-lvol-blk   for running an integrity test with filesystem and lvol backends"
...
case $1 in
	...
	-ilb|--integrity-lvol-blk)
		...
		--ctrl-type=vhost_blk
		;;
	-fls|--fs-integrity-lvol-scsi)
		echo 'Running lvol ntfs integrity nightly suite with one core and one controller'
		./lvol/lvol_test.sh --fs-type=ntfs --max-disks=1 \
		--ctrl-type=vhost_scsi
		echo 'Running lvol btrfs integrity nightly suite with one core and two controllers'
		./lvol/lvol_test.sh --fs-type=btrfs --max-disks=1 \
		--ctrl-type=vhost_scsi
		echo 'Running lvol ext4 integrity nightly suite with two cores and two controllers'
		./lvol/lvol_test.sh --fs-type=ext4 --max-disks=1 \
		--ctrl-type=vhost_scsi --distribute-cores
		echo 'Running lvol xfs integrity nightly suite with one core and one controller'
		./lvol/lvol_test.sh --fs-type=xfs --max-disks=1 \
		--ctrl-type=vhost_scsi

		;;
	-flb|--fs-integrity-lvol-blk)
		echo 'Running lvol ntfs integrity nightly suite with one core and one controller'
		./lvol/lvol_test.sh --fs-type=ntfs --max-disks=1 \
		--ctrl-type=vhost_blk
		echo 'Running lvol btrfs integrity nightly suite with one core and two controllers'
		...
		echo 'Running lvol ext4 integrity nightly suite with two cores and two controllers'
		./lvol/lvol_test.sh --fs-type=ext4 --max-disks=1 \
		--ctrl-type=vhost_blk --distribute-cores
		echo 'Running lvol xfs integrity nightly suite with one core and one controller'
		./lvol/lvol_test.sh --fs-type=xfs --max-disks=1 \
		--ctrl-type=vhost_blk
