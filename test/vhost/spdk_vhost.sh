...
case $1 in
	...
	-fs|--fs-integrity-scsi)
		...
		run_test case $WORKDIR/integrity/integrity_start.sh -i $VM_IMAGE -m scsi -f "xfs ntfs btrfs ext4"
		report_test_completion "vhost_fs_integrity_scsi"
		;;
	-fb|--fs-integrity-blk)
		echo 'Running filesystem integrity suite...'
		run_test case $WORKDIR/integrity/integrity_start.sh -i $VM_IMAGE -m blk -f "xfs ntfs btrfs ext4"
