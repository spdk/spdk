...
trap "error_exit; rm -f $aio_file; rm -rf $WIN_SCSI_DIR/results;  exit 1" SIGINT SIGTERM EXIT

mkdir -p $WIN_SCSI_DIR/results
dd if=/dev/zero of=$aio_file bs=512 count=1048576
...
vm_sshpass "$vm_num" "$ssh_pass" "cd /cygdrive/c/SCSI; powershell.exe -file compliance_test.ps1"
