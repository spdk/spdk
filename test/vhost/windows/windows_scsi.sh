...
while [[ $TIMEO -gt 0 && rc -ne 0 ]]; do
    sshpass -p 'root' ssh -o ConnectTimeout=1 root@192.200.200.253 "true"
    rc=$?
    ((TIMEO-=1))
    sleep 1
done
...
sshpass -p 'root' ssh root@192.200.200.253 "cd /cygdrive/c/SCSI; powershell.exe -file compliance_test.ps1"
sshpass -p 'root' scp root@192.200.200.253:/cygdrive/c/SCSI/WIN_SCSI_* $WORKSPACE/results/
