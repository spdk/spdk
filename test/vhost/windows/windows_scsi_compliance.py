...
for log in scsi_logs:
    ...
    mo = re.findall(pattern, log_text)
    if len(mo) < 1:
    ...
    for m in mo:
        ...
        if "FAIL" in result.upper():
            fails.append([log, test_name, result])
        elif "WARN" in result.upper():
            warns.append([log, test_name, result])
        ...
        try:
            results[ver][test_name][d_name] = result
        except KeyError:
...
with open(os.path.join("./", "results.html"), 'a') as RH:
    html = "<html>"
