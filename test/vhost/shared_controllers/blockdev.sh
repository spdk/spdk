...
{
	[[ ! -z $2 ]] && ( echo "$2"; echo ""; )
	echo "Script for running test for vhost initiator with shared controlesrs."
...
trap 'rm -f *.state $ROOT_DIR/spdk.tar.gz $ROOT_DIR/fio.tar.gz;\
...
function create_bdev_config()
{
	...
	$RPC_PY construct_vhost_blk_controller naa.Nvme0n1_blk0.0 Nvme0n1p0
	$RPC_PY construct_vhost_blk_controller naa.Nvme0n1_blk1.0 Nvme0n1p1
...
vm_setup --disk-type=spdk_vhost_scsi --force=$vm_no --os=$os_image \
 --disks="Nvme0n1_blk0,spdk_vhost_blk:Nvme0n1_blk1,spdk_vhost_blk:Nvme0n1_blk0,spdk_vhost_blk_shared:Nvme0n1_blk1,spdk_vhost_blk_shared" \
