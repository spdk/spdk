...
function scsi_test_case2() {
    ...
    if [ -z $hot ]; then
        ...
        $rpc_py construct_vhost_scsi_controller naa.Nvme0n1p0.0
        $rpc.py add_vhost_scsi_lun naa.Nvme0n1p0.0 0 ${hot}Nvme0n1p0
        $rpc_py construct_vhost_scsi_controller naa.Nvme0n1p1.1
        $rpc.py add_vhost_scsi_lun naa.Nvme0n1p1.1 0 Nvme0n1p1
    fi
    ...
    for vm_num in $used_vms; do
        ...
        $run_fio;
        wait $!
        check_fio_retcode "SCSI HOT REMOVE test case 2: After unbind disk." 1 $?) &
    done
    sleep 1
    unbind_bdev $bdf
    sleep 1
    ...
    for vm_num in $used_vms; do
        prepare_fio $vm_num
        $run_fio
        check_fio_retcode "SCSI HOT REMOVE test case 2: After reboot disk." 1 $?
        vm_ssh $vm_num "if lsblk -d /dev/$NVME_DISK>>/dev/null; then echo \"Error: NVMe disk is found\"; fi"
    done
    ...
    vm_shutdown_all
    if [ -z $hot ]; then
        $rpc_py remove_vhost_scsi_dev naa.${hot}Nvme0n1p0.0 0
    else
...
function scsi_test_case3() {
    ...
    if [ -z $hot ]; then
        ...
        $rpc.py add_vhost_scsi_lun naa.Nvme0n1p0.0 0 ${hot}Nvme1n1p0
        $rpc_py construct_vhost_scsi_controller naa.Nvme1n1p0.1
        $rpc.py add_vhost_scsi_lun naa.Nvme1n1p0.1 0 Nvme1n1p0
    fi
    ...
    if [ -z $hot ]; then
        ...
        $rpc_py remove_vhost_scsi_dev naa.${hot}Nvme1n1p0.0 0
    fi
    $rpc_py remove_vhost_scsi_dev naa.Nvme0n1p0.0 0
    $rpc_py remove_vhost_controller naa.Nvme0n1p0.0
    $rpc_py remove_vhost_scsi_dev naa.Nvme1n1p0.1 1
...
function scsi_test_case4() {
    ...
    if [ -z $hot ]; then
        ...
        $rpc_py construct_vhost_scsi_controller naa.Nvme0n1p0.0
        $rpc.py add_vhost_scsi_lun naa.Nvme0n1p0.0 0 ${hot}Nvme2n1p0
        $rpc_py construct_vhost_scsi_controller naa.Nvme0n1p1.1
        $rpc.py add_vhost_scsi_lun naa.Nvme1n1p1.1 0 ${hot}Nvm2n1p1
        $rpc_py construct_vhost_scsi_controller naa.Nvme1n1p0.1
        $rpc.py add_vhost_scsi_lun naa.Nvme0n1p1.1 1 Nvme1n1p1
        $rpc_py construct_vhost_scsi_controller naa.Nvme1n1p1.0
        $rpc.py add_vhost_scsi_lun naa.Nvme1n1p1.0 1 Nvme1n1p0
    fi
    ...
    for vm_num in $used_vms; do
        if [[ $vm_num == $VM_NUM ]]; then
           ...
           fio_pid=$!
            ...
            prepare_fio_scsi $vm_num
            $run_fio &
            (wait $!; retcode=$?; check_fio_retcode "SCSI HOT REMOVE test case 4: After unbind disk." 1 $retcode) &
        fi
    done
    ...
    for vm_num in $used_vms; do
        ...
        if [[ $vm_num == $VM_NUM ]]; then
           wait $!
           check_fio_retcode "SCSI HOT REMOVE test case 4: After reboot disk." 0 $?
           vm_ssh $vm_num "if ! lsblk -d /dev/$NVME_DISK>>/dev/null; then echo \"Error: NVMe disk is found\"; fi"
        else
           check_fio_retcode "SCSI HOT  REMOVE test case 4: After reboot disk." 1 $?
    ...
    vm_shutdown_all
    if [ -z $hot ]; then
        $rpc_py remove_vhost_scsi_dev naa.${hot}Nvme2n1p0 0
        $rpc_py remove_vhost_scsi_dev naa.${hot}Nvme2n1p1 0
    else
        $rpc_py remove_vhost_scsi_dev naa.Nvme0n1p0.0 0
