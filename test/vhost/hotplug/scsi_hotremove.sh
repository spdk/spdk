...
# 1. Run the command to hot remove NVMe disk.
    delete_nvme "Nvme0n1"
# 2. If vhost crashed then tests stopped running
    sleep 1
...
# 3. Run two VMs, attached to scsi controllers.
    vms_setup
    vm_run_with_arg 0 1
    vms_prepare "0 1"

    vm_check_scsi_location "0"
    local disks0="$SCSI_DISK"
    vm_check_scsi_location "1"
    local disks1="$SCSI_DISK"

    traddr=""
    get_traddr "Nvme0"
    prepare_fio_cmd_tc1 "0"
# 4. Run FIO I/O traffic with verification enabled on on both NVMe disks in VM.
    $run_fio &
    local last_pid=$!
    sleep 3
...
# 6. Check that fio job run on hot-remove device stopped on VM.
#    Expect: Fio should return error message and return code != 0.
    set +xe
    wait $last_pid
    check_fio_retcode "Scsi hotremove test case 2: Iteration 1." 1 $?

# 7. Check if removed devices are gone from VM.
    vm_check_scsi_location "0"
    check_disks "$disks0" "$SCSI_DISK"
    vm_check_scsi_location "1"
    check_disks "$disks1" "$SCSI_DISK"
# 8. Reboot both VMs.
