...
#	ssh-copy-id -i $HOME/.ssh/spdk_vhost_id_rsa.pub 127.0.0.1
	#$SSHCMD root@$VM_IP "$BASE_DIR/readonly_vm.sh"
	vm_ssh 0 "bash -s" < $BASE_DIR/readonly_vm.sh
	vm_shutdown 0
	sleep 6
	$rpc_py remove_vhost_controller naa.Nvme0n1.0
	$rpc_py construct_vhost_blk_controller -r naa.Nvme0n1.0 Nvme0n1
	...
	$BASE_DIR/../common/vm_run.sh $x --work-dir=$TEST_DIR 0
	vm_wait_for_boot 600
	vm_ssh 0 "bash -s" < $BASE_DIR/readonly_vm2.sh
	vm_shutdown 0
	sleep 6
}
