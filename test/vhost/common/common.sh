...
function run_fio()
{
	...
	for vm in ${vms[@]}; do
		...
		fio_disks+="127.0.0.1:$(vm_fio_socket $vm_num)$(printf ':/dev/%s' $vmdisks),"
	done

	python $SPDK_BUILD_DIR/test/vhost/common/run_fio.py --job-file=/root/job.file $fio_bin $out ${fio_disks%,}
}
