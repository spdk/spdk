...
function spdk_vhost_usage_common()
{
  ...
  ENVIRONMENT:
    ...
    VHOST_TEST_ARGS     Program arguments without COMMON ARGUMENTS. Please use this variable in your scripts.
    TEST_DIR            Any test data (VMs, configs etc) will be created here.
                        Default is: $SPDK_VHOST_DEFAULT_TEST_DIR.

    SPDK_BUILD_DIR      Path to SPDK we are running against.
                        Default is: $SPDK_DEFAULT_BUILD_DIR

    Those environment variables are read only and MIGHT NOT be changed after sourcing this file.

    FOLDERS AND FILES:
    TEST_DIR/           Will be created if not exist.
        	/vhost/     SPDK vhost working directory. Here are created sockets, configs etc.
            /vms/[0-N]/ Generated VMs
            /root/bin   Place where all tools (like QEMU) are installed 
"
}
...
echo "INFO: Using SSH key file $SPDK_VHOST_SSH_KEY_FILE"
#
...
Usage: spdk_vhost_run [OPTIONS]
  ...
  DESCRIPTION:
    Run SPDK vhost. Parameters accepted by vhost app are accepted here, except: -r, -f, -c.
      '-r' and '-f' are used internally so can't be changed.
...
function spdk_vhost_run()
{
	...
	if [[ $EUID -ne 0 ]]; then
		error "spdk_vhost_run: Go away user come back as root"
		return 1
	elif [[ ! -x $vhost_app ]]; then
		error "spdk_vhost_run: application '$vhost_app' not found. Try building SPDK first."
		return 1
	elif [[ -z "$vhost_reactor_mask" ]] || [[ -z "$vhost_master_core" ]]; then
		error "spdk_vhost_run: Parameters vhost_reactor_mask or vhost_master_core not set. Don't know how to run vhost :("
		return 1
	elif [[ -r "$VHOST_DIR/vhost.pid" ]] && kill -0 $VHOST_DIR/vhost.pid; then
		error "spdk_vhost_run: One instance of vhost already running in '$VHOST_DIR'. Kill it first." 
		return 1
	fi
	...
	if [[ ! -z "$conf_in" ]]; then
		...
		echo "INFO: Using generated configuration file:"
		cat "$VHOST_DIR/vhost.conf"
	fi
	
	echo "INFO: running SPDK vhost app:"
	echo "INFO: gdb: $do_gdbserver"
	echo "INFO: $vhost_app ${vhost_args[@]}"
	
	DEFAULT_RPC_ADDR="$VHOST_DIR/vhost_rpc.socket"
	
	if [[ ! -z "$gdbserver_addr" ]]; then
		echo "INFO: "
		echo "INFO: Runing under gdbserver on address $gdbserver_addr - connect to continue"
		echo "INFO: "
	...
	if (( pid_timeout == 0 )); then
		echo "INFO: not waiting for vhost to start"
		return 0
	fi
	...
	for (( ; pid_timeout; pid_timeout--)); do
		...
		echo "INFO: vhost started - pid=$(cat $VHOST_DIR/vhost.pid)"
		return 0
	done
...
Usage: vm_setup [OPTIONS]
  OPTIONS:
    --vm-num=VM_NUM         OPTIONAL: Force VM_NUM reconfiguration if already exist. If not provided,
                            next free VM number will be used.
    --disk=TYPE[,PARAMS]    MANDATORY: Disk to add (can be added multiple times). TYPE can be one of
                              ...
                              spdk-vhost-blk - the SPDK vhost Block controller name. Parameters:
                                ...
                                example: spdk_vhost_blk:path=ctrlr1:size=10G:
    --os=[MODE:]OS_QCOW2    MANDATORY: OS image file in qcow2 format. MODE is optional (default snapshot)
    ...
    --qemu-args=ARGS        OPTIONAL: Custom QEMU args to be directly appended to QEMU command line.
"
	spdk_vhost_usage_common
}
...
function vm_setup()
{
		...
		case "$arg" in
			...
			--os=*)
				if [[ ! "${arg#*=}" =~ ^((snapshot|backing|original)[,])?([^,]+)$ ]]; then
					error "vm_setup: invalid argument '$arg'"
					return 0
	...
	if [[ ! -z "$vm_num" ]]; then
		...
		if (( i == 256 )); then
			error "vm_setup: no free VM found. do some cleanup (256 VMs created, are you insane?)"
			return 1
		fi
	...
	if [[ -z "$qemu_mask" || -z "$qemu_numa_node" ]]; then
		error "vm_setup: Parameters VM_${vm_num}_qemu_mask or VM_${vm_num}_qemu_numa_node not found in autotest.config file"
		return 1
	fi
	...
	echo "INFO: TASK MASK: $qemu_mask"
	echo "INFO: NUMA NODE: $qemu_numa"
		
	if [[ -z "$os_img" || -z "${disks[@]}" ]]; then
		error "vm_setup: Missing mandatory parameter: os='$os_img', disks='${disks[@]}'"
		return 1
	fi
	
	local qemu_devices=""
	if [[ $os_mode = "backing" ]]; then
		echo "INFO: creating backing file for OS image file: $os_img"
		if ! $INSTALL_DIR/bin/qemu-img create -f qcow2 -b $os_img $vm_dir/os.qcow2; then
			error "vm_setup: Failed to  create OS backing file in '$vm_dir/os.qcow2' using $os_img"
			return 1
		fi
		
		local os_img=$vm_dir/os.qcow2
	elif [[ $os_mode == "original" ]]; then
		echo "WARNING: Using original OS image file: $os_img"
	...
	for disk in ${disks[@]}; do
		if [[ ! "$disk" =~ ^(virtio-scsi|kernel-vhost-scsi|spdk-vhost-scsi|spdk-vhost-blk)([,](.*))?$ ]]; then
			error "vm_setup: invalid disk '$disk'"
			return 1
		fi
		...
		case ${BASH_REMATCH[1]} in
			virtio-scsi)
				...
				if [[ -z "$disk_path" ]]; then
					...
					if [[ -z "$size" ]]; then
						error "vm_setup: need 'size' for virtio disk"
						return 1
					fi
					
					echo "INFO: Creating Virtio disc $disk_path_disk of size=$size. This might take a while..."
					if ! $INSTALL_DIR/bin/qemu-img create -f raw -o "preallocation=falloc" $disk_path $size; then
						error "vm_setup: Failed to  create '$disk_path'"
						return 1
					fi
				elif [[ ! -z "$size" ]]; then
					error "vm_setup: 'size' is only valid with 'path'"
					return 1
				elif [[ $disk_path =~ ^/dev/.+$ ]] && [[ ! -b $disk_path ]]; then
					error "vm_setup: Virtio disk point to missing device ($disk_path) this is probably not what you want."
					return 1 
				fi
			...
			spdk-vhost-scsi)
				if [[ ! "$params" =~ ctrlr=([^,]+)$ ]]; then
					error "vm_setup: spdk-vhost-scsi invalid or missing ctrlr param '$param'"
					return 1
				fi	
				
				echo "INFO: using socket $VHOST_DIR/${BASH_REMATCH[1]}"
			...
			spdk-vhost-blk)
				...
				if [[ -z "$ctrlr" || -z "$size" ]]; then
					error "vm_setup: invalid or missing parameter param for '$disk'"
					return 1
				fi
									
				echo "INFO: using socket $VHOST_DIR/$ctrlr"
			...
			kernel-vhost-scsi)
				...
				if [[ -z $wwn ]]; then
					error "vm_setup: invalid WWN number: $wwn"
					return 1
				else
					echo "INFO: Using kernel vhost disk wwn=$disk"
...
taskset -a $qemu_mask $INSTALL_DIR/bin/qemu-system-x86_64 $([[ $os_mode == 'snapshot' ]] && echo '-snapshot') \\
 -m 1024 --enable-kvm -smp $cpu_num -cpu host -vga std -vnc :$vnc_socket -daemonize \\
 -object memory-backend-file,id=mem,size=1G,mem-path=/dev/hugepages,share=on,prealloc=yes,host-nodes=$qemu_numa_node,policy=bind \\
 -numa node,memdev=mem \\
 -pidfile $vm_dir/qemu.pid \\
 -serial file:$vm_dir/serial.log \\
 -D $vm_dir/qemu.log \\
 -net user,hostfwd=tcp::$(cat $vm_dir/ssh_socket)-:22,hostfwd=tcp::$(cat $vm_dir/fio_socket)-:8765 -net nic \\
 ${qemu_devices%\\$'\n'} $( [[ ! -z "$qemu_args" ]] && echo "\\" && echo "$qemu_args" )

echo 'Waiting for QEMU pid file'
sleep 1 && [[ ! -f $qemu_pid_file ]] && sleep 1
