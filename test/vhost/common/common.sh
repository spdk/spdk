		...
		case $disk_type in
			...
			spdk_vhost_scsi)
				...
				if [[ "$disk" == "$boot_from" ]]; then
					cmd+="-device vhost-user-scsi-pci,id=scsi_$disk,num_queues=$queue_number,chardev=char_$disk,bootindex=0 ${eol}"
			...
			spdk_vhost_blk)
				...
				if [[ "$disk" == "$boot_from" ]]; then
					...
					cmd+="-device vhost-user-blk-pci,num-queues=$queue_number,chardev=char_$disk ${eol},bootindex=0 ${eol}"
					boot_disk_present=true
				fi
				;;
			kernel_vhost)
