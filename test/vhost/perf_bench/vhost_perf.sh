...
while getopts 'xh-:' optchar; do
	case "$optchar" in
		...
		case "$OPTARG" in
			...
			measure-cpu) vm_measure_cpu=true ;;
			throttle-iops=*) vm_throttle="${OPTARG#*=}" ;;
...
if [[ $lvol_precondition == true && $use_split == false ]]; then
	...
	--group_reporting --direct="1" --size="5G" --loops="2" --bs="256k" \
