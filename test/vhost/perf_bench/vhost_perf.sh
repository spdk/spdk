...
function cleanup_fdisk_config()
{
	local disks=$(ls /dev/nvme*n1)
	newline=$'\n'
	script="d
1
w
...
if [[ "$ctrl_type" == "kernel_vhost" ]]; then
	...
	# Split disks using fdisk for kernel vhost
	newline=$'\n'
	for (( i=0; i<$max_disks; i++ ));do
		total_sectors=$(fdisk /dev/nvme${i}n1 -l | grep -Po "\K(\d+)+(?= sectors)")
		total_sectors=$(($total_sectors-1))
		part_size=$(($total_sectors/${splits[$i]}))
		echo "Total sectors on nvme${i}n1: $total_sectors"
		echo "  Creating ${splits[$i]} partitions of size ${part_size}"
		fdisk_script="p
n
e
1
...
$total_sectors
"
		# Count offset for each new logical partition and create it
		for p in $(seq 1 ${splits[$i]}); do
			fdisk_script+="n${newline}"
			fdisk_script+="${newline}"
			offset=$(($part_size*$p))
			fdisk_script+="${offset}${newline}"
		done
		fdisk_script+="p${newline}"
		fdisk_script+="w${newline}"
