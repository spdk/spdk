...
function usage()
{
	...
	echo "    --sar-delay=INT         Wait for X seconds before sarting SAR measurement. Default: 0."
	...
	echo "                            Example:"
	echo "                            NVME BDF,Spdk Bdev Name,Split Count,VM List"
	echo "                            NVME BDF,Spdk Bdev Name,Split Count,VM List"
	echo "                            NVME BDF,Spdk Bdev Name,Split Count,VM List"
...
function cleanup_split_cfg()
{
	...
	for disk in ${disk_cfg_spdk_names[@]}; do
		$rpc_py destruct_split_vbdev ${disk}n1
	done
}
...
function create_spdk_controller() {
	...
	if [[ "$ctrl_type" == "spdk_vhost_scsi" ]]; then
		$rpc_py construct_vhost_scsi_controller naa.0.$vm_num
		notice "Created vhost scsi controller naa.0.$vm_num"
		$rpc_py add_vhost_scsi_lun naa.0.$vm_num 0 $bdev
		notice "Added LUN 0/$bdev to controller naa.0.$vm_num"
	elif [[ "$ctrl_type" == "spdk_vhost_blk" ]]; then
		$rpc_py construct_vhost_blk_controller naa.0.$vm_num $bdev
...
for line in "${disk_cfg[@]}"; do
	...
	if [[ "$ctrl_type" == "kernel_vhost" ]]; then
		tmp=$(find /sys/devices/pci* -name ${s[0]} | xargs sh -c 'ls $0/nvme')
...
if [[ "$ctrl_type" == "kernel_vhost" ]]; then
	...
	for (( i=0; i<${#disk_cfg_kernel_names[@]}; i++ )); do
		...
		partitions=$(ls -1 /dev/${nvme}n1* | sort --version-sort | grep -P 'p(?!1$)\d+')
	...
	if [[ $use_split == true ]]; then
		...
		for (( i=0; i<${#disk_cfg_bdfs[@]}; i++ )); do
			nvme_bdev=$($rpc_py construct_nvme_bdev -b ${disk_cfg_spdk_names[$i]} -t pcie -a ${disk_cfg_bdfs[$i]})
			notice "Created NVMe Bdev: $nvme_bdev with BDF ${disk_cfg_bdfs[$i]}"

			splits=$($rpc_py construct_split_vbdev $nvme_bdev ${disk_cfg_splits[$i]})
			splits=($splits)
		...
		for (( i=0; i<${#disk_cfg_bdfs[@]}; i++ )); do
			...
			ls_guid=$($rpc_py construct_lvol_store $nvme_bdev lvs_$i --clear-method none)
			lvol_stores+=("$ls_guid")
