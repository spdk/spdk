...
function usage()
{
	...
	echo "    --throttle=INT          Throttle devices on VMs to a number of IOPS."
...
if [[ -z $fio_job ]]; then
	warning "No FIO job specified! Will use defalult from common directory."
...
# Calculate number of needed bdevs so that each VM gets it's own
split_per_disk=$(($vm_count/$max_disks))

notice "Preparing NVMe setup..."
notice "Using $max_disks NVMe drives"
notice "Each disk split into $split_per_disk parts"
...
if [[ $use_split == true ]]; then
	...
	split_bdevs=()
	for (( i=0; i<$max_disks; i++ ));do
		out=$($rpc_py construct_split_vbdev Nvme${i}n1 $split_per_disk)
		for s in $out; do
			split_bdevs+=("$s")
		done
	done
	...
	for (( i=0; i<$max_disks; i++ ));do
		...
		lvol_stores+=("$ls_guid")
		for (( j=0; j<$split_per_disk; j++)); do
			free_mb=$(get_lvs_free_mb "$ls_guid")
			size=$((free_mb / (split_per_disk-j) ))
