...
run_spdk_fio "$SHARED_DIR/default_shared_lower.job --filename=$virtio_bdevs --spdk_conf=$SHARED_DIR/bdev.conf" &
run_fio_pid=$!
sleep 1
run_spdk_fio "$SHARED_DIR/default_shared_upper.job --filename=$virtio_bdevs --spdk_conf=$SHARED_DIR/bdev.conf"
wait $run_fio_pid
spdk_vhost_kill
