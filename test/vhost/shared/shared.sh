...
run_spdk_fio "$SHARED_DIR/default_shared.job --section=write_0 --filename=VirtioBlk0 --spdk_conf=$SHARED_DIR/bdev.conf" &
run_fio_pid=$!
sleep 1
run_spdk_fio "$SHARED_DIR/default_shared.job --section=write_1 --filename=VirtioBlk0 --spdk_conf=$SHARED_DIR/bdev.conf"
wait $run_fio_pid
spdk_vhost_kill
