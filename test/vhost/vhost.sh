...
if [ $RUN_NIGHTLY -eq 1 ]; then
	...
	run_test case "vhost_integrity_blk" $WORKDIR/fiotest/fio.sh -x --fio-bin=$FIO_BIN \
	...
	run_test case "vhost_integrity_scsi" $WORKDIR/fiotest/fio.sh -x --fio-bin=$FIO_BIN \
	--vm=0,$VM_IMAGE,Nvme0n1p0:RaidBdev0:RaidBdev1:RaidBdev2 \
	...
	run_test case "vhost_integrity_scsi_fs" $WORKDIR/integrity/integrity_start.sh --ctrl-type=spdk_vhost_scsi --fs="xfs ntfs btrfs ext4"
	...
	run_test case "vhost_integrity_blk_fs" $WORKDIR/integrity/integrity_start.sh --ctrl-type=spdk_vhost_blk --fs="xfs ntfs btrfs ext4"
	...
	if [[ $DISKS_NUMBER -ge 2 ]]; then
		...
		run_test case "vhost_scsi_mcore_mctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
		...
		run_test case "vhost_scsi_score_mctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
	...
	run_test case "vhost_scsi_score_sctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
	...
	if [[ $DISKS_NUMBER -ge 2 ]]; then
		echo 'Running lvol integrity nightly suite with two cores and two controllers'
		run_test case "vhost_blk_mcore_mctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
		--ctrl-type=spdk_vhost_blk --max-disks=2 --distribute-cores --vm-count=2

		echo 'Running lvol integrity nightly suite with one core and two controllers'
		run_test case "vhost_blk_score_mctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
	...
	echo 'Running lvol integrity nightly suite with one core and one controller'
	run_test case "vhost_integrity_score_sctrl" $WORKDIR/lvol/lvol_test.sh --fio-bin=$FIO_BIN \
...
run_test case "vhost_lvol_test" $WORKDIR/lvol/lvol_test.sh -x --fio-bin=$FIO_BIN \
...
echo 'Running lvol integrity suite...'
run_test case "vhost_lvol_integrity" $WORKDIR/lvol/lvol_test.sh -x --fio-bin=$FIO_BIN \
