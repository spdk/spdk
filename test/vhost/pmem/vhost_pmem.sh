...
work_dir=$TEST_DIR
fio_bin=$TEST_DIR/fio_universal
...
function usage() {
    ...
    echo "    --os=                 OS - VM os disk path (optional)"
    exit 0
}
...
$BASE_DIR/../common/run_vhost.sh $x --work-dir=$work_dir --conf-dir=$BASE_DIR &
pid=$!
waitforlisten $pid
...
for i in `seq 0 $LOOP_RANGE`; do
        ...
        if [ $test_type == "spdk_vhost_blk" ]; then
            ...
            $rpc_py construct_vhost_scsi_controller naa.$PMEM_BDEVS.0
        fi
        sleep 1

        $BASE_DIR/../common/vm_setup.sh $x --work-dir=$work_dir --test-type=$test_type --os=$os --disk=$PMEM_BDEV
        $BASE_DIR/../common/vm_run.sh $x --work-dir=$work_dir $i
done
