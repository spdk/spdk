...
def main():
    ...
    fio.communicate(create_fio_config(io_size, queue_depth, device_paths, test_type, runtime, verify).encode('UTF-8'))
    fio.stdin.close()
    rc = fio.wait()
...
def get_target_devices():
    output = check_output('lsblk -l -o NAME', shell=True).decode("utf-8")
    return re.findall(r"(sd[a-z])\n", output)
