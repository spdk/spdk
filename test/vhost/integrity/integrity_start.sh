...
function clean_lvol_cfg()
{
	...
	$rpc_py delete_bdev lvol_store/lvol_bdev || true
	$rpc_py destroy_lvol_store -l lvol_store || true
}
...
cd $SPDK_BUILD_DIR/..
tar --exclude="*.o" --exclude="*.d" --exclude="*.git" -C $SPDK_BUILD_DIR -zcf spdk.tar.gz .

mv spdk.tar.gz $SPDK_BUILD_DIR/
cd $SPDK_BUILD_DIR
trap "rm -f $SPDK_BUILD_DIR/spdk.tar.gz; error_exit ${FUNCNAME} ${LINENO}" SIGTERM SIGABRT ERR

# Run tests on VM
vm_scp 0 $BASE_DIR/integrity_vm.sh root@127.0.0.1:/root/integrity_vm.sh
vm_scp 0 $SPDK_BUILD_DIR/spdk.tar.gz root@127.0.0.1:/root/spdk.tar.gz
...
spdk_vhost_kill
rm -f $SPDK_BUILD_DIR/spdk.tar.gz
