...
fuzz_rpc_py="$rootdir/scripts/rpc.py -s $FUZZ_RPC_SOCK"
...
$fuzz_rpc_py fuzz_vhost_create_dev -s `pwd`/Vhost.1 -b -v
# test the vhost scsi I/O queue with valid data buffers on a valid lun.
$fuzz_rpc_py fuzz_vhost_create_dev -s `pwd`/naa.VhostScsi0.1 -l -v
# test the vhost scsi management queue with valid data buffers.
$fuzz_rpc_py fuzz_vhost_create_dev -s `pwd`/naa.VhostScsi0.2 -v -m
# The test won't actually begin until this option is passed in.
$fuzz_rpc_py start_subsystem_init

wait $fuzzpid

$FUZZ_APP -j $rootdir/test/app/fuzz/vhost_fuzz/example.json &
fuzzpid=$!
waitforlisten $fuzzpid $FUZZ_RPC_SOCK
