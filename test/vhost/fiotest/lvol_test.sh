...
function usage()
{
    ...
    echo "    --use-fs              Create partition table and format disks"
    echo "                          in guest systems. Default: False"
...
# Temporary
mv $BASE_DIR/vhost.conf.in $BASE_DIR/vhost.conf.in.bak
...
# On each of previously created lvol stores create one lvol bdev
# and create another, nested lvol store
for lvol_store in "${lvol_stores[@]}"; do
    echo "INFO: Creating lvol bdev on lvol store $ls_guid"
    lb_guid=$($rpc_py construct_lvol_bdev $ls_guid 16000)
    lvol_bdevs+=("$lb_guid")
...
for (( i=0; i<$vm_count; i++)); do
    ...
    setup_cmd="$BASE_DIR/vm_setup.sh $x --work-dir=$TEST_DIR"
...
run_fio="python $BASE_DIR/run_fio.py --fio-bin=$fio_bin"
run_fio+=" --job-file=$BASE_DIR/fio_jobs/default_integrity.job"
...
spdk_vhost_kill
mv $BASE_DIR/vhost.conf.in.bak $BASE_DIR/vhost.conf.in
