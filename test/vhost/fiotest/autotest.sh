...
if $shared_controllers; then
	...
	fio_config_gen $AUTOTEST_BASE_DIR/bdev.fio read
	for b in $(echo $bdevs | jq -r '.name'); do
		fio_config_add_job $AUTOTEST_BASE_DIR/bdev.fio $b
	done
	LD_PRELOAD=$AUTOTEST_BASE_DIR/../../../examples/bdev/fio_plugin/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev --iodepth=8 --bs=4k --runtime=5 $AUTOTEST_BASE_DIR/bdev.fio --spdk_conf=$AUTOTEST_BASE_DIR/bdev.conf
	wait $run_fio_pid
