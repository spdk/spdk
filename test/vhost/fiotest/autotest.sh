	...
	if [[ $test_type =~ "spdk_vhost" ]]; then
		...
		while IFS=':' read -ra disks; do
			for disk in "${disks[@]}"; do
				...
				if $rpc_py construct_vhost_scsi_controller vhost.invalid.cpumask --cpumask 9 > /dev/null; then
					echo "ERROR: Creating controller with incorrect cpumask succeded, but it shouldn't"
					false
				fi

				echo "INFO: Trying to remove device from inexistent scsi controller"
				if $rpc_py remove_vhost_scsi_dev vhost.inexistent.name 0 > /dev/null; then
				...
				if $rpc_py add_vhost_scsi_lun vhost.inexistent.name 0 malloc0 > /dev/null; then
				...
				if $rpc_py remove_vhost_block_dev vhost.inexistent.name 0 > /dev/null; then
