...
if [[ $test_type == "spdk_vhost_scsi" ]]; then
	for vm in ${vms[@]}; do
		for disk in ${vms_ctrlrs_disks[$vm]}; do
			notice "Hotdetach test. Trying to remove existing device from a controller ${vms_ctrlrs[$vm]}"
			$rpc_py remove_vhost_scsi_dev ${vms_ctrlrs[$vm]} 0
			sleep 0.1

			notice "Hotattach test. Re-adding device 0 to ${vms_ctrlrs[$vm]}"
			$rpc_py add_vhost_scsi_lun ${vms_ctrlrs[$vm]} 0 ${vms_ctrlrs_disks[$vm]}
		done
	done
fi
...
if [[ $test_type =~ "spdk_vhost" ]]; then
	...
	for vm in ${vms[@]}; do
		notice "Removing all vhost devices from controller ${vms_ctrlrs[$vm]}"
		if [[ "$test_type" == "spdk_vhost_scsi" ]]; then
			$rpc_py remove_vhost_scsi_dev ${vms_ctrlrs[$vm]} 0
		else
			$rpc_py remove_vhost_controller $disk
		fi
		
		if [[ "$test_type" == "spdk_vhost_scsi" ]]; then
			notice "Removing vhost ${vms_ctrlrs[$vm]}"
			$rpc_py remove_vhost_controller ${vms_ctrlrs[$vm]}
		fi
	done
fi
