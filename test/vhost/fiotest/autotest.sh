...
function usage()
{
	...
	echo "    --lvol-run			The test use bdav with an lvol store"
		...
		while IFS=':' read -ra disks; do
			for disk in "${disks[@]}"; do
				...
				if $lvol_run -eq true ;then
					echo "INFO: Add the lvol to naa.$disk.${conf[0]}"
					$rpc_py construct_lvol_store naa.$disk.${conf[0]} 
					
					echo "INFO: Add a bdev with an lvol backend to naa.$disk.${conf[0]}"					
					$rpc_py construct_lvol_bdev 123 32 
					
					echo "INFO: Extend an lvol store to naa.$disk.${conf[0]}"
					$rpc_py extend_lvol_bdev naa.$disk.${conf[0]}  64 
										
					echo "INFO: Destroy an lvol store to naa.$disk.${conf[0]}"
					$rpc_py destroy_lvol_store naa.$disk.${conf[0]}
				fi_auto
