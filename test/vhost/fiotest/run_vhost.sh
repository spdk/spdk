...
# Printing help will force vhost to exit with no error
...
$VHOST_APP -c /path/to/non_existing_file/conf -f $SPDK_VHOST_SCSI_TEST_DIR/vhost.pid &

# Check if created pid file has a valid pid number
test $(cat $SPDK_VHOST_SCSI_TEST_DIR/vhost.pid) == $!
