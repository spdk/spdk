...
trap 'rm -f *.state $nvme_fio_results $virtio_fio_results; error_exit "${FUNCNAME}""${LINENO}"' ERR SIGTERM SIGABRT
...
run_spdk_nvme_fio --filename="$filename" --runtime=$RUNTIME --iodepth=$IODEPTH --bs=$BLK_SIZE -rw=$RW\
  --rwmixread=$MIX --ramp_time=$RAMP_TIME
...
run_spdk_virtio_fio  --filename=$virtio_bdevs  --runtime=$RUNTIME --bs=$BLK_SIZE --rw=$RW\
 --rwmixread=$MIX --ramp_time=$RAMP_TIME --iodepth=$IODEPTH
timing_enter spdk_virtio_fio
