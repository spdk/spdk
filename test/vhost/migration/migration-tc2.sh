...
function migration_tc2_configure_vhost()
{
	...
	# here to teardown in test cleanup function
	...
	$SPDK_BUILD_DIR/app/nvmf_tgt/nvmf_tgt -s 1024 -m 0x4 -c $nvmf_dir/nvmf.conf -r $nvmf_dir/rpc.sock &
	local nvmf_tgt_pid=$!
	...
	$rpc_nvmf construct_nvmf_subsystem nqn.2016-06.io.spdk:cnode1 "trtype:RDMA traddr:$nvmf_target_ip trsvcid:4420" "" -a -s SPDK00000000000001 -n Nvme0n1
	...
	vm_setup --os="$os_image" --force=$incoming_vm --disk-type=spdk_vhost_scsi --disks=VhostScsi0 --migrate-to=$target_vm --vhost-num=0
	vm_setup --force=$target_vm --disk-type=spdk_vhost_scsi --disks=VhostScsi0 --incoming=$incoming_vm --vhost-num=1
