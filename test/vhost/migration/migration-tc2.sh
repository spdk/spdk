...
function migration_tc2_configure_vhost()
{
	...
	# We need 2x1G for VM, 2x256M fo vhost and 512M for nvmf = 3G
	HUGEMEM=3072 $SPDK_BUILD_DIR/scripts/setup.sh
	...
	spdk_vhost_run --conf-path=$BASE_DIR --memory=256 --vhost-num=0
	# Those are global intentionaly
	vhost_1_reactor_mask=0x2
	vhost_1_master_core=1
	spdk_vhost_run --conf-path=$BASE_DIR --memory=256 --vhost-num=1
