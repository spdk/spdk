...
function migration_tc2_cleanup_nvmf_tgt()
{
	...
	if [[ ! -z "$1" ]] && pkill --signal 0 -F $nvmf_dir/nvmf_tgt.pid; then
		...
		cat $nvmf_dir/nvmf_tgt.pid;
		pkill --signal SIGKILL -F $nvmf_dir/nvmf_tgt.pid || true
		for (( i=0; i<10; i++ )); do
			echo "====="
...
function migration_tc2_configure_vhost()
{
	...
	vm_setup --os="$os_image" --force=$incoming_vm --disk-type=spdk_vhost_scsi --disks=VhostScsi0 \
		--migrate-to=$target_vm  --memory=512 --vhost-num=0
	vm_setup --force=$target_vm --disk-type=spdk_vhost_scsi --disks=VhostScsi0 --incoming=$incoming_vm --memory=512 \
		--vhost-num=1
