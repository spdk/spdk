...
{
	ssh_remote $MGMT_INITIATOR_IP "nohup bash -c '$share_dir/spdk/test/vhost/migration/migration.sh --test-cases=3b --work-dir=$TEST_DIR --os=$share_dir/migration.qcow2' 1>$share_dir/stdout.log 2>$share_dir/stderr.log"
...
function migration_tc3()
{
	...
	ssh_remote $MGMT_INITIATOR_IP "$share_dir/spdk/test/vhost/migration/migration.sh --test-cases=3c --work-dir=$TEST_DIR"
