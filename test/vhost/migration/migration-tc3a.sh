...
function host_2_create_share()
{
	...
	ssh_remote $MGMT_INITIATOR_IP "sshfs -o ssh_command=\
	 \"ssh -o UserKnownHostsFile=/dev/null\ /-o StrictHostKeyChecking=no -o ControlMaster=auto\
...
function host_2_start_vhost()
{
	ssh_remote $MGMT_INITIATOR_IP "nohup $share_dir/spdk/test/vhost/migration/migration.sh\
	 ...
	 --rdma_tgt_ip=$RDMA_TARGET_IP &>$share_dir/output.log &"
