...
run_fio $fio_bin --job-file="$job_file" --vm="${incoming_vm}$(printf ':/dev/%s' $SCSI_DISK)"

# Check if fio is still running by asking to dump stats
if ! vm_ssh $incoming_vm 'pkill -0 -F /root/fio.pid'; then
...
if ! vm_ssh $target_vm 'pkill -0 /root/fio.pid'; then
	vm_ssh $target_vm "cat /root/$(basename ${job_file}).out"
	
	warning "FIO is not running after migration: process crashed or finished too early"
	read	
fi

notice "Waiting for fio to finish"
while vm_ssh $target_vm 'pkill -0 -F /root/fio.pid'; do
	sleep 1
	echo -n "."
done
