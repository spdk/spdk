...
function usage()
{
	...
	echo "    --mgmt_tgt_ip=IP      IP address of target."
	echo "    --mgmt_init_ip=IP     IP address of initiator."
	echo "    --rdma_tgt_ip=IP      IP address of targets rdma capable NIC."
	echo "    --rdma_init_ip=IP     IP address of initiators rdma capable NIC."
...
for param in "$@"; do
	case "$param" in
		...
		--mgmt_tgt_ip=*) MGMT_TARGET_IP="${param#*=}" ;;
		--mgmt_init_ip=*) MGMT_INITIATOR_IP="${param#*=}" ;;
		--rdma_tgt_ip=*) RDMA_TARGET_IP="${param#*=}" ;;
		--rdma_init_ip=*) RDMA_INITIATOR_IP="${param#*=}" ;;
		-x) set -x ;;
