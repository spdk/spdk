...
function run_host_fio() {
		...
		local fio_job_conf=""
		case "$2" in
			"unmap") fio_job_conf="--section=fio_job_unmap_trim_sequential --section=fio_job_unmap_trim_random --section=fio_job_unmap_write" ;;
			...
			"host") fio_job_conf="--section=fio_job_host_randwrite" ;;
...
function start_and_prepare_vm() {
		...
		$COMMON_DIR/vm_setup.sh $x --work-dir=$TEST_DIR --test-type=$test_type --os=$os --disk=$disk -f $force_vm_num --memory=4096 --queue_num=16
