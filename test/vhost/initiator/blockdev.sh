...
sed -i "s|/tmp/vhost.1|$rootdir/../vhost/naa.Malloc0.1|g" $testdir/bdev.conf
sed -i "s|/tmp/vhost.2|$rootdir/../vhost/naa.Malloc1.2|g" $testdir/bdev.conf
...
if [ -d /usr/src/fio ]; then
        ...
        i=0
        test_bdevs=($bdevs)
        for vbdev in $vbdevs; do
                cp $testdir/../common/fio_jobs/default_initiator.job $testdir/bdev.fio
                prepare_fio_job_for_unmap "$vbdev"
