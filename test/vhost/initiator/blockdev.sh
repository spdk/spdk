...
function start_and_prepare_vm() {
	...
	vm_wait_for_boot 600 $vm_num
        vm_ssh $vm_num "GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash default_hugepagesz=1G hugepagesz=1G hugepages=3\" >> /etc/default/grub"
		...
		for bdev_type in "nvme"; do
			...
			if [ -d /usr/src/fio ]; then
				timing_enter fio
					...
					if [ $host_type == "guest" ]; then
						run_guest_fio "$host_type - $block_size - $bdev_type" --spdk_conf=/root/bdev.conf 0
					else
