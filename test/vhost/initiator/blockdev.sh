...
function prepare_fio_job_for_guest() {
        ...
        if [ $rw == "read" ] || [ $rw == "randread" ]; then
                echo "size=100m" >> $testdir/$fio_job_name
                echo "io_size=400m" >> $testdir/$fio_job_name
        fi
        if [ $rw == "read" ] || [ $rw == "randread" ]; then
                echo "[job_write]" >> $testdir/$fio_job_name
                echo "stonewall" >> $testdir/$fio_job_name
                echo "rw=write" >> $testdir/$fio_job_name
                echo "do_verify=0" >> $testdir/$fio_job_name
                echo -n "filename=" >> $testdir/$fio_job_name
                for b in $(echo $fio_bdevs | jq -r '.name'); do
                        echo -n "$b:" >> $testdir/$fio_job_name
                done
                echo "" >> $testdir/$fio_job_name
        fi
        echo "[job_$rw]" >> $testdir/$fio_job_name
        echo "stonewall" >> $testdir/$fio_job_name
        echo "rw=$rw" >> $testdir/$fio_job_name
        echo -n "filename=" >> $testdir/$fio_job_name
        for b in $(echo $fio_bdevs | jq -r '.name'); do
                echo -n "$b:" >> $testdir/$fio_job_name
        done
...
function prepare_fio_job_for_host() {
        ...
        if [ $rw == "read" ] || [ $rw == "randread" ]; then
                for b in $(echo $fio_bdevs | jq -r '.name'); do
                        echo "[job_write_$i]" >> $testdir/$fio_job_name
                        echo "rw=write" >> $testdir/$fio_job_name
                        echo "do_verify=0" >> $testdir/$fio_job_name
                        echo "filename=$b" >> $testdir/$fio_job_name
                done
                i=$((i+1))
        fi
        i=0
        for b in $(echo $fio_bdevs | jq -r '.name'); do
                echo "[job_$rw_$i]" >> $testdir/$fio_job_name
                if [ $i == 0 ]; then
                        echo "stonewall" >> $testdir/$fio_job_name
                fi
                echo "rw=$rw" >> $testdir/$fio_job_name
...
function prepare_fio_job_for_unmap() {
        ...
        echo "[trim_sequential]" >> $testdir/$fio_job_name
        echo "stonewall" >> $testdir/$fio_job_name
        echo "rw=trim" >> $testdir/$fio_job_name
        echo "trim_verify_zero=1" >> $testdir/$fio_job_name

        # Check that random TRIM/UNMAP operations 'zeroes' disk space
        echo "[trim_random]" >> $testdir/$fio_job_name
        echo "stonewall" >> $testdir/$fio_job_name
        echo "rw=randtrim" >> $testdir/$fio_job_name
        echo "trim_verify_zero=1" >> $testdir/$fio_job_name

        # Check that after TRIM/UNMAP operation disk space can be used for read
        # by using write with verify (which implies reads)
        echo "[write]" >> $testdir/$fio_job_name
        echo "stonewall" >> $testdir/$fio_job_name
        echo "rw=write" >> $testdir/$fio_job_name
}
...
function prepare_fio_job_4G() {
        ...
        fio_bdevs="$2"
        fio_job_name="$3"
        echo "size=1G" >> $testdir/$fio_job_name
        echo "io_size=4G" >> $testdir/$fio_job_name
        echo "offset=4G" >> $testdir/$fio_job_name
        if [ $rw == "read" ] || [ $rw == "randread" ]; then
                echo "[job_write]" >> $testdir/$fio_job_name
                echo "stonewall" >> $testdir/$fio_job_name
                echo "rw=write" >> $testdir/$fio_job_name
