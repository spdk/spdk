...
function run_host_fio() {
        LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev \
                --runtime=5 $testdir/bdev.fio "$2" --spdk_mem=1024
        fio_status=$?
...
function run_guest_fio() {
        ...
        run_fio+=" --ioengine=spdk_bdev --runtime=5 /root/bdev.fio $2 --spdk_mem=1024"
...
cp $testdir/bdev4g.conf.in $testdir/bdev4g.conf
sed -i "s|/tmp/vhost.0|$rootdir/../vhost/naa.Nvme0n1.2|g" $testdir/bdev4g.conf
...
fourgbdevs=$(discover_bdevs $rootdir $testdir/bdev4g.conf 5261 | jq -r '.[] | select(.claimed == false)')
...
if [ -d /usr/src/fio ]; then
...

	{
        ...
        for rw in "${fio_rw[@]}"; do
                ...
                cp $testdir/../common/fio_jobs/default_initiator.job $testdir/bdev.fio
                prepare_fio_job_4G "$rw" "$fourgbdevs"
                run_host_fio "Host +4G" --spdk_conf=$testdir/bdev4g.conf 0
