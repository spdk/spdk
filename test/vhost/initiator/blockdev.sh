...
function run_fio()
{
        LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev --iodepth=128 --bs=4k --runtime=4 $testdir/bdev.fio "$@"
        fio_status=$?
...
for block_size in 512 4096; do
        if [ $block_size == 512 ]; then
                cp $testdir/vhost.conf.malloc512 $testdir/vhost.conf.in
                if [ $RUN_NIGHTLY -eq 1 ]; then
                        format_disk_512
                fi
        else
                cp $testdir/vhost.conf.malloc4096 $testdir/vhost.conf.in
                if [ $RUN_NIGHTLY -eq 1 ]; then
                        format_disk_4096
                fi
        fi
                        ...
                        for rw in $fio_rw; do
	                        timing_enter fio_rw_verify
                                ...
                                if [ $rw == "read" ] || [ $rw == "randread" ]; then
                                        echo "size=25m" >> $testdir/bdev.fio
                                        echo "io_size=100m" >> $testdir/bdev.fio
