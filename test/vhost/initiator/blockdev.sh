...
function run_host_fio() {
        LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev --iodepth=1 --bs=192k --runtime=10 $testdir/bdev.fio "$2" --spdk_mem=1024
        fio_status=$?
...
function start_and_prepare_vm() {
	...
	vm_wait_for_boot 600 $vm_num
        ...
        set -e
	vm_wait_for_boot 600 $vm_num
	get_fio="git clone http://git.kernel.dk/fio.git fio_src "
