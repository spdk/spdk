...
function run_host_fio() {
        LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev --iodepth=128 --bs=192k --runtime=10 $testdir/bdev.fio "$2" --spdk_mem=1024
        fio_status=$?
...
function start_and_prepare_vm() {
	...
	vm_wait_for_boot 600 $vm_num
        vm_ssh $vm_num "echo 'GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash default_hugepagesz=1G hugepagesz=1G hugepages=3\"' >> /etc/default/grub"
        vm_ssh $vm_num "update-grub"
        set +e
        vm_ssh $vm_num "reboot"
        set -e
	vm_wait_for_boot 600 $vm_num
	get_fio="export http_proxy=http://proxy-mu.intel.com:911/ ; git clone http://git.kernel.dk/fio.git fio_src "
        make_fio="cd fio_src ; ./configure ; make ; make install"
        vm_ssh $vm_num "[ ! -d /root/fio_src ] && ( $get_fio ) || echo"
        vm_ssh $vm_num "$make_fio"
	...
	init_spdk+=" ./scripts/pkgdep.sh "
        ...
        pull_spdk+=" cd spdk ; git checkout -- . ; git checkout master ; git pull"
	vm_ssh $vm_num "[ -d /root/spdk ] && ( $pull_spdk ) || ( $init_spdk )"
