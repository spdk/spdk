...
function run_host_fio() {
        LD_PRELOAD=$plugindir/fio_plugin /usr/src/fio/fio --ioengine=spdk_bdev \
                --runtime=8 $testdir/bdev.fio "$2" --spdk_mem=1024
        fio_status=$?
...
function run_guest_fio() {
        ...
        run_fio+=" --ioengine=spdk_bdev --runtime=8 /root/bdev.fio $2 --spdk_mem=1024"
...
for bdev in $bdevs; do
        ...
        if [ -d /usr/src/fio ]; then
                ...
                for rw in "${fio_rw[@]}"; do
                        ...
                        run_host_fio "Host $bdev" --spdk_conf=$testdir/bdev.conf 0
