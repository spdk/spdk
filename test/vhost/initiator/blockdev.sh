...
function usage()
{
	...
	echo "    --fiopath=PATH    Path to fio directory on host [default=/usr/src/fio]"
}
...
tar --exclude="spdk.tar.gz" --exclude="*.o" --exclude="*.d" --exclude=".git" -C $FIO_PATH -zcf $ROOT_DIR/fio.tar.gz .
