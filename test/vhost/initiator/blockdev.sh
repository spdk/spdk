...
run_spdk_fio $BDEV_FIO --filename=$virtio_bdevs --section=job_randwrite --section=job_randrw \
	--section=job_write --section=job_rw --spdk_conf=$BASE_DIR/bdev.conf
timing_exit run_spdk_fio

timing_enter run_spdk_fio
run_spdk_fio $BDEV_FIO --filename=$virtio_with_unmap --section=job_randwrite --section=job_randrw \
	--section=job_unmap_trim_sequential --section=job_unmap_trim_random --section=job_unmap_write \
	--spdk_conf=$BASE_DIR/bdev.conf --spdk_conf=$BASE_DIR/bdev.conf
timing_exit run_spdk_fio
