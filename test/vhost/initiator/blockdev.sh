...
function run_host_fio() {
        ...
        local fio_bdev_conf=$2
        local expected_fio_status=$3
        LD_PRELOAD=$PLUGIN_DIR/fio_plugin fio --ioengine=spdk_bdev \
                --runtime=10 $BASE_DIR/bdev.fio "$fio_bdev_conf" --spdk_mem=1024
        fio_status=$?
        if [ $fio_status != $expected_fio_status ]; then
                echo "Test $echo_message failed."
                vm_shutdown_all
                spdk_vhost_kill
                exit 1
        fi
...
function prepare_fio_job_for_host() {
        ...
        if [ $rw == "read" ] || [ $rw == "randread" ]; then
                for b in $(echo $fio_bdevs | jq -r '.name'); do
                        echo "[job_write_$b]" >> $BASE_DIR/bdev.fio
                        echo "rw=write" >> $BASE_DIR/bdev.fio
                        echo "do_verify=0" >> $BASE_DIR/bdev.fio
...
function run_guest_bdevio() {
        vm_num="0"
        conf_file="$BASE_DIR/bdevvm.conf"
...
function error_exit() {
        set +e
        echo "Error on ${FUNCNAME[1]} - ${LINENO[1]}"
        print_backtrace > /dev/stderr
...
if [ -d /usr/src/fio ]; then
        ...
        if [ $RUN_NIGHTLY -eq 1 ]; then
                fio_rw=("read" "write" "randwrite" "randread" "rw" "randrw")
        else
                fio_rw=("read")
        fi
        ...
        for rw in "${fio_rw[@]}"; do
                ...
                run_host_fio "Host $bdevs" --spdk_conf=$BASE_DIR/bdev.conf 0
        ...
        for vbdev in $(echo $vbdevs | jq -r '.name'); do
                ...
                run_host_fio "Host unmap ${test_bdevs[i]}" --spdk_conf=$BASE_DIR/bdev.conf 0
                rm -f *.state
                rm -f $BASE_DIR/bdev.fio
                i=$((i+1))
        done
