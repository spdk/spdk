...
if [ -d /usr/src/fio ]; then
...

	{
        ...
        for rw in "${fio_rw[@]}"; do
                ...
                run_host_fio "Host $bdevs" --spdk_conf=$testdir/bdev.conf 0
        ...
        run_host_fio "Host multisocket" --spdk_conf=$testdir/bdevms.conf 0
        rm -f *.state
        rm -f $testdir/bdev.fio

        #Host test for multiqueue
        cp $testdir/../common/fio_jobs/default_initiator.job $testdir/bdev.fio
        prepare_fio_job_for_host "write" "$mqbdevs"
