...
fio_jobs_nightly=("--section=fio_job_unmap_trim_sequential --section=fio_job_unmap_trim_random --section=fio_job_unmap_write"\
	...
	"--section=fio_job_randrw --section=fio_job_randwrite --section=fio_job_rw --section=fio_job_write")
...
function create_bdev_config()
{
	...
	$RPC_PY add_vhost_scsi_lun naa.Nvme0n1.1 0 Nvme0n1p1
	$RPC_PY add_vhost_scsi_lun naa.Nvme0n1.1 1 Nvme0n1p2
	$RPC_PY add_vhost_scsi_lun naa.Nvme0n1.1 2 Nvme0n1p3
	$RPC_PY add_vhost_scsi_lun naa.Nvme0n1.1 3 Nvme0n1p4
	sed -i "s|/tmp/vhost.1|$ROOT_DIR/../vhost/naa.Nvme0n1.1|g" $BASE_DIR/bdev.conf
	...
	$RPC_PY add_vhost_scsi_lun naa."$malloc_name".3 0 $malloc_name
	sed -i "s|/tmp/vhost.3|$ROOT_DIR/../vhost/naa."$malloc_name".3|g" $BASE_DIR/bdev.conf

	bdevs=$(discover_bdevs $ROOT_DIR $BASE_DIR/bdev.conf | jq -r '.[] | select(.claimed == false)')
...
function bdevio_and_fio_guest_test()
{
	...
	run_guest_fio --spdk_conf=/root/bdev.conf "${fio_jobs[${test_type[fio]}]}"
	run_guest_fio --spdk_conf=/root/bdev.conf "${fio_jobs[${test_type[4G]}]}"
	run_guest_fio --spdk_conf=/root/bdev.conf "${fio_jobs[${test_type[unmap]}]}"
...
function fio_host_test()
{
	...
	run_host_fio --spdk_conf=$BASE_DIR/bdev.conf "${fio_jobs[${test_type[fio]}]}"
	run_host_fio --spdk_conf=$BASE_DIR/bdev.conf "${fio_jobs[${test_type[unmap]}]}"

	rm -f $BASE_DIR/fio_jobs.fio
	cp $BASE_DIR/fio_jobs.fio.in $BASE_DIR/fio_jobs.fio
	sed -i 's/\bfilename\b/&=VirtioScsi0t0:VirtioScsi1t0:VirtioScsi1t1:VirtioScsi1t2:VirtioScsi1t3/' \
		$BASE_DIR/fio_jobs.fio
