...
echo "FILL with $module (block $size, core_mask $mask, nbufs $nbufs, queue $queue $option)..."

if [ $module != "software" ]; then
	$rootdir/build/examples/accel_perf -m $mask -q $queue -w fill -o $size -C $nbufs $option -r /var/tmp/spdk.sock --wait-for-rpc &
	spdk_pid=$!
	sleep 1

	$rootdir/scripts/rpc.py $module"_scan_accel_module"
	$rootdir/scripts/rpc.py accel_assign_opc -o fill -m accel_$module
