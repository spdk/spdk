...
if [ $nvme_num -gt 0 ]; then
    nvme0_traddr=$(echo $bdfs | cut -d ' ' -f 1)
    ...
    echo $nvme_controller_result | jq -r -S '.[]' > $rootdir/test/nvmf/rpc/get_nvme_controllers_nvmf.test
    ...
    bdev_name=$(echo $nvme_controller_result | jq -r '.[] .name')
    bdev_traddr=$(echo $nvme_controller_result | jq -r '.[] .trid .traddr')
    if [ $bdev_traddr != $nvme0_traddr ]; then
