...
if [ -n "$bdfs" ]; then
	...
	lb_guid=$($rpc_py construct_lvol_bdev -u $ls_guid  lbd_nest 16000)
	...
	$rpc_py construct_nvmf_subsystem nqn.2016-06.io.spdk-cnode3 "trtype:RDMA traddr:$NVMF_FIRST_TARGET_IP trsvcid:4420" "" -a -s SPDK00000000000002 -n "$lb_nested_guid"
fi

#Test fio_plugin connect to nvmf subnqn with lvol backend
