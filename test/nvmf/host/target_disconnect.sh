...
$rootdir/examples/nvme/reconnect/reconnect -q 32 -o 4096 -w randrw -M 50 -t 20 -c 0xF \
...
if [ -n "$NVMF_SECOND_TARGET_IP" ]; then
	...
	$rootdir/examples/nvme/reconnect/reconnect -q 32 -o 4096 -w randrw -M 50 -t 20 -c 0xF \
	-r "trtype:$TEST_TRANSPORT adrfam:IPv4 traddr:$NVMF_FIRST_TARGET_IP trsvcid:$NVMF_PORT" \
	-f "trtype:$TEST_TRANSPORT adrfam:IPv4 traddr:$NVMF_SECOND_TARGET_IP trsvcid:$NVMF_PORT" &
	perfpid=$!
