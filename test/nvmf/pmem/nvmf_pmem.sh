...
# Create pmem backends on each
for i in `seq 1 $SUBSYS_NR`; do
	bdevs=""
	for c in `seq 1 $SUBSYS_NR`; do
		$rpc_py create_pmem_pool /tmp/pool_file$i$c 128 512
		bdevs+="$($rpc_py construct_pmem_bdev /tmp/pool_file$i$c) "
	done
...
$testdir/../fio/nvmf_fio.py 131072 16 randwrite $RUNTIME verify
