...
if [ $SPDK_TEST_NVME_CLI -eq 1 ]; then
	run_test "case" "nvmf_nvme_cli" test/nvmf/target/nvme_cli.sh "${TEST_ARGS[@]}"
fi
run_test "case" "nvmf_lvol" test/nvmf/target/nvmf_lvol.sh "${TEST_ARGS[@]}"
#TODO: disabled due to intermittent failures. Need to triage.
# run_test "case" "nvmf_srq_overwhelm" test/nvmf/target/srq_overwhelm.sh $TEST_ARGS
run_test "case" "nvmf_vhost" test/nvmf/target/nvmf_vhost.sh "${TEST_ARGS[@]}"
run_test "case" "nvmf_bdev_io_wait" test/nvmf/target/bdev_io_wait.sh "${TEST_ARGS[@]}"
run_test "case" "nvmf_create_transport." test/nvmf/target/create_transport.sh "${TEST_ARGS[@]}"
...
run_test "case" "nvmf_bdevperf" test/nvmf/host/bdevperf.sh "${TEST_ARGS[@]}"
run_test "case" "nvmf_identify" test/nvmf/host/identify.sh "${TEST_ARGS[@]}"
run_test "case" "nvmf_perf" test/nvmf/host/perf.sh "${TEST_ARGS[@]}"
