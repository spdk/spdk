...
if [ $RUN_NIGHTLY -eq 1 ]; then
	run_test "nvmf_fuzz_nightly" test/nvmf/target/fuzz.sh "${TEST_ARGS[@]}"
	run_test "nvmf_multiconnection_nightly" test/nvmf/target/multiconnection.sh "${TEST_ARGS[@]}"
	run_test "nvmf_initiator_timeout_nightly" test/nvmf/target/initiator_timeout.sh "${TEST_ARGS[@]}"
fi
