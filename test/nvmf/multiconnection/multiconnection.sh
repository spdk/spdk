...
HUGEMEM=12288 $rootdir/scripts/setup.sh
...
# 1 NVMe-OF subsystem per nvme bdev / lvol store / 128 lvol bdevs
# Create lvol bdevs on each lvol store
for i in `seq 0 $CONNECTION_NUMBER`; do
	$rpc_py construct_lvol_bdev -u $ls_guid lbd_$i $LVOL_BDEV_SIZE
	#lvol_bdevs+=("$lb_name")
	#$rpc_py construct_nvmf_subsystem nqn.2016-06.io.spdk:cnode$i "trtype:RDMA traddr:$NVMF_FIRST_TARGET_IP trsvcid:$NVMF_PORT" '' -a -s SPDK$i -n "$lb_name"
done
sleep 1
...
$testdir/../fio/nvmf_fio.py 4096 8 randwrite 10
sync
