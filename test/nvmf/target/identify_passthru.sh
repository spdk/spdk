...
# Expected values
nvme_serial_number=$(run_app "$SPDK_BIN_DIR/spdk_nvme_identify" -r "trtype:PCIe traddr:${bdf}" -i 0 | grep "Serial Number:" | awk '{print $3}')
nvme_model_number=$(run_app "$SPDK_BIN_DIR/spdk_nvme_identify" -r "trtype:PCIe traddr:${bdf}" -i 0 | grep "Model Number:" | awk '{print $3}')
...
# Discovered values
nvmf_serial_number=$(run_app "$SPDK_BIN_DIR/spdk_nvme_identify" -r "\
        trtype:$TEST_TRANSPORT \
        adrfam:IPv4 \
        traddr:$NVMF_FIRST_TARGET_IP \
        trsvcid:$NVMF_PORT \
        subnqn:nqn.2016-06.io.spdk:cnode1" \
	| grep "Serial Number:" | awk '{print $3}')

nvmf_model_number=$(run_app "$SPDK_BIN_DIR/spdk_nvme_identify" -r "\
        trtype:$TEST_TRANSPORT \
        adrfam:IPv4 \
        traddr:$NVMF_FIRST_TARGET_IP \
        trsvcid:$NVMF_PORT \
        subnqn:nqn.2016-06.io.spdk:cnode1" \
	| grep "Model Number:" | awk '{print $3}')
