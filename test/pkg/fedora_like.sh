...
function vm_ssh_root() {
	local tmp_sh=/tmp/$(mktemp XXXXXX.sh)

	echo "$*" | vm_ssh $vm_no "cat > $tmp_sh"
	vm_ssh $vm_no bash -ex $tmp_sh
}

function vm_ssh_user() {
	local tmp_sh=/tmp/$(mktemp XXXXXX.sh)

	echo "$*" | vm_ssh $vm_no sudo -i -u "$RPM_BUILD_USER" "cat > $tmp_sh"
	vm_ssh $vm_no sudo -i -u "$RPM_BUILD_USER" bash -ex $tmp_sh
}
