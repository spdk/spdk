...
function test_setup() {
	...
	vm_ssh_root "yum -y remove 'spdk*' 'dpdk*' || true"
}
...
function vm_ssh_user() {
	vm_ssh $vm_no sudo -i -u "$RPM_BUILD_USER" "set -ex; $*"
}
...
pkg_version=$(sed -rn 's/^Version: ([0-9.]+)$/\1/p' ${ROOT_DIR}/pkg/spdk.spec)
[[ ! -z "$pkg_version" ]]
