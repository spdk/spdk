...
INITIATOR_NAME=ANY
PORTAL_TAG=1
ISCSI_APP="$TARGET_NS_CMD ./app/iscsi_tgt/iscsi_tgt -L sock_vpp"
...
function kill_vpp() {
	...
	# FIXIT: I send kill -0 to the VPP process, however it is running in gdb and
	#        I should to wait for gdb termination.
	# killprocess $vpp_pid
	kill $vpp_pid
	sleep 5
}
