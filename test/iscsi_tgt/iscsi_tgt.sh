...
run_test suite ./test/iscsi_tgt/rpc_config/rpc_config.sh
...
if [ $SPDK_TEST_NVMF -eq 1 ]; then
	...
	run_test suite ./test/iscsi_tgt/nvme_remote/fio_remote_nvme.sh 0
	# Test configure remote NVMe device from conf file
	run_test suite ./test/iscsi_tgt/nvme_remote/fio_remote_nvme.sh 1
fi
