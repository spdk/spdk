...
if [ $RUN_NIGHTLY -eq 1 ]; then
	if [ $SPDK_TEST_PMDK -eq 1 ]; then
		run_test "iscsi_tgt_pmem_nightly" ./test/iscsi_tgt/pmem/iscsi_pmem.sh 4096 10
	fi
	run_test "iscsi_tgt_ext4test_nightly" ./test/iscsi_tgt/ext4test/ext4test.sh
	run_test "iscsi_tgt_digests_nightly" ./test/iscsi_tgt/digests/digests.sh
fi
...
if [ $RUN_NIGHTLY -eq 1 ]; then
	run_test "iscsi_tgt_multiconnection_nightly" ./test/iscsi_tgt/multiconnection/multiconnection.sh
fi
