...
function run_nvme_remote() {
	...
	if [ "$1" = "remote" ]; then
		$rpc_py -s $iscsi_rpc_addr construct_nvme_bdev "-b Nvme0 -f ipv4 -n nqn.2016-06.io.spdk:cnode1 -t rdma -s $NVMF_PORT -a $NVMF_FIRST_TARGET_IP"
	fi
