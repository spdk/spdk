...
delete_tmp_conf_files() {
	rm -f /usr/local/etc/spdk/its.conf
	rm -f /usr/local/etc/spdk/auth.conf
}
...
# Copy the calsoft config file to /usr/local/etc/spdk
mkdir -p /usr/local/etc/spdk/
cp $testdir/its.conf //usr/local/etc/spdk/
cp $testdir/auth.conf /usr/local/etc/spdk/

# Append target ip to calsoft config
echo "IP=$TARGET_IP" >> /usr/local/etc/spdk/its.conf
