...
function run_fio_malloc_test_cases() {
...
function get_first_usable_disk() {
    ...
    if [ "$device" = "/dev/" ]; then
		echo 'No usable disk found for aiobackend test'
		exit 1
	fi
	echo $device
}

function run_fio_aiobackend_test_cases() {
	timing_enter start_iscsi_tgt
	...
	$rpc_py add_portal_group 1 $TARGET_IP:$PORT
	$rpc_py add_initiator_group $INITIATOR_TAG $INITIATOR_NAME $NETMASK
	$rpc_py construct_aio_bdev $(get_first_usable_disk) AIO0 512
	# "Malloc0:0" ==> use Malloc0 blockdev for LUN0
	# "1:2" ==> map PortalGroup1 to InitiatorGroup2
...
run_fio_malloc_test_cases
run_fio_aiobackend_test_cases
