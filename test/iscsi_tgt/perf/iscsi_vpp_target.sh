...
function usage()
{
	...
	echo "    --dpdk_drv=STR    Name of the DPDK driver to bind to target NIC."
}
...
setup_vpp_iscsi
$rootdir/app/iscsi_tgt/iscsi_tgt -m 0x2 -r $testdir/rpc_iscsi.sock -c $testdir/iscsi.conf &
pid=$!
...
ssh_initiator bash -s - < $testdir/iscsi_initiator.sh --runtime=$RUNTIME --ramp_time=$RAMP_TIME --bs=$BLK_SIZE\
 --rw=$RW --rwmixread=$MIX --iodepth=$IODEPTH --cpumask=$CPUMASK --ip_tgt_int=$IP_T --ip_init_int=$IP_I\
  --numjobs=$NUM_JOBS --fiopath=$FIO_PATH
