...
def verify_add_net_interface_delete_ip_address(rpc_py):
    rpc = spdk_rpc(rpc_py)
    nics = json.loads(rpc.net_get_interfaces())
    ...
    for x in nics[:2]:
        ...
        try:
            ...
            verify(False, 1,
                   ...
                   (faked_ip, x["name"]))
    print("verify_add_net_interface_delete_ip_address passed.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test_type == 'posix':
            verify_add_net_interface_delete_ip_address(rpc_py)
