...
function test_json_config() {
	...
	$JSON_DIR/config_filter.py -method "skip_params" -filename $full_config > $base_json_config
	$clear_config_py clear_config
	$rpc_py save_config -f $tmp_config
	$JSON_DIR/config_filter.py -method "skip_params" -filename $tmp_config > $skip_params_config
	...
	$rpc_py save_config -f $tmp_config
	$JSON_DIR/config_filter.py -method "skip_params" -filename $tmp_config > $last_json_config
...
function test_params() {
	$rpc_py save_config -f $full_config
	python $JSON_DIR/config_filter.py -method "get_params" -filename $full_config > $base_json_config
	killprocess $spdk_tgt_pid
	run_spdk_tgt $full_config
	$rpc_py load_config -f $full_config
	$rpc_py save_config -f $full_config
	python $JSON_DIR/config_filter.py -method "get_params" -filename $full_config > $last_json_config
	if [ "$(diff $base_json_config $last_json_config)" != "" ]; then
