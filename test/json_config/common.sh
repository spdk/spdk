...
function create_bdev_subsystem_config() {
	...
	if [ $SPDK_TEST_CRYPTO -eq 1 ]; then
        ...
        if [ $SPDK_TEST_REDUCE -eq 1 ]; then
                $rpc_py construct_compress_bdev 8 1024 --name Malloc5
                if [ $(lspci -d:37c8 | wc -l) -eq 0 ]; then
                        $rpc_py construct_compress_bdev -b Malloc5 -c CompMalloc5 -d compress_isal
                # TODO: once QAT for comp is enabled in this series
		#else
                #       $rpc_py construct_compress_bdev -b Malloc5 -c CompMalloc5 -d compress_qat
                fi
        fi
