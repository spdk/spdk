...
function test_json_config() {
	...
	$JSON_DIR/config_filter.py -method "delete_startup_rpcs" -filename $tmp_config > $skip_params_config
...
function clean_nvmf_subsystem_config() {
	$rpc_py nvmf_subsystem_remove_host nqn.2016-06.io.spdk:cnode1 nqn.2016-06.io.spdk:host1
	$rpc_py nvmf_subsystem_remove_listener nqn.2016-06.io.spdk:cnode1 -t RDMA -a $NVMF_FIRST_TARGET_IP -s "$NVMF_PORT"
...
function clean_bdev_subsystem_config() {
	...
	$clear_config_py clear_config
	if [ $SPDK_TEST_PMDK -eq 1 ] && [ -f /tmp/pool_file1 ]; then
		rm /tmp/pool_file1
	fi
