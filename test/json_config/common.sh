...
# 1. Save bdevs
# 2. Save full json config
# 3. Clear spdk_tgt config but not startaup parameters.
# 4. Load previously saved full config
# 5. Saved current bdevs
# 6. Check if config before clear and after clear is the same
# 7. Check if bdevs saved before clear are the same with bdevs saved after clear
...
function clear_bdev_subsystem_config() {
	...
	$rpc_py delete_bdev lvs_test/snapshot0
