...
# Delete methods from spdk target config that are loading in first level
# before start_subsystem_init RPC call
def delete_startup_rpcs(filename):
    ...
    for subsystem in data['subsystems']:
        ...
        if subsystem['subsystem'] == "bdev":
            for config_method in reversed(delete_unneeded_params(
                    subsystem['config'], ["set_bdev_options"])):
                subsystem['config'].remove(config_method)
    print json.dumps(data, indent=2)


# Delete methods from spdk target config that are loading in second level
# after start_subsystem_init RPC call
def delete_runtime_rpcs(filename):
    with open(filename) as json_file:
        data = json.loads(json_file.read())
    for subsystem in data['subsystems']:
        if subsystem['subsystem'] == "iscsi":
            ...
            for config_method in reversed(delete_unneeded_params(
                ...
                subsystem['config'].remove(config_method)
        elif subsystem['subsystem'] == "bdev":
            for config_method in reversed(delete_unneeded_params(
                    subsystem['config'], ["set_bdev_options"], False)):
                subsystem['config'].remove(config_method)
        else:
...
if __name__ == "__main__":
    ...
    if args.method == "delete_startup_rpcs":
        delete_startup_rpcs(args.filename)
    if args.method == "delete_runtime_rpcs":
        delete_runtime_rpcs(args.filename)
