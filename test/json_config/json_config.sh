...
function create_bdev_subsystem_config() {
	...
	if [[ $SPDK_TEST_BLOCKDEV -eq 1 ]]; then
		...
		echo "construct_split_vbdev $lvol_store_base_bdev 2
		...
		construct_malloc_bdev 16 4096 --name Malloc1" >> $testdir/rpcs.txt
		...
		echo "construct_lvol_store -c 1048576 ${lvol_store_base_bdev}p0 lvs_test
		...
		clone_lvol_bdev        lvs_test/snapshot0 clone0" >> $testdir/rpcs.txt
	...
	if [[ $SPDK_TEST_CRYPTO -eq 1 ]]; then
		echo "construct_malloc_bdev 8 1024 --name MallocForCryptoBdev" >> $testdir/rpcs.txt
		...
		echo "construct_crypto_bdev -b MallocForCryptoBdev -c CryptoMallocBdev -d $crypto_dirver -k 0123456789123456" >> $testdir/rpcs.txt
		expected_notifications+=(
	...
	if [[ $SPDK_TEST_PMDK -eq 1 ]]; then
		...
		rm -f $pmem_pool_file
		echo "create_pmem_pool $pmem_pool_file 128 4096
		construct_pmem_bdev -n pmem1 $pmem_pool_file" >> $testdir/rpcs.txt
		expected_notifications+=( bdev_register:pmem1 )
	fi

	if [[ $SPDK_TEST_RBD -eq 1 ]]; then
		rbd_setup 127.0.0.1
		echo "construct_rbd_bdev $RBD_POOL $RBD_NAME 4096" >> $testdir/rpcs.txt
		expected_notifications+=( bdev_register:Ceph0 )
	fi
	tgt_rpc < $testdir/rpcs.txt
...
function cleanup_bdev_subsystem_config() {
	...
	if [[ $SPDK_TEST_BLOCKDEV -eq 1 ]]; then
		echo "destroy_lvol_bdev     lvs_test/clone0
		destroy_lvol_bdev     lvs_test/lvol0
		destroy_lvol_bdev     lvs_test/snapshot0
		destroy_lvol_store -l lvs_test" > $testdir/rpcs.txt
		tgt_rpc < $testdir/rpcs.txt
	fi
...
function create_vhost_subsystem_config() {
	...
	echo "construct_malloc_bdev 64 1024 --name MallocForVhost0
	...
	add_vhost_nvme_ns                 VhostNvmeCtrlr0 MallocForVhost0p6" > $testdir/rpcs.txt
...
function create_iscsi_subsystem_config() {
	timing_enter $FUNCNAME
	echo "construct_malloc_bdev 64 1024 --name MallocForIscsi0
	add_portal_group $PORTAL_TAG 127.0.0.1:$ISCSI_PORT
	add_initiator_group $INITIATOR_TAG $INITIATOR_NAME $NETMASK
	construct_target_node Target3 Target3_alias 'MallocForIscsi0:0' $PORTAL_TAG:$INITIATOR_TAG 64 -d" > $testdir/rpcs.txt
	tgt_rpc < $testdir/rpcs.txt
	timing_exit $FUNCNAME
}
...
function create_nvmf_subsystem_config() {
	...
	echo "construct_malloc_bdev 8 512 --name MallocForNvmf0
	...
	nvmf_subsystem_add_listener nqn.2016-06.io.spdk:cnode1 -t RDMA -a $NVMF_FIRST_TARGET_IP -s "$NVMF_PORT"" > $testdir/rpcs.txt
	tgt_rpc < $testdir/rpcs.txt
