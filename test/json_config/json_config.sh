...
function tgt_check_notifications() {
	...
	while read event_line; do
		...
		if (( $# == 0 )); then
			...
			rc=false
			continue
		elif [[ "$ev_type" != "$ex_ev_type" || "$ev_ctx" != "$ex_ev_ctx" ]]; then
...
function create_bdev_subsystem_config() {
	...
	if [[ $SPDK_TEST_BLOCKDEV -eq 1 ]]; then
		...
		expected_notifications+=(
			bdev_register:Malloc{0,1} bdev_register:Malloc1
			vbdev_register:${lvol_store_base_bdev}p{0, 1}
		)
		...
		expected_notifications+=(
			bdev_register:lvs_test/lvol{0,1}
			bdev_register:{_UUID_,_UUID_}
		)
	fi
	...
	if [[ $SPDK_TEST_CRYPTO -eq 1 ]]; then
		...
		expected_notifications+=( bdev_register:{MallocForCryptoBdev,_BDEV_NAME_} )
	fi
