...
echo "SPDK_TEST_BLOCKDEV=$SPDK_TEST_BLOCKDEV"
echo "SPDK_TEST_PMDK=$SPDK_TEST_PMDK"
echo "SPDK_TEST_RBD=$SPDK_TEST_RBD"
echo "SPDK_TEST_CRYPTO=$SPDK_TEST_CRYPTO"
echo "SPDK_TEST_ISCSI=$SPDK_TEST_ISCSI"
echo "SPDK_TEST_NVMF=$SPDK_TEST_NVMF"
echo "SPDK_TEST_NVMF=$SPDK_TEST_NVMF"
echo "SPDK_TEST_VHOST=$SPDK_TEST_VHOST"
echo "SPDK_TEST_VHOST_INIT=$SPDK_TEST_VHOST_INIT"
...
$rootdir/test/json_config/json_diff <(tgt_rpc save_config) "${configs_path[target]}"
if [ $SPDK_TEST_VHOST_INIT -eq 1 ]; then
	echo "INFO: Checking if virtio initiator configuration is the same..."
	$rootdir/test/json_config/json_diff <(initiator_rpc save_config) "${configs_path[initiator]}"
fi
...
if $rootdir/test/json_config/json_diff <(tgt_rpc save_config) "${configs_path[target]}" >/dev/null; then
