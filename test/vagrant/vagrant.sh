...
function usage()
{
	...
	echo "Usage: ${0##*/} [-h|--help] [-i|--install] [-n <num-cpus>] [-s <ram-size>] [-t|--test][-d|--distro=<linux-distro>]"
	...
	echo "-s                    VM memory in MB [default=$VM_MEMORY]"
...
while getopts ':n:s:d:t:h-:' optchar; do
	case "$optchar" in
		-)
		case "$OPTARG" in
			help) usage $0 && exit 0 ;;
...
$vagrantdir/create_vbox.sh -s $VM_MEMORY -n $VM_CORES $VM_DISTO
cd $vagrant_vm
...
if $RUN_AUTOTEST; then
	...
	vagrant ssh -c "cp /spdk_repo/spdk/test/vagrant/autorun-spdk.conf /spdk_repo/spdk"
	vagrant ssh -c " ./spdk_repo/spdk/autobuild.sh; sudo ./spdk_repo/spdk/autotest.sh"
