...
function usage()
{
	...
	echo "    --reactor_mask=       Set vhost reactor mask (default "0x1")"
	echo "    --master_core=        Set vhost core mask (default "0")"
...
while getopts 'xh-:' optchar; do
	case "$optchar" in
		...
		case "$OPTARG" in
			help) usage $0 ;;
			reactor_mask=*) vhost_rm="${OPTARG#*=}" ;;
			master_core=*) vhost_mc="${OPTARG#*=}" ;;
...
{
	pmem_print_tc_name "pmem_pool_info_tc1"
...
{
	pmem_print_tc_name "pmem_pool_info_tc2"
...
{
	pmem_print_tc_name "pmem_pool_info_tc3"
...
{
	pmem_print_tc_name "pmem_pool_info_tc4"
...
{
	pmem_print_tc_name "create_pmem_pool_tc1"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "create_pmem_pool_tc2"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "create_pmem_pool_tc3"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "create_pmem_pool_tc4"
	...
	pmem_clean_pool_file $TEST_DIR/test/pmem/ramspace/pool_file;

	if ! $rpc_py create_pmem_pool $TEST_DIR/test/pmem/ramspace/pool_file 32 512; then
...
{
	pmem_print_tc_name "create_pmem_pool_tc5"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "create_pmem_pool_tc6"
	pmem_clean_pool_file
	...
	do
		if (( "$i" < $pmem_min_block_size)); then
			pmem_block_size=$pmem_min_block_size
		else
			pmem_block_size=$i
		fi
		...
		if [ $pmem_block_size != $created_pmem_block_size ]; then
			error "Invalid block size of pmem pool!"
		fi
...
{
	pmem_print_tc_name "create_pmem_pool_tc7"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "create_pmem_pool_tc9"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "delete_pmem_pool_tc1"
	pmem_clean_pool_file
...
{
	pmem_print_tc_name "delete_pmem_pool_tc3"
	pmem_clean_pool_file
	pmem_create_pool_file
...
{
	pmem_print_tc_name "delete_pmem_pool_tc4"
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc1"
	pmem_clean_pool_file
	pmem_create_pool_file
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc2"
	pmem_clean_pool_file
	pmem_create_pool_file
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc3"
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc4"
	pmem_clean_pool_file $TEST_DIR/test/pmem/obj_pool_file
	pmem_create_obj_pool_file
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc5"
	pmem_clean_pool_file
	pmem_create_pool_file
...
{
	pmem_print_tc_name "construct_pmem_bdev_tc6"
	local pmem_bdev_name
...
{
	pmem_print_tc_name "delete_bdev_tc1"
	local pmem_bdev_name
	local malloc_bdev_name
	local bdevs_names
	local is_there_nvme=false
	local aio_bdev_name="aio0"
	pmem_clean_pool_file

	#check if nvme bdev has been created during vhost start
	if $rpc_py get_bdevs | jq -r '.[] .name' | grep -qi Nvme; then
		is_there_nvme=true
	fi

	malloc_bdev_name=$($rpc_py construct_malloc_bdev 32 512)
	if [ $? != 0 ]; then
		error "Failed to create Malloc bdev"
	fi

	if [ -f $TEST_DIR/test/pmem/aio_bdev_file ]; then
		rm $TEST_DIR/test/pmem/aio_bdev_file
	fi

	dd if=/dev/zero of=$TEST_DIR/test/pmem/aio_bdev_file bs=1024 count=32000
	if ! $rpc_py construct_aio_bdev $TEST_DIR/test/pmem/aio_bdev_file $aio_bdev_name 1024; then
	...
	if echo $bdevs_names | grep -qi pmem; then
		error "Pmem bdev is not deleted"
	fi

	if ! echo $bdevs_names | grep -qi Malloc; then
		error "Malloc bdev not found!"
	fi

	if ! echo $bdevs_names | grep -qi aio0; then
		error "Aio bdev not found!"
	fi

	if $is_there_nvme; then
		if ! echo $bdevs_names | grep -qi Nvme; then
			error "Nvme bdev not found!"
		fi
	fi

	if ! $rpc_py delete_bdev $aio_bdev_name; then
...
{
	pmem_print_tc_name "delete_bdev_tc2"
	pmem_clean_pool_file
	pmem_create_pool_file $default_pool_file 256 512
	local pmem_bdev_name
