...
- Call pmem_exists with missing path argument
...
Steps & expected results:
- Call pmem_exists with path argument that points to not existing file.
- Check that return code != 0 and error code = ENODEV (TODO: check that later)

#### pmem_exists_tc3
Positive test for checking pmem file.
Call with existing pmem pool file.
Steps & expected results:
- Call pmem_exists with path argument that points to existing file.
...
Call create_pmem with missing arguments.
Steps & expected results:
- call construct_pmem_bdev with correct path argument
- call exist_pmem and check if pmem file exists
- call construct_pmem_bdev with correct path to file but with no block size and total size
- call return code != 0; using get_bdevs verify that no pmem file was created
- call construct_pmem_bdev with correct path to file and total size but no block size
- call return code != 0; using get_bdevs verify that no pmem file was created
- call delete_pmem on previously created pmem
- return code = 0 and no error code
...
Steps & expected results:
- call construct_pmem_bdev with incorrect (not existing) path and
...
Negative test for constructing new pmem bdev.
Call construct_pmem_bdev with total size less than 16MB.
Steps & expected results:
- call construct_pmem_bdev with correct path argument
- call exist_pmem and check if pmem file exists
- call construct_pmem_bdev with correct, existing path and correct block size (>512)
...
- call exist_pmem and check if pmem file exists
- return code = 0, no errors
- call construct_pmem_bdev with with correct path and total size,
...
- call construct_pmem_bdev again on the same pmem file; use different
