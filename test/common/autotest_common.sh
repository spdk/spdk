...
function create_test_list() {
	grep -rshI --exclude="autotest_common.sh" --exclude="$rootdir/test/common/autotest_common.sh" -e "run_test " $rootdir | awk '{print $2, $3}' | sed 's/\"//g' > $output_dir/all_tests.txt || true
}
