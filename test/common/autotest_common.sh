...
function waitforlisten() {
	...
	for (( i=0; i<40; i++ )); do
		...
		if $have_ss_cmd; then
			...
			$ns_cmd netstat -an
			echo "=== waitforlisten: grep ==="
			$ns_cmd netstat -an | egrep "$rpc_addr" || true
			$ns_cmd netstat -an | egrep "$rpc_addr\$" || true
			$ns_cmd netstat -an | egrep "\s+$rpc_addr" || true
			...
			#1>&2 2>/dev/null
			if python $rootdir/scripts/rpc.py -t 1 -s "$rpc_addr" get_rpc_methods; then
				break
			fi
		fi
	done
