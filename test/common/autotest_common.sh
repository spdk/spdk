...
function create_test_list() {
	grep -rsh --exclude="autotest_common.sh" --exclude="$rootdir/scripts/autotest_common.sh" -e "report_test_completion" $rootdir | sed 's/report_test_completion//g; s/[[:blank:]]//g; s/"//g;' > $output_dir/all_tests.txt || true
}
