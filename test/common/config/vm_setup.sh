...
UPGRADE=false
INSTALL=false
CONF="librxe,iscsi,rocksdb,fio,flamegraph,tsocks,qemu,vpp,libiscsi,nvmecli,qat,cas"
...
function install_cas()
{
    local version="1ba0955ebc71426855b0e07af45b4cef346f9d00"
    local targetdir="/usr/src/ocf"

    if echo $CONF | grep -q cas; then
        if [ ! -d "$targetdir" ]; then
            sudo mkdir -p $targetdir
            cd $targetdir
            sudo chown "${USER}:${USER}" "$PWD"

            git clone "${GIT_REPO_CAS}" "$PWD"
            git checkout "$version"
        else
            echo "CAS already installed. Skipping"
        fi
    fi
}
...
: ${GIT_REPO_CAS=https://github.com/Open-CAS/ocf}; export GIT_REPO_CAS
