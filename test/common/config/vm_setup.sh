...
if [ $(echo $CONF | grep rocksdb) ]; then
...
if [ $(echo $CONF | grep qemu) ]; then
    ...
    if [ $(echo $CONF | grep tsocks) ]; then
        if hash tsocks 2> /dev/null; then
            ./configure --with-git='tsocks git' --prefix=/usr/local/qemu/$SPDK_QEMU_BRANCH --target-list="x86_64-softmmu" --enable-kvm --enable-linux-aio --enable-numa
        else
...
if [ $(echo $CONF | grep libiscsi) ]; then
