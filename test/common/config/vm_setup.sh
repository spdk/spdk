...
function get_package_manager()
{
    ...
    if [ $PACKAGEMNG == 'dnf' ] && [ $OSID != 'fedora' ]; then
        echo 'Located apt-get package manager, but it was tested for Fedora only'
    fi
}
...
if $INSTALL; then
    ...
    if [ $PACKAGEMNG == 'dnf' ]; then
        ...
        if ! sudo apt-get install -y libasan5; then
            echo "Latest libasan5 is not available, installing libasan2" >&2
            sudo apt-get install -y libasan2
        fi
        if ! sudo apt-get install -y libubsan1; then
            echo "Latest libubsan1 is not available, installing libubsan0" >&2
        ...
        if ! sudo apt-get install -y targetcli-fb; then
            echo "Package targetcli-fb is available at Ubuntu 18 [universe] repositorium" >&2
            false
        fi
