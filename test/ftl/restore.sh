...
$testdir/generate_configs.sh -a $device -n nvme0 -l 0-3 \
	$([ -n "$uuid" ] && echo "-u $uuid" || echo "")
$rootdir/test/app/bdev_svc/bdev_svc --max-delay=0 -c $testdir/config/ftl.conf &
svcpid=$!
...
$testdir/generate_configs.sh -a $device -n nvme0 -l 0-3 -u $(cat $testdir/.testfile_nvme0.0)
$rootdir/test/app/bdev_svc/bdev_svc --max-delay=0 -c $testdir/config/ftl.conf &
svcpid=$!
