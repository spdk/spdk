...
restore_kill() {
	rm -f $testdir/config/ftl.json || true
	rm -f $testdir/empty || true
	rm -f $testdir/testblock || true
	rm -f $testdir/testfile.md5 || true
...
# Send band worth of data in 2 steps (some data should be written to 2nd band due to metadata overhead and bad chunks)
dd if=/dev/urandom of=/dev/nbd0 bs=4K count=$(($band_size - $chunk_size))
sync
dd if=/dev/urandom of=/dev/nbd0 bs=4K count=$chunk_size seek=$(($band_size - $chunk_size))
sync
...
if kill -0 $svcpid; then
	...
	kill -9 $svcpid
	unlink /var/tmp/spdk.sock || true
	rm /var/run/spdk_bdev-1 || true
else
	exit
fi
...
# Wait until bdev_svc starts
waitforlisten $svcpid
if ! $rpc_py -t 1 rpc_get_methods &>/dev/null; then
	sleep 1
	waitforlisten $svcpid
fi

# Ftl should recover, though with a loss of data (-r config option)
