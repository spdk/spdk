...
run_test suite $testdir/bdevperf.sh $device basic
...
if [ $SPDK_TEST_FTL_EXTENDED -eq 1 ]; then
	...
	timing_enter bdevperf_extended
	run_test suite $testdir/bdevperf.sh $device extended $uuid
	timing_exit bdevperf_extended
fi
