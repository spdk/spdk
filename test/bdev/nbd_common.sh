...
function nbd_start_disks_optional() {
	local rpc_server=$1
	local bdev_list=($2)
	local i
	local nbd_device
...
function nbd_stop_disks_optional() {
	local rpc_server=$1
	local nbd_disk

	nbd_disks_json=`$rootdir/scripts/rpc.py -s $rpc_server get_nbd_disks`
	nbd_disks_name=`echo "${nbd_disks_json}" | jq -r '.[] | .nbd_device'`
	for nbd_disk in ${nbd_disks_name}; do
		$rootdir/scripts/rpc.py -s $rpc_server stop_nbd_disk $nbd_disk
		waitfornbd_exit $(basename $nbd_disk)
	done
}
...
function nbd_rpc_operation_verify() {
	local rpc_server=$1
	local bdev_list=($2)

	nbd_start_disks_optional $rpc_server "${bdev_list[*]}"

	nbd_stop_disks_optional $rpc_server
