...
function fio_test_suite() {
	...
	if [ $RUN_NIGHTLY -eq 0 ]; then
		...
		run_test "bdev_fio_rw_verify_ext_nightly" fio_bdev $fio_ext_params --spdk_mem=$PRE_RESERVED_MEM \
		--output=$output_dir/blockdev_fio_verify.txt
	fi
	rm -f ./*.state
	...
	if [ $RUN_NIGHTLY -eq 0 ]; then
		...
		run_test "bdev_fio_trim_ext_nightly" fio_bdev $fio_ext_params --output=$output_dir/blockdev_trim.txt
	fi
...
if [ -d /usr/src/fio ] && [ "$test_type" != "nvme" ]; then
...
# if [ $RUN_NIGHTLY -eq 1 ]; then
	# run_test "bdev_gpt_reset_nightly" $testdir/bdevperf/bdevperf -c $conf_file -q 16 -w reset -o 4096 -t 60
# fi
