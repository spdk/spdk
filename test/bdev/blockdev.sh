...
function get_io_result() {
	...
	# To avoid overflow as bandwidth is in byte
	run_time_in_sec=$(((ticks_after-ticks_before)/tick_rate))
	echo $(((io_result_after-io_result_before)/run_time_in_sec))
}
...
function qos_function_test() {
	...
	if [ $iops_limit -gt $qos_lower_iops_limit ]; then
		...
		bw_limit=$((bw_limit/1024/1024/10))
