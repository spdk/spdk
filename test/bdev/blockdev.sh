...
QOS_DEV="Null_0"
QOS_RUN_TIME=5
QOS_RESULT_PER_SEC=0

function qos_get_result() {
	local limit_type=$1
	...
	QOS_RESULT_PER_SEC=$(((io_result_after-io_result_before)/QOS_RUN_TIME))
}
...
function qos_run_test() {
	...
	if [ $QOS_RESULT_PER_SEC -lt $lower_limit ] || [ $QOS_RESULT_PER_SEC -gt $upper_limit ]; then
...
function qos_check_test() {
	...
	io_result=$(((QOS_RESULT_PER_SEC)/QOS_RUN_TIME/4))
