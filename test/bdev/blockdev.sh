...
function check_qos_results() {
	local qos_limit=$1
	local limit_type=$2

	io_result=$($rpc_py -s $QOS_RPC_SERVER get_bdevs_iostat -b Raid0)
	...
	if [ $RUN_QOS = false ]; then
		...
		retval=$(echo "$qos_limit*0.9 < $io_result && $io_result < $qos_limit*1.1" | bc)
		if [ $retval -eq 0 ]; then
...
check_qos_results $MIN_IOPS_LIMIT $LIMIT_TYPE
if [ $RUN_QOS = true ]; then
	...
	check_qos_results $IOPS_LIMIT $LIMIT_TYPE
	...
	check_qos_results $BANDWIDTH_LIMIT $LIMIT_TYPE
	...
	check_qos_results $READ_BANDWIDTH_LIMIT $LIMIT_TYPE
fi
