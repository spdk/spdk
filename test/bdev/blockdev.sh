...
function fio_test_suite() {
	...
	if [ $RUN_NIGHTLY -eq 0 ] && [ -f $testdif/bdev.fio ]; then
...
run_test "case" "bdev_hello_world" $rootdir/examples/bdev/hello_world/hello_bdev -c $conf_file -b $hello_world_bdev
run_test "case" "bdev_bounds" bdev_bounds
run_test "case" "bdev_nbd" nbd_function_test $conf_file "$bdevs_name"
...
run_test "case" "bdev_verify" $testdir/bdevperf/bdevperf -c $conf_file -q 128 -o 4096 -w verify -t 5
run_test "case" "bdev_write_zeroes" $testdir/bdevperf/bdevperf -c $conf_file -q 128 -o 4096 -w write_zeroes -t 1
...
# if [ $RUN_NIGHTLY -eq 1 ]; then
	# run_test "case" "bdev_gpt_reset_nightly" $testdir/bdevperf/bdevperf -c $conf_file -q 16 -w reset -o 4096 -t 60
# fi
