...
RUN_QOS=true
BDEV_PERF="$testdir/bdevperf/bdevperf -m 0xF -c $testdir/bdev_qos.conf -q 128 -o 4096 -w randread -t 5"

# Run bdevperf with QoS disabled first
check_qos_works_well false $IOPS_LIMIT IOPS
...
EOL
	check_qos_works_well true $IOPS_LIMIT IOPS
...
EOL
	check_qos_works_well true $BANDWIDTH_LIMIT BANDWIDTH
...
EOL
	check_qos_works_well true $READ_BANDWIDTH_LIMIT BANDWIDTH
fi
