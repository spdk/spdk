...
function run_qos_test() {
	...
	io_result=$($rpc_py -s $QOS_RPC_SERVER bdev_get_iostat -b Raid0)
	...
	if [ $limit_type = IOPS ]; then
		...
		io_result=$(((io_result_after-io_result_before)/RUN_TIME))
	else
	...
	if [ $RUN_QOS = false ]; then
		...
		# Once min IOPS limit (1000) can hit, the min bandwidth limit (1MB) can also hit as I/O size is 4K
		io_result=$((io_result/2))
		if [ $io_result -gt $MIN_IOPS_LIMIT ]; then
			RUN_QOS=true
			IOPS_LIMIT=$((io_result/MIN_IOPS_LIMIT*MIN_IOPS_LIMIT))
		else
			echo "$io_result without rate limiting is less than min $qos_limit - exit QoS testing"
		fi
	else
		down_limit=$(echo "$qos_limit 0.85" | awk '{printf("%i",$1*$2)}')
		up_limit=$(echo "$qos_limit 1.15" | awk '{printf("%i",$1*$2)}')
...
run_qos_test $MIN_IOPS_LIMIT $LIMIT_TYPE
if [ $RUN_QOS = true ]; then
	# Run bdevperf with IOPS rate limit
	$rpc_py -s $QOS_RPC_SERVER bdev_set_qos_limit --rw_ios_per_sec $IOPS_LIMIT Raid0
	run_qos_test $IOPS_LIMIT $LIMIT_TYPE

	# Run bdevperf with IOPS and bandwidth rate limits
	LIMIT_TYPE=BANDWIDTH
	$rpc_py -s $QOS_RPC_SERVER bdev_set_qos_limit --rw_mbytes_per_sec $BANDWIDTH_LIMIT Raid0
	run_qos_test $BANDWIDTH_LIMIT $LIMIT_TYPE

	# Run bdevperf with additional read only bandwidth rate limit
	$rpc_py -s $QOS_RPC_SERVER bdev_set_qos_limit --r_mbytes_per_sec $READ_BANDWIDTH_LIMIT Raid0
	run_qos_test $READ_BANDWIDTH_LIMIT $LIMIT_TYPE
fi
