...
function scanbuild_make {
	...
	sed -n '/Leaving directory /,$p' $out/build_output.txt | grep "CC" | sed 's/\s\sCC\s//g' | sed 's/\.o//g' > $out/built_c_files.txt
	paste -d '\n' $out/all_c_files.txt $out/built_c_files.txt | grep -v "^test"| sort | uniq -u > $out/unbuilt_c_files.txt
...
if [ "$SPDK_TEST_AUTOBUILD" -eq 1 ]; then
	...
	run_test "make" ./configure $config_params && $MAKE $MAKEFLAGS
fi
