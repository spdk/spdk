...
# synlinks to tools
mkdir -p %{sbindir}
ln -sf -r %{datadir}/scripts/rpc.py %{sbindir}/spdk-rpc
ln -sf -r %{datadir}/scripts/spdkcli.py %{sbindir}/spdk-cli


# Install examples
find examples -regextype egrep -type f -regex '.*([.][ch]|Makefile|.*.conf|.*[.]md)' \
	-exec cp --parents -t %{datadir} {} +

cp -r --parents -t %{datadir} mk
