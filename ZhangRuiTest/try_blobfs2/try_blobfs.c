...
static void writefile()
{
	...
	f = fopen("smallfile", "r");
	set_channel();
	rc = spdk_fs_open_file(g_fs, g_sync_args.channel, "smallfile", SPDK_BLOBFS_OPEN_CREATE, &g_file);
	while ((rc = fread(buf, sizeof(unsigned char), MAXLEN, f)) != 0) {

		res = spdk_file_write(g_file, g_sync_args.channel, buf, index, rc);
		if (res < 0) {
...
cache_read_after_write(void)
{
	uint64_t length;
	int rc, fd;
	char w_buf[100] = "hello world", r_buf[2000] = "zhangrui";
	uint64_t filelength;
	int now = 0, eachtime = 1024, remain;
	...
	remain = filelength;
	while (remain > 0) {
		eachtime = remain > eachtime ? eachtime : remain;
		rc = spdk_file_read(g_file, g_sync_args.channel, r_buf, now, eachtime);
		if (rc < 0) {
			printf("read error %s\n", strerror(rc));
			scanf("%s", r_buf);
		}
