...
class UIRoot(UINode):
    ...
    def get_bdevs(self, bdev_type):
        # Following replace needs to be done in order for some of the bdev listings to work.
    ...
    def _get_new_bdev(self, name):
        new_bdev = rpc.bdev.get_bdevs(self.client, an(name=name))
        self.current_bdevs.append(new_bdev[0])
        if new_bdev[0]["aliases"]:
    ...
    def create_malloc_bdev(self, **kwargs):
        ...
        return self._get_new_bdev(response[0])
    ...
    def remove_vhost_controller(self, **kwargs):
        ...
        for ctrlr in self.current_vhost_ctrls:
            if ctrlr["ctrlr"] == kwargs["ctrlr"]:
                self.current_vhost_ctrls.remove(ctrlr)
                return kwargs["ctrlr"]
    ...
    def create_vhost_scsi_controller(self, **kwargs):
        ...
        for ctrlr in ctrlrs:
            if ctrlr["ctrlr"] == kwargs["ctrlr"]:
                self.current_vhost_ctrls.append(ctrlr)
                return kwargs["ctrlr"]
