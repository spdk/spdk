...
class RTSRoot(Node):
    ...
    def list_bdevs(cls):
        ...
        RTSRoot.current_bdevs = rpc_bdev.get_bdevs(args)

    @classmethod
    def create_malloc_bdev(cls, **kwargs):
        print(kwargs)
        args = argparse.Namespace(client=cls.client, **kwargs)
        test = rpc_bdev.construct_malloc_bdev(args)
        args = argparse.Namespace(client=cls.client, name=test[0])
        new_bdev = rpc_bdev.get_bdevs(args)
        cls.current_bdevs.append(new_bdev[0])

    @classmethod
    def get_bdevs(cls, bdev_type):
        print("ROOT CURRENT BDEVS OF TYPE %s:" % bdev_type)
