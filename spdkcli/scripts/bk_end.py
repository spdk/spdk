...
class BKRoot():
    ...
    @classmethod
    def list_bdevs(cls):
        BKRoot.current_bdevs = rpc_bdev.get_bdevs(cls.client, an(name=""))

    @classmethod
    def list_lvols(cls):
        BKRoot.current_lvol_stores = rpc_lvol.get_lvol_stores(cls.client,
                                                              an(lvs_name="", uuid=""))
    ...
    def create_malloc_bdev(cls, **kwargs):
        response = rpc_bdev.construct_malloc_bdev(cls.client, an(**kwargs))
        new_bdev = rpc_bdev.get_bdevs(cls.client, an(name=response[0]))
        cls.current_bdevs.append(new_bdev[0])
        return new_bdev[0]["name"]

    @classmethod
    def create_aio_bdev(cls, **kwargs):
        response = rpc_bdev.construct_aio_bdev(cls.client, an(**kwargs))
        new_bdev = rpc_bdev.get_bdevs(cls.client, an(name=response[0]))
        cls.current_bdevs.append(new_bdev[0])
        return new_bdev[0]["name"]
    ...
    def create_lvol_bdev(cls, **kwargs):
        response = rpc_lvol.construct_lvol_bdev(cls.client, an(**kwargs))
        new_bdev = rpc_bdev.get_bdevs(cls.client, an(name=response[0]))
        cls.current_bdevs.append(new_bdev[0])
    ...
    def delete_bdev(cls, name):
        rpc_bdev.delete_bdev(cls.client, an(bdev_name=name))

    @classmethod
    def create_lvol_store(cls, **kwargs):
        response = rpc_lvol.construct_lvol_store(cls.client, an(**kwargs))
        new_lvs = rpc_lvol.get_lvol_stores(cls.client, an(uuid=response[0], lvs_name=None))
        cls.current_lvol_stores.append(new_lvs[0])
        return new_lvs[0]["name"]

    @classmethod
    def delete_lvol_store(cls, **kwargs):
        rpc_lvol.destroy_lvol_store(cls.client, an(**kwargs))
