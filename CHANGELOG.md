...
EXPERIMENTAL: New "libreduce" library implementing block compression scheme based on using SSDs
for storing compressed blocks of storage and persistent memory for metadata. The scheme is generic
and not tied to any specific block device framework such as the SPDK block device (bdev) framework.
Higher-level software modules can built on top of this library to create and present block devices
in a specific block device framework. In the future, a bdev_reduce module will serve as a wrapper
around the libreduce library, to present the compressed block devices as an SPDK bdev.
See the [documentation](https://spdk.io/doc/compression.html) for more details.
