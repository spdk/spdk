...
The logic which support hotplug of vfio-attached devices has been implemented in SPDK, but compare
with uio driver, vfio-pci driver does not trigger "remove" uevent at the very beginning of the remove
interface of pci driver, it will wait until the application release the device fd. But for SPDK, the
application will wait for the event and then release the device fd, this is a "logical dead lock". To
solve this issue, we add a new approch which will help on detect the hot remove event for vfio-attached
devices, we encourage user to add a timer in the application which will read the nvme register, if
the device hot removed, the register will be all "F", then the upper level application know that the
device is removed.
