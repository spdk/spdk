...
Shared receive queue can now be disabled even for NICs that support it using the configuration
file parameter `NoSrq` and `nvmf_create_transport` RPC method parameter `no_srq`. The actual
use of a shared receive queue is predicated on hardware support when this flag is not used.
