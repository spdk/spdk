...
2. Run fio test with sequential jobs: trim, randtrim, write.
   All this jobs run with verification enabled. We verify trim to check if zeros has been set.
   We run write after trim to check if we still can write and read from device.
