...
nvme_active_ns_ctx_create(struct spdk_nvme_ctrlr *ctrlr, struct nvme_completion_poll_status *status,
			  nvme_ctrlr_active_ns_cb cb_fn, void *cb_arg)
{
...
bool
nvme_ctrlr_identify_ns_error_is_fatal(struct spdk_nvme_ctrlr *ctrlr,
				      const struct spdk_nvme_cpl *cpl)
{
	assert(spdk_nvme_cpl_is_error(cpl));

	/* A namespace becoming inactive/invalid during NVMe controller initialization should not be
	...
	if (cpl->status.sct == SPDK_NVME_SCT_GENERIC &&
		...
		NVME_CTRLR_DEBUGLOG(ctrlr, "Identify command unsupported or inactive/invalid namespace.\n");
		return false;
	}
...
nvme_ctrlr_identify_active_ns_async_done(struct nvme_active_ns_ctx *ctx)
{
	...
	if (ctx->cb_fn) {
		ctx->cb_fn(ctrlr, ctx->cb_arg, status);
	}
...
static void
nvme_ctrlr_init_identify_active_ns_finish(struct spdk_nvme_ctrlr *ctrlr, void *cb_arg, int status)
{
	struct spdk_nvme_ns *ns;
...
nvme_ctrlr_init_identify_active_ns(struct spdk_nvme_ctrlr *ctrlr)
{
	...
	ctx = nvme_active_ns_ctx_create(ctrlr, NULL, nvme_ctrlr_init_identify_active_ns_finish, NULL);
	if (!ctx) {
		nvme_ctrlr_set_state(ctrlr, NVME_CTRLR_STATE_ERROR, NVME_TIMEOUT_INFINITE);
		return;
	}
...
nvme_ctrlr_identify_active_ns(struct spdk_nvme_ctrlr *ctrlr)
{
	...
	ctx = nvme_active_ns_ctx_create(ctrlr, status, NULL, NULL);
	if (!ctx) {
		spdk_free(status);
		return -ENOMEM;
	}
