...
void nvme_ctrlr_reset(struct spdk_nvme_ctrlr *ctrlr) {
	...
	/* Call the actual SPDK reset function */
	int rc = spdk_nvme_ctrlr_reset(ctrlr);
    
	if (rc != 0) {
		NVME_CTRLR_WARNLOG(ctrlr, "Controller reset failed with error code: %d\n", rc);
	} else {
		NVME_CTRLR_WARNLOG(ctrlr, "Controller reset successfully completed.\n");
	}
}
