...
spdk_app_json_config_load_subsystem_config_entry(void *_ctx)
{
	...
	/* We will never get NULL here as spdk_json_next will point to the next JSON value
	 * what ever it is. Just go back by one element to get "params" _END position.
	 */
	params_end = cfg.params + spdk_json_val_len(cfg.params);

	params_len = params_end->start - cfg.params->start;
