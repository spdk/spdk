...
typedef void (*client_resp_handler)(struct load_json_config_ctx *, struct spdk_jsonrpc_client_response *);
...
rpc_client_poller(void *arg)
{
	...
	if (resp->error) {
		SPDK_ERRLOG("error response: %*s", resp->error->len, (char *)resp->error->start);
		spdk_jsonrpc_client_free_response(resp);
		ctx->client_rc = -EINVAL;
		spdk_app_json_config_load_done(ctx, ctx->client_rc);
	} else {
		...
		if (cb == NULL) {
			assert(false);
			/* If there is no handler for response there is osmething wrong there */
...
static void
spdk_app_json_config_load_subsystem_config_entry_next(struct load_json_config_ctx *ctx, struct spdk_jsonrpc_client_response *resp)
{
...
spdk_app_json_config_load_subsystem_config_entry(void *_ctx)
{
	...
	if (rc == -EPERM) {
		SPDK_WARNLOG("Method '%s' not allowed -> skipping\n", cfg.method);
		/* Invoke later to avoid recurency */
		ctx->config_it = spdk_json_next(ctx->config_it);
...
static void
subsystem_init_done_resp_cb(struct load_json_config_ctx *ctx, struct spdk_jsonrpc_client_response *resp)
{
...
spdk_app_json_config_load_subsystem(void *_ctx)
{
	...
	if (ctx->subsystems_it == NULL) {
		if (spdk_rpc_get_state() == SPDK_RPC_STARTUP) {
			SPDK_WARNLOG("=== Calling 'start_subsystem_init' ===\n");
