...
struct load_json_config_ctx {
	/* LCore & thread used during configuration */
	uint32_t lcore;
...
{
	assert(spdk_env_get_current_core() == ctx->lcore);

	if (ctx->client_conn_poller) {
...
static struct spdk_json_object_decoder jsonrpc_cmd_decoders[] = {
	{"method", offsetof(struct config_entry, method), cap_string},
	{"params", offsetof(struct config_entry, params), cap_object, true}
};

/* TODO: make these names shorter */
static int _load_subsystem_config_entry_resp_poll(void *_ctx);
...
spdk_app_json_config_load_subsystem_config_entry(void *_ctx)
{
	...
	if (ctx->config_it == NULL) {
		SPDK_WARNLOG("Subsystem '%.*s': configuration done.\n", ctx->subsystem_name->len,
	...
	if (spdk_json_decode_object(ctx->config_it, jsonrpc_cmd_decoders,
		...
		SPDK_ERRLOG("Failed to method\n");
		spdk_app_json_config_load_done(ctx, -EINVAL);
		return;
	}

	/* TODO: check if methods is allowed. Just find "method" in conig_it
	 * and compare vs currently allowed.
	 */

	/* We will never get NULL here as next will point to the next JSON value
	 * what ever it is. Just go back by one element to get "params" _END position.
	 */
	params_end = spdk_json_next(cfg.params);
	params_end--;

	params_len = params_end->start - cfg.params->start + 1;
	SPDK_WARNLOG("\tmethod: %.*s\n", cfg.method->len, (char *)cfg.method->start);
	SPDK_WARNLOG("\tparams: %.*s\n", (int)params_len, (char *)cfg.params->start);
	...
	spdk_json_write_named_string_fmt(w, "method", "%.*s", (int)cfg.method->len,
					 (char *)cfg.method->start);
	...
	assert(ctx->client_conn_poller == NULL);
	ctx->client_conn_poller = spdk_poller_register(_load_subsystem_config_entry_resp_poll, ctx, 100);
}

static int
_load_subsystem_config_entry_resp_poll(void *_ctx)
{
	struct load_json_config_ctx *ctx = _ctx;
	struct spdk_jsonrpc_client_response *resp;

	ctx->client_rc = spdk_jsonrpc_client_poll(ctx->client_conn);
	if (ctx->client_rc == -EAGAIN) {
		/* FIXME: add timeout */
		/* No response yet */
		return -1;
	}
	...
	if (ctx->client_rc != 0 && ctx->client_rc != -EAGAIN) {
		/* Something went wrong */
		spdk_app_json_config_load_done(ctx, ctx->client_rc);
	}

	resp = spdk_jsonrpc_client_get_response(ctx->client_conn);
	if (resp->error) {
		/* TODO: add some message about what option failed */
		spdk_app_json_config_load_done(ctx, -EINVAL);
	} else {
		/* Don't care the reponse as long as it is not and error */
...
spdk_app_json_config_load_subsystem(void *_ctx)
{
	...
	SPDK_WARNLOG("Loading subsystem '%.*s' configuration\n", ctx->subsystem_name->len,
		     (char *)ctx->subsystem_name->start);
...
static int
spdk_app_json_config_client_connect_poller(void *_ctx)
{
	struct load_json_config_ctx *ctx = _ctx;

	ctx->client_rc = spdk_jsonrpc_client_poll(ctx->client_conn);
	/* FIXME: Add connection timeout */
	if (ctx->client_rc == -ENOTCONN) {
		/* Still connecting */
		return -1;
	}
	...
	if (ctx->client_rc == 0 || ctx->client_rc == -EAGAIN) {
		/* We are connected, start loading subsystems. */
...
/* Bottom half */
static void
spdk_app_json_config_load_ev(void *_ctx, void *_unused)
{
	struct load_json_config_ctx *ctx = _ctx;
	int rc;
	...
	ctx->client_conn_poller = spdk_poller_register(spdk_app_json_config_client_connect_poller, ctx,
				  100);
...
void
spdk_app_json_config_load(const char *json_path, struct spdk_event *done_event)
{
	...
	ctx->lcore = spdk_env_get_current_core();
	/* thread will be set in event */
	...
	/* Get first subsystem
	 *
	 * TODO: iterate or at least check config using SPDK subsystems order not the configurations order.
	 */
	...
	/* Invoke on poller context so the thread handler can be obtained. */
	spdk_event_call(spdk_event_allocate(ctx->lcore, spdk_app_json_config_load_ev, ctx, NULL));
	return;
