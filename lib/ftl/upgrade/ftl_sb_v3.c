...
static bool
superblock_v3_md_region_overflow(struct spdk_ftl_dev *dev,
...
static int
superblock_v3_md_layout_add(struct spdk_ftl_dev *dev, struct ftl_superblock_v3_md_region *sb_reg,
			    uint32_t reg_type, uint32_t reg_version, uint64_t blk_offs, uint64_t blk_sz)
{
	if (superblock_v3_md_region_overflow(dev, sb_reg)) {
...
static int
superblock_v3_md_layout_add_free(struct spdk_ftl_dev *dev,
				 struct ftl_superblock_v3_md_region **sb_reg,
				 uint32_t reg_type, uint32_t free_type, uint64_t total_blocks)
{
	...
	if (superblock_v3_md_layout_add(dev, *sb_reg, free_type, 0,
...
int
ftl_superblock_v3_md_layout_build(struct spdk_ftl_dev *dev)
{
	...
	for (; n < FTL_LAYOUT_REGION_TYPE_MAX;) {
		...
		if (superblock_v3_md_layout_add(dev, sb_reg, reg->type, reg->current.version,
	...
	if (is_empty) {
		superblock_v3_md_layout_add_free(dev, &sb_reg, FTL_LAYOUT_REGION_LAST_NVC,
						 FTL_LAYOUT_REGION_TYPE_FREE_NVC, layout->nvc.total_blocks);

		superblock_v3_md_layout_add_free(dev, &sb_reg, FTL_LAYOUT_REGION_LAST_BASE,
...
static void
ftl_superblock_v3_md_layout_free_region(struct spdk_ftl_dev *dev,
					struct ftl_superblock_v3_md_region *sb_reg)
{
	/* Major upgrades are not implemented in v3 */
	sb_reg->type = FTL_LAYOUT_REGION_TYPE_FREE_NVC;
}

int
ftl_superblock_v3_md_layout_upgrade_region(struct spdk_ftl_dev *dev,
		struct ftl_layout_region *reg, uint32_t new_version)
{
	struct ftl_superblock_v3 *sb = (struct ftl_superblock_v3 *)dev->sb;
	struct ftl_superblock_v3_md_region *sb_reg = reg->prev.sb_md_reg;
	struct ftl_superblock_v3_md_region *sb_reg_iter = &sb->md_layout_head;
	...
	while (sb_reg_iter->type != FTL_LAYOUT_REGION_TYPE_INVALID) {
		...
		if (sb_reg_iter->version == new_version) {
			...
			ftl_superblock_v3_md_layout_free_region(dev, sb_reg);
			goto exit;
		}
...
next_sb_reg_iter:
		...
		if (superblock_v3_md_region_overflow(dev, sb_reg_iter)) {
			FTL_ERRLOG(dev, "Buffer overflow\n");
			return -EOVERFLOW;
		}
	}
