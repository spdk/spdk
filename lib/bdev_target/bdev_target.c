...
/* spdk target and channel structures */
...
struct spdk_bdev_aio_req {
	...
	void* private_data; // user specific record to a request
	int req_rc; // result of spdk internal calling functions
	struct spdk_bdev_ret ret; // result from the nvme device
...
/* spdk-env-init-per-thread start */
...
_spdk_env_thread_init(void)
{
	...
	g_spdk_thread_env_args.thd = spdk_allocate_thread(_spdk_send_msg, NULL,
			NULL, NULL, "spdk_bt_thread");
...
/* spdk-env-init-per-thread end */

/* spdk channel operation start */
...
spdk_env_get_io_channel(struct spdk_bdev_target *bt)
{
	...
	for (i = 0; i < SPDK_BT_MAX_NUM; i++) {
		...
		/* Check whether it is a stale channel for reopen */
...
int
spdk_bdev_aio_ctx_setup(struct spdk_bdev_aio_ctx **_ctx, struct spdk_bdev_target *bt)
{
	struct spdk_bt_io_channel *bt_io_channel;
	struct spdk_io_channel *bt_spdk_io_channel;
	struct spdk_bdev_aio_ctx *ctx = NULL;
	int rc;
	...
	rc = pthread_spin_init(&ctx->ctx_lock, 0);
	if (rc != 0) {
		return rc;
	}
...
__aio_ctx_get_reqs(void *_args)
{
	...
	/* check whether get_reqs notify is already registered */
	...
	SPDK_DEBUGLOG(SPDK_LOG_BT, "Completed request number is %d\n", nreqs);
	for (i = 0; i < nreqs; i++) {
...
{
	SPDK_DEBUGLOG(SPDK_LOG_BT, "Trigger sem\n");
	sem_t *sem = cb_arg;
	sem_post(sem);
}
		...
		struct timespec *timeout)
{
	...
	if (ctx == NULL || reqs == NULL) {
		return -1;
	}

	if (timeout != NULL && (timeout->tv_nsec != 0 && timeout->tv_sec != 0)) {
		SPDK_DEBUGLOG(SPDK_LOG_BT, "Timeout is not implemented\n");
		return -1;
	}
	...
	sem_wait(&sem);
	SPDK_DEBUGLOG(SPDK_LOG_BT, "Get reqs triggered\n");
...
int
spdk_bdev_aio_ctx_destroy(struct spdk_bdev_aio_ctx *ctx)
{
	int rc = 0;
...
_bdev_aio_ctx_req_complete(void *arg, int bterrno, struct spdk_bdev_ret *nvm_ret)
{
	...
	/* Check if there is a get_reqs waiting for req's completion */
	if (ctx->get_reqs == NULL) {
		return;
	}
	...
	ctx->get_reqs->get_reqs_rc = nreqs;
...
/* aio_ctx_submit end */
int
spdk_bdev_aio_req_size(void)
{
	return sizeof(struct spdk_bdev_aio_req);
}

struct spdk_bdev_ret *
spdk_bdev_aio_req_get_ret(struct spdk_bdev_aio_req *req)
{
	return &req->ret;
}

void
spdk_bdev_aio_req_set_private_arg(struct spdk_bdev_aio_req *req, void *private_arg)
{
	req->private_data = private_arg;
}

void *
spdk_bdev_aio_req_get_private_arg(struct spdk_bdev_aio_req *req)
{
	return req->private_data;
}

/* async req funcs start */
static void
__passthru_from_ot(void *_args)
{
	...
	SPDK_DEBUGLOG(SPDK_LOG_BT, "bdev target process passthru cmd (opc is %d)\n", req->op.passthru.cmd.opc);
...
void
spdk_bdev_aio_req_set_admin_passthru(struct spdk_bdev_aio_req *req,
		struct spdk_nvme_cmd *cmd, void *pin_buf, size_t data_len)
{
	...
	SPDK_DEBUGLOG(SPDK_LOG_BT, "req %p passthru fn %p\n", req, req->queue_req_fn);
	req->user_complete_cb = NULL;
}

void
spdk_bdev_aio_req_set_io_passthru(struct spdk_bdev_aio_req *req, struct spdk_nvme_cmd *cmd,
...
/* directly callback req funcs start */
void
...
int
spdk_bdev_aio_cb_submit(struct spdk_bdev_target *bt, int nr, struct spdk_bdev_aio_req *reqs[])
{
	SPDK_DEBUGLOG(SPDK_LOG_BT, "cb_submit is not implemented\n");
	return -1;
}
...
int
spdk_bdev_req_admin_passthru_sync(struct spdk_bdev_target *bt, struct spdk_nvme_cmd *cmd,
		void *pin_buf, size_t data_len, struct spdk_bdev_ret *ret)
{
	...
	spdk_bdev_aio_req_set_admin_passthru(req, cmd, pin_buf, data_len);
...
int
spdk_bdev_req_io_passthru_sync(struct spdk_bdev_target *bt, struct spdk_nvme_cmd *cmd,
		void *pin_buf, size_t data_len, void *pin_meta, size_t md_len, struct spdk_bdev_ret *ret)
{
	...
	spdk_bdev_aio_req_set_io_passthru(req, cmd, pin_buf, data_len, pin_meta, md_len);
	spdk_bdev_aio_req_set_cb(req, _spdk_bt_io_cb_sync, &sem);
	reqs[0] = req;
