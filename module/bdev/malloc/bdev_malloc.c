...
malloc_verify_pi(struct spdk_bdev_io *bdev_io) {
    ...
    if (rc != 0) {
        SPDK_ERRLOG("DIF/DIX verify failed: lba %"
        PRIu64
        ", num_blocks %"
        PRIu64
        ", "
...
static TAILQ_HEAD(, malloc_disk

)
g_malloc_disks = TAILQ_HEAD_INITIALIZER(g_malloc_disks);
...
SPDK_BDEV_MODULE_REGISTER(malloc,
...
bdev_malloc_readv(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
    ...
    SPDK_DEBUGLOG(bdev_malloc, "read %zu bytes from offset %#"
    PRIx64
    ", iovcnt=%d\n",
    ...
    SPDK_DEBUGLOG(bdev_malloc, "read metadata %zu bytes from offset%#"
    PRIx64
    "\n",
            md_len, md_offset);
...
bdev_malloc_writev(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
    ...
    SPDK_DEBUGLOG(bdev_malloc, "wrote %zu bytes to offset %#"
    PRIx64
    ", iovcnt=%d\n",
            len, offset, bdev_io->u.bdev.iovcnt);
    ...
    SPDK_DEBUGLOG(bdev_malloc, "wrote metadata %zu bytes to offset %#"
    PRIx64
    "\n",
            md_len, md_offset);
...
bdev_malloc_copy(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
    ...
    SPDK_DEBUGLOG(bdev_malloc, "Copy %zu bytes from offset %#"
    PRIx64
    " to offset %#"
    PRIx64
    "\n",
...
bdev_malloc_submit_request(struct spdk_io_channel *ch, struct spdk_bdev_io *bdev_io) {
    ...
    else if (_bdev_malloc_submit_request(mch, bdev_io) != 0) {
