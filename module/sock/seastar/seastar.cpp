...
{
	if (sock->write_packets.empty() || sock->write_in_progress) {
		return;
	}
	...
	(void)sock->output.write(std::move(p)).handle_exception([](auto e) {
		std::cout << "Exception: " << e << "\n";
	}).then([sock] {
		return sock->output.flush();
	}).handle_exception([](auto e) {
		std::cout << "Exception flush: " << e << "\n";
	}).then([sock] {
...
spdk_seastar_sock_writev(struct spdk_sock *_sock, struct iovec *iov, int iovcnt)
{
	...
	for (i = 0; i < iovcnt; i++) {
		...
		p.append(seastar::net::packet(seastar::net::fragment{buf, len}, seastar::make_free_deleter(buf)));
		bytes += len;
	}
