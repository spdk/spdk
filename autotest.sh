...
if [ $SPDK_RUN_FUNCTIONAL_TEST -eq 1 ]; then
	...
	if [ $SPDK_TEST_BLOCKDEV -eq 1 ]; then
		run_test "suite" "blockdev_general" test/bdev/blockdev.sh
	...
	if [ $SPDK_TEST_NVME -eq 1 ]; then
		run_test "suite" "blockdev_nvme" test/nbdev/blockdev.sh "nvme"
		run_test "suite" "blockdev_nvme_gpt" test/nbdev/blockdev.sh "gpt"
	...
	if [ $SPDK_TEST_NVMF -eq 1 ]; then
		if [ "$SPDK_TEST_NVMF_TRANSPORT" = "rdma" ]; then
			run_test "suite" "nvmf_rdma" ./test/nvmf/nvmf.sh --transport=$SPDK_TEST_NVMF_TRANSPORT
			run_test "case" "spdkcli_nvmf_rdma" ./test/spdkcli/nvmf.sh
		elif [ "$SPDK_TEST_NVMF_TRANSPORT" = "tcp" ]; then
			run_test "suite" "nvmf_tcp" ./test/nvmf/nvmf.sh --transport=$SPDK_TEST_NVMF_TRANSPORT
			run_test "case" "spdkcli_nvmf_tcp" ./test/spdkcli/nvmf.sh
		else
			echo "unknown NVMe transport, please specify rdma or tcp."
			exit 1
		fi
	fi
	...
	if [ $SPDK_TEST_PMDK -eq 1 ]; then
		run_test "suite" "blockdev_pmem" ./test/bdev/blockdev.sh "pmem"
		run_test "case" "pmem" ./test/pmem/pmem.sh -x
		run_test "case" "spdkcli_pmem" ./test/spdkcli/pmem.sh
	fi

	if [ $SPDK_TEST_RBD -eq 1 ]; then
		run_test "suite" "blockdev_rbd" ./test/bdev/blockdev.sh "rbd"
	...
	if [ $SPDK_TEST_CRYPTO -eq 1 ]; then
		run_test "suite" "blockdev_crypto" ./test/bdev/blockdev.sh "crypto"
	fi
fi
