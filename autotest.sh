...
if [ $SPDK_RUN_FUNCTIONAL_TEST -eq 1 ]; then
	...
	run_test suite "tcp" test/spdkcli/tcp.sh
	...
	if [ $SPDK_TEST_NVME -eq 1 ]; then
		...
		if [[ $SPDK_TEST_NVME_CLI -eq 1 ]]; then
			run_test suite "spdk_nvme_cli" test/nvme/spdk_nvme_cli.sh
		fi
		if [[ $SPDK_TEST_NVME_CUSE -eq 1 ]]; then
			run_test suite "spdk_nvme_cli_cuse" test/nvme/spdk_nvme_cli_cuse.sh
		fi
		# Only test hotplug without ASAN enabled. Since if it is
		# enabled, it catches SEGV earlier than our handler which
		# breaks the hotplug logic.
		if [ $SPDK_RUN_ASAN -eq 0 ]; then
			run_test suite "hotplug" test/nvme/hotplug.sh intel
		fi
	fi
	...
	if [ $SPDK_TEST_ISCSI -eq 1 ]; then
		...
		run_test suite "iscsi" ./test/spdkcli/iscsi.sh
	...
	if [ $SPDK_TEST_VHOST_INIT -eq 1 ]; then
		...
		run_test suite "virtio" ./test/spdkcli/virtio.sh
